...
**🆕 追加報告(2026-08-14夜、上記デプロイ後): 全体は軽くなったが「連動アニメ自体だけFPSが低い」**。原因はスクロール中も80〜400msの「追いつき」transitionを掛けていたため、進み具合の書き換え頻度(約16〜20msおき)にtransitionが追いつけず常に目標を追いかけ続ける状態だったこと。スクロール中(`data-mobile-scrolling="1"`)は`transition: none`にして指の位置に直結、指を離した後の戻り演出だけ既存の400ms easingを使う方式に変更(`src/index.css`)。ビルド・型チェック・関連テスト通過、transition-durationが意図通り(スクロール中0s→離した後0.4s)であることも実測確認済み。**まだユーザー実機未確認・push前**。次セッション最初: push前ゲート→push→デプロイ→ユーザー実機確認。
