...
- **🟣 スプシ軽減表取り込み: 実装完了・push前・要実機確認(2026-06-21)** ブランチ `feat/spreadsheet-import`。純粋関数4本(parse/skill解決/party/build)+ウィザードmodal+i18n(ja/en/ko/zh)+配線。SDD全6タスク完了・最終レビュー[opus]=Ready to merge=Yes(Critical/Important 0)。build EXIT0・sheetImport 28/28緑。**既存取込(FFLogs/importTimelineEvents/importModes)無改変を branch 全体で確認済**。新規プランは `commitNewPlan` で安全作成(計画書の createPlanFromTemplate は誤り→修正)。未対応8技はskip集計。**残(ユーザー)**: ①`npm run dev`で実データ全フェーズ取込→プレビュー→作成を実機確認 ②モーダル/入口ボタンの見た目承認(ui-design承認フロー) ③5/5上限の破壊チューザー(置換/削除)はv1未実装=安全停止のみ→full chooser要否を判断。OK後 main merge+デプロイ。詳細=`.superpowers/sdd/progress.md`。
