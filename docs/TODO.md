...
- **最新セッション（2026-05-08 後半・Sub-spec 2A 実機テスト + バグ修正 + §5 認証モデル漏れ判明）**: 朝のセッションで Sub-spec 2A 完成・本番デプロイ後、ユーザー実機テストで複数の課題発見。①ログインプロンプトが `/miti` リダイレクト型だったので、その場で LoginModal を開く形に修正 (Discord/X 両対応のまま) ②本番 `/api/housing?action=can-register` が ERR_MODULE_NOT_FOUND で 500 → src/utils/housing{Quota,Validation,Duplicate}.ts の相対 import に `.js` 拡張子追加 (Vercel ESM ランタイム要件) ③HousingTabBar 右端に ConsolidatedHeader と同じプロフィールアバターボタン追加 (アカウント切替/ログアウト導線) ④軽減表側への影響ゼロを Playwright + grep + diff の 4 点セットで証明。**重要発見**: ハウジング設計書 §5 認証モデル (アカウントリンク・featureSessions opt-in・オンボーディング選択フロー) を Sub-spec 2A 実装プラン作成時に Claude が完全に見落としていた。設計書 §5.3 の `linkWithCredential` は LoPo 認証プライバシー原則 (DESIGN_DECISIONS.md 2026-04-04) と矛盾するため、自前 uid マッピング方式 (`account_links/{provider:id} → primaryUid`) で実装する方針合意。次セッションは **Phase A 補完** = ①HousingRegisterView リアクティブ認証購読化 (ログイン直後リロード必要バグ修正) ②featureSessions opt-in 書き込み ③オンボーディング選択フロー化 ④i18n + テスト + デプロイ。工数半日〜1 日。アカウントリンクは **Phase B** で別セッション (1.5-2 日)、Sub-spec 2B 着手前に。
...
### 次にやること（優先順）
- **【最優先・次セッション】Sub-spec 2A Phase A 補完 (§5 認証モデル実装)**: 設計書 §5 認証モデルが Sub-spec 2A プラン段階で漏れていたため補完。①HousingRegisterView を `useAuthStore` 経由のリアクティブ購読に変更 (ログイン直後リロード必要バグ修正) ②`users/{uid}/featureSessions/housing.activated` の opt-in 書き込み (§5.2) ③HousingOnboardingDialog を「同じアカウントで OK / 別のアカウントでログインし直す」選択型に改修 (§5.2) ④i18n 4 言語 ⑤テスト + tsc + build + デプロイ。工数半日〜1 日。詳細メモリ: `project_housing_phase_status.md` / 認証原則: `feedback_auth_privacy.md`
- **【次々セッション】Phase B アカウントリンク機能**: Discord と Twitter の uid 紐付け。LoPo 認証プライバシー原則維持のため自前マッピングテーブル方式 (`account_links/{provider:id} → primaryUid`)。ユーザー UX は LoginModal に「他プロバイダ連携」ボタン → 30 秒で完了。バックエンド: 連携 API + 既存ログインの分岐 + 解除 API。フロントエンド: LoginModal 拡張。工数 1.5-2 日。Sub-spec 2B 着手前に実装が望ましい。
