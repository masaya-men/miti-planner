...
- **直近セッション（2026-04-18 後半）**: 学者エーテルフロー仕様を全面刷新。スキル化＋自動配置＋ポップアップ実装、Pattern 切替ボタン廃止。デプロイ済み、実機確認は次セッション。
...
### 次セッションで実機確認すること（学者フロー刷新 2026-04-18）
- [ ] SCH をパーティに新規投入 → 転化 @ t=1 + エーテルフロー @ t=13, 73, 133... が自動配置されるか
- [ ] エーテルフローを手動で置く → 「リキャストごとに配置しますか？」ポップアップが出るか
- [ ] ポップアップ「はい」で 60s 毎の連鎖配置が走るか
- [ ] 既存プラン（Pattern 2 で作ったもの）を開くと、マイグレーションでエーテルフローが追加されるか
- [ ] エーテルフロー/転化をドラッグ移動・削除 → フロー stack 判定が追従するか
- [ ] ムービー中などで手動ずらし → 期待どおり動くか

### 既知の残課題
- 韓国語「野良主流」ボタン `야생 메타` の文言見直し
- ヘッダー縦罫線 2 番目のサブピクセル残存（実害なし、放置判断）
- エーテルフロー仕様に関する追加の細かい調整（次セッション着手前に指示待ち）
