...
- **今セッション完了**:
  - FAB言語切替を展開式セレクターに改善（spring staggered横一列展開）
  - SPRING.bouncyトークン追加（damping:15のバウンスプリセット）
  - 選択時アニメーション（選択チップscale拡大→吸い込み、他は逆stagger）
  - ビルド成功・テスト116全パス

### 次セッションでやること（バグ修正）
1. FAB言語セレクターの実機微調整（チップ位置・サイズ・ラベルとの間隔）
2. 長押しチュートリアルをチュートリアルカード内に組み込む（現在は下部バナー表示）
3. スマホUI実機確認・微調整（透過感、余白、フェーズラベル位置など）
