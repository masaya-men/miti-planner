...
- **ブランチ**: main直接
- **デプロイ済み**: FFLogsマッパーV6.0 + スキル管理画面統合 + フォーカスモード修正
...
- **確認済み**: FFLogsインポートV6.0 — 詠唱/ダメージGUID違いで重複残るが現状維持（ユーザー削除で対応）
- **確認済み**: スキル管理画面 — 見た目OK、実運用はこれから
- **修正済み未確認**: フォーカスモード時の雲マーク非表示（本番で確認予定）
- **実装済み未確認**: 翻訳管理画面（/admin/translations — スキル/コンテンツ/攻撃名/フェーズ/その他のzh/ko翻訳、CSV連携）
...
### 中（特定環境・管理者向け）
- [ ] ジョブ名ツールチップが言語設定に追従しない（韓国語モードでも日本語表示）
...
### 管理画面改善
- [x] スキル管理画面の実装（追加/編集モーダル完成）
- [x] 翻訳管理画面（カテゴリ別テーブル + CSVインポート/エクスポート + シリーズ名Firestore化）
