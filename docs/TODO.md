...
0-1. **✅ 共同編集: オーナー自動判別 + 人数変更確定ボタン: 2026-08-07 実装完了(TDD 6タスク+最終レビュー、本番デプロイは実機確認後)**。①オーナーが自分の共有リンクを開くと自動でいつもの編集画面(`/miti`)へ案内(`check-owner` API + `CollabJoinerPage.tsx`)。②「入れる人数」を仮の値+明示的な確定ボタン方式に変更、保存漏れバグも修正。**重要な設計変更**: 最終レビューで「オーナー自動判別のリダイレクトが誤って別プランのデータを共同編集ルームへ不可逆に上書きしうる」経路が判明したため、案内条件を「そのタブで既にこのプランを開いている場合のみ」に絞る安全側ガード(`canOpenOwnerEditor`)を追加(=別プランを開いた状態でリンクを踏んだ場合は従来通り参加者ビューのまま。今後拡張するなら`/miti`側でのプラン安全ロード導線が別途必要)。詳細=`.superpowers/sdd/2026-08-07-collab-owner-link-and-live-capacity/final-review-fix-report.md`(worktree内)。次は実機確認(下記チェックリスト)→本番push。
...
### 🟡 共同編集: オーナー自動判別+人数変更確定ボタンは実装・全レビュー完了(詳細→最上部0-1番、worktree `collab-owner-link-and-live-capacity`)。次は実機確認(①別タブで自分のリンク→編集画面に飛ぶ ②他人/未ログインは従来通り参加者画面 ③人数+/-→確定ボタン→閉じて開き直しても保持 ④確定せず閉じたら元の値に戻る)→ユーザーOKでworktreeをmainへ統合・本番push。
...
- **🔴 共同編集: 別プランのデータが他人の部屋へ不可逆pushされうる既存経路(2026-08-07発見・未修正・要判断)**: `/collab/:token`で一度でも編集同意した端末は再訪時に自動で`readOnly:false`昇格→sync時の`reseedEmptyDocFields`(空フィールド再シード)が、その時点で作業ストアに載っている**別プランの中身**を部屋のFirestore/DOへ押し込みうる(全参加者に見える形で不可逆)。オーナー自身の端末でも発生しうる。オーナー自動判別機能の実装計画外で見つかった既存バグ(このブランチが原因ではない・本ブランチはこの経路を悪化させていない)。対策案=参加者は必ず一度`readOnly:true`で入室しreseedはオーナーセッションのみに限定、等。要brainstorming。
- **🟡 共同編集: マルチタブでのstale read残余(2026-08-07発見・優先度低)**: `usePlanStore`はタブ間でin-memory同期しない(storageイベント購読なし)。同一ユーザーの2タブで「タブAでプラン切替→タブBで/collabリンクを踏む」が同時発生すると、上記reseed経路に乗る狭い窓が残る。発生条件が狭く既存由来のため急ぎ不要。
