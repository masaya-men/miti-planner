...
### 5.0 Revision 2 (2026-05-08 実機テスト後)

**Revision 1 (§5.1〜5.6) は実機検証で「ダイアログが見えないうちに取り込みが完了している」問題が判明し撤回。** ユーザーとの再ブレストで以下方針に変更:

#### 設計変更の核心

- **アップロード自体は黙って行う**: ログイン直後 `migrateLocalPlansToFirestore` がローカルプランを Firestore に保存する (元の設計に戻す)。これにより取り込み確実性を担保
- **ダイアログは「確認 + 取捨選択」**: 既にアップロード済みのプランを **リスト表示**、チェックボックスでユーザーが取捨選択
- **取捨選択の意味**:
  - チェック ON (デフォルト) → クラウドに残す (= 引き継ぎ完了)
  - チェック OFF → 裏で Firestore から削除 + ローカル `ownerId='local'` に戻す。ユーザー視点では「ローカルだけに残る」
- **「削除」という言葉はユーザーに見せない**: ユーザーの認知モデル「取り込むかどうか選ぶ」を尊重し、文言は「クラウドに保存」「ローカルにだけ残ります」

#### 失敗時のフェイルセーフ (3 重)

1. **削除失敗の自動リトライ**: 既存 `_deletedPlanIds` キューを流用、`syncToFirestore` の次回呼び出しで自動再試行
2. **ローカルデータ保持**: チェック外したプランは Firestore 削除 + ローカル `ownerId='local'` 復元 → 次回ログイン時に再ダイアログ対象
3. **誤操作耐性**: 背景クリック / ESC / X ボタンを **無効化**、ボタンを押さずに離脱した場合は「全部キープ」状態 (= データロスゼロ)

#### タイミング (アニメーション)

ログイン後の認証オーバーレイがフェードアウトしてから次の順で表示:

1. 認証オーバーレイ消去後 **700ms 待機** (ユーザーが画面を視認する余裕)
2. ダイアログ `dialogIn 250ms` cubic-bezier スプリング (Revision 1 の 200ms から延長)
3. リストアイテム stagger 表示: 各 80ms 遅延でフェードイン
...
- glass-tier3 ダイアログ、幅 480px (リスト表示用にやや大きめ)
- 各行: コンテンツ名 (太字) + プラン名 + パーティメンバーアイコン (4-8 個、jobId 由来)
- リスト最大表示数: 5 件 (それ以上はスクロール)
- 「キャンセル」: 何もしない (全プランキープ)
- 「これで OK」: チェック OFF のものを Firestore 削除 + ownerId='local' 復元
- ESC / 背景クリック / X ボタン: **無効** (誤操作防止)

#### Revision 1 で実装したコードの扱い

- `localImportPlanner.ts` の `computeImportPlan`: Revision 2 では使わない (新 ID 発行・採番ロジック不要)。**削除する**
- `usePlanStore.importLocalPlans` action: Revision 2 では `applyLocalImportSelection(uid, uncheckedPlanIds)` に置き換え
- `LocalImportDialog.tsx`: `LocalImportListDialog.tsx` に置き換え
- `useLocalImportDialog` ストア: `planList: SavedPlan[]` フィールド追加で拡張
- `migrateLocalPlansToFirestore` の `localOnly` upload loop: **復活** (Task 3 のリバート、ただし `uploadedIds` を返り値に追加)
- Layout 統合: `migrateOnLogin` 完了後、`pullFromFirestore`、`isAuthRedirecting=false`、700ms 待機 → ダイアログ open
- LoginModal 明示ボタン: そのまま (ダイアログ呼び出し先だけ更新)

#### 動作シナリオ別検証 (Revision 2)

| シナリオ | 期待動作 |
|---|---|
| ログアウト中 3 件作成 → ログイン | 認証オーバーレイ消去 → 700ms → ダイアログでチェックリスト 3 件全 ON で表示 → OK → クラウドに 3 件残る |
| 同上 → 1 件チェック外し → OK | クラウドに 2 件残る、外した 1 件はローカルに `ownerId='local'` で残る |
| ダイアログ表示中に X ボタンや ESC | 反応しない (無効化済み) |
| 「キャンセル」クリック | 何もしない (全 3 件クラウドに残る) |
| 「次回から表示しない」 + OK | フラグ保存、次回ログイン時はダイアログ自動表示なし |
| ネットワーク切断中に OK | 削除失敗 → `_deletedPlanIds` キュー入り、次回 syncToFirestore で再試行、ローカル状態は即時反映 |
| LoginModal 明示ボタンクリック | フラグ無視 + チェックボックス非表示でリスト表示 (常に全 ON で確定) |

---

### 5.1 トリガー

> **以下 §5.1〜5.6 は Revision 1。Revision 2 では §5.0 が優先。** 文書として残すが新規実装の参照先は §5.0 とすること。
