...
- **最新セッション（2026-04-28 終盤・3 段目）**: 2 段目の `key={ogImageUrl}` でも治らないことが実機で判明。原因は **過去セッションと同じ imageHash になるケース**で setOgImageUrl(同値) が React の state bail-out で何もせず、`<img>` も再レンダリングされないこと。3 段目として **`updateShareLogo` 冒頭で `setOgImageUrl(null)` を挟む**ことで `<img>` を確実にアンマウントしてから新 URL で再マウント。await fetch をまたぐので React の render が間に入り、`<img>` が一度消えてから新 src で再生成される。build+test 244 PASS。
...
### 次セッションで実機検証すること（OGP プレビュー 2 段目修正 2026-04-28 終盤・続き）
- [x] 共有モーダルでロゴ削除→ロゴ無し OGP が即生成・表示されること（1 段目で OK 確認済み）
- [ ] 共有モーダルでロゴ新規アップロード→ロゴ有り OGP が即生成・表示されること（2 段目修正対象）
- [ ] 連続でロゴ削除→ロゴ追加→ロゴ削除...と切り替えても毎回プレビューが更新されること
