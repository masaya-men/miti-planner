...
- **🚧 2026-06-18 攻略の進捗トラッキングHUD spec化+Plan1作成済 (次=Plan1実装)**: 共同編集の「進捗管理UI/UX」。設計書=[specs/2026-06-18-progress-tracking-hud-design.md](./superpowers/specs/2026-06-18-progress-tracking-hud-design.md)。実装計画=[plans/2026-06-18-progress-tracking-hud-part1.md](./superpowers/plans/2026-06-18-progress-tracking-hud-part1.md)(本体・ソロ完結。collab同期はPlan2で別途)。軌跡の光=試作`4c0b94b`を1:1移植・勝手に変えない([[feedback_keep_liked_prototype_visuals]])。**実装ブランチ`feat/progress-tracking-hud`作成済(main 4ac4c90起点・未着手)・ledger初期化済(.git/sdd/progress.md)**。次=subagent-driven-developmentでPhase A(データ層) Task A1から。確定=軌跡(日ごと最高到達点の非単調な波・棒グラフ+SVG線の尾)/進捗%デフォ表示/**collab限定せず全プラン対象**・端末グローバル・デフォルトON・ワンクリックでオフ・⋯メニュー復帰・空状態は誘導型/データ=`PlanData.progress?`(表ごと・既存保存に相乗り・コスト塵)/記録=HUDクリック→タイムライン時間クリックで打点+クリアお祝い(confetti+スキルアイコン降り)/活動日数・時間=任意オプション・デフォ非表示/**活動○×はスケジュール調整機能と一緒に後で(今回スコープ外)**。試作は`feat/progress-celebration-proto`(4c0b94b)に保全=**見た目は本実装へ移植して1:1再現・移植完了まで撤去しない**。光の尾だけ未確定(実機で詰める)。次フェーズ=スケジュール調整機能(別spec)。
...
- **🚧 2026-06-19 進捗HUD イテレーション2 + ドロワーUI磨き + 共有除外 (branch `feat/progress-tracking-hud`・HEAD `75bcc59`・未push/未merge/未deploy=エンドユーザー不可視)**: イテレーション2本体(A記録ハイライト根治/Bトースト/C記録ドロワー/Dホバー停止根治)完了。本セッションで**ドロワーUIを実機合わせで磨き込み(16コミット)**: 横位置=画面中央 / ライト白基調 / トーストをテーマ対応(light青み抑制) / 開閉ミラー演出(開360ms・閉153ms) / 外側クリック&帯再クリックで閉じる / U型でヘッダー下端ハンドル線に密着(引き出し化) / コンパクト3段+CLEAR!をライン上 / 活動±撤去+ダブルクリック直接編集+固定幅(ガタつき防止)+(任意) / 戻るボタンをドロワー右下に常時表示+LoPo Tooltip(最新打点1つ削除)。設計=specs/2026-06-19-progress-hud-iteration2-design.md / 経緯=[.private/2026-06-18-progress-hud-iteration2.md](./.private/2026-06-18-progress-hud-iteration2.md)。memory `reference_waapi_transform_clobbers_centering`。
  - **★共有コピーから個人状態(progress/memos)を除外＝二重防御 (2026-06-19・要次セッション検証)**: 共有(コピー)に進捗とメモを載せない。`stripSharedPersonalData`(非破壊・TDD5件)/ **Layer1=api/share POST strip**(保存時・単一+bundle) / **Layer2=api/share GET strip**(取得時・全コピー経路[buildNewPlan/MitigationSheet]+プレビュー+旧共有を網羅) / **クライアントbelt**=ShareModal送信前/buildNewPlan/MitigationSheet.copyPlan。**getSnapshot・自分のプラン保存(syncToFirestore)・BackupExportは不変=自分のデータ無傷**。build EXIT0/関連36テスト緑。コピー経路は敵対監査で洗い出し済。
  - **✅ データロスト/破損ゼロ 多エージェント敵対監査 完了 (2026-06-19・77 agents/6レンズ/66検証)**: 結論=**コード経路上、自分の progress/memos 消失リスク=ゼロ / undefined クラッシュ=ゼロ / 要修正の実問題=ゼロ**。根拠: ①strip は共有/コピー境界の **4 本番呼び出しのみ**(api/share・ShareModal・MitigationSheet・buildShareImportItems)、owner-write 経路(getSnapshot を使う syncToFirestore/persistWorkingStore/BackupExport/Sidebar 保存・複製/SyncButton 等)は strip 非経由=自分のデータ無傷。strip は非破壊(浅クローン+delete・null安全)。②受け手側 loadSnapshot が `memos ?? []`/`normalizeProgress(progress)` で undefined 吸収、preview/OG/popular/template は progress/memos を読まない(grep 0件)。strip 後の形=本番に既存の旧プラン(undefined)と同形で既存要件。完全性批判3 surface(compression/persist partialize/型エラー)も追加監査で安全確認。**残るのは privacy 物理残存のみ**(下記)。**判定=E2E後マージ可。検証OKまで merge/deploy しない。**
  - **🔴 デプロイ後E2E必須(観点③・api/share strip は vite dev 非実行)**: (a)自分のプランに進捗+メモ→共有→自分の元プランで両方残存 (b)共有先でコピー→コピーに progress/memos が一切無い (c)bundle 同様 (d)人気カードから直接コピー (e)旧共有(strip前保存)を GET→コピーで Layer2 strip が効く (f)OG画像が正常生成。**全項目OKならマージ可**。
  - **🟡 別タスク(privacy・低・マージ非ブロッカー)**: Layer1 導入前に保存済みの旧 shared_plans doc は Firestore に progress/memos が物理残存(応答は Layer2 GET strip で遮断済=漏洩経路は閉・周辺経路popular/template/OGも progress/memos 非射影で安全)。完全消去には旧 doc 一括 strip sweep マイグレーションが必要(データロストではなく privacy 残存)。/ MitigationSheetPreview の timelineEvents/partyMembers が `?? []` 無し=**本変更非起因の先行リスク**(strip は core を削らない)、別途防御の価値あり。
  - **🔴 残(検証後)**: ③進捗一括リセット機能(未着手・要否ユーザー確認) → ユーザー実機総点検(A/B/C/D+4言語+記録モードON/OFF回帰+collab viewer) → main merge=本番デプロイ(デフォルトON・ヘッダー直撃)。次フェーズ=スマホ記録対応・Plan2 collab同期。
...
- **⚠ ブランチ要確認 (2026-06-18 発覚)**: 下記3件(管理画面リデザイン/サンドボックス/バグ修正3件)が載る `feat/mobile-bottom-nav-redesign` が**ローカルブランチ一覧から消失**。コミット自体は git 内に残存(管理画面=609fab9〜826d1c8 / バグ修正含む)。**作業消失か単なるブランチ削除/改名か未確定**。次セッションで「main にマージ済か / 別ブランチへ退避か / dangling か」を要特定(`git log --all --oneline --grep=AdminPage` 等)。ユーザー認識=管理画面/スマホ最適化は**まだ未完成**(2026-06-18)。
...
- **🟡 共同編集 再接続時の「一部欠け」データ消失 (2026-06-18 解析・near-term backlog・先送り合意)**: collab 部屋を離れて固まる前に戻ると、サーバーが「他人の軽減は有るが自分の直前のドロー等だけ欠けた」状態を返し、空上書き防御(まるごと空のみ保護・[collabReseed.ts:15](../src/lib/collab/collabReseed.ts#L15))をすり抜け消える。根=再接続で new Y.Doc+丸呑み / 引き金=保存debounce(5-15s)前の素早い切替。確率低・被害は直前の一部のみ・退室時flush等の保険有り。直し方A(離脱側=確定待ち・安価)→near-term / B(再接続側=一部欠け補完・根本)。詳細・運用回避=[docs/.private/2026-06-18-collab-reconnect-partial-loss.md](./.private/2026-06-18-collab-reconnect-partial-loss.md)。**Undo機能とは別件**(あちらは3回監査+実機確認済で安全)。
