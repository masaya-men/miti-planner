...
0-1. **🔴 共同編集: オーナー自動判別 + 人数変更確定ボタン(2件セット・未実装)**: ①オーナーが自分の共有リンクを踏むと参加者扱いになり編集権限を失う ②「入れる人数」の+/-が仮の値のままで、確定ボタンが無い(閉じ直すと保存前の値に戻るバグも既知)。2026-08-07朝に設計書・実装計画は作成済み(`docs/superpowers/specs/2026-08-07-collab-owner-link-and-live-capacity-design.md` / 実装計画は同日コミット`9474aad9`、TDDで6タスク)、コードは未着手。今回のクラッシュ修正とは無関係の既知バグ(ユーザーが同日実機で両方とも再確認)。次はTDDで実装計画どおり着手。
...
## 現在の状態 (次セッションはここから読む)
### 🟡 共同編集participantクラッシュ+副次2件(強制展開/FAB無し)は修正・本番デプロイ済み(詳細→最上部0番)。実機で「外部共有リンクからの参加(展開されずFABが使えるか)」を確認待ち。オーナー自身のリンクは0-1番(別問題・未実装)。
