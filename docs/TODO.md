...
- **✅ タンクスイッチ(挑発スキル) 本番デプロイ済 (2026-06-15・main マージ→push→Vercel `lopo-bxng07k8v` Ready / seed-icons+seed-skills-stats ADDITIVE 実行済・dataVersion++)**: 挑発を置くと同一フェーズ内・以降の攻撃の on対象 MT⇄ST を反転(derived/非破壊)。中核=純粋関数 `getEffectiveTarget`(TDD 10緑)+全計算/表示サイト適用(Timeline/CheatSheet/PC行/スマホ行)+recast30s通常スキル(`isTankSwap`)+アイコン切替アニメ+autoPlan除外。最終レビューで1件発見→修正済(CheatSheet行表示/致死)。build緑/test 1710緑(既知5のみ)。seed は挑発4件のみ追加・既存164無変更を dry-run で実証。設計=specs/2026-06-15-tank-switch-provoke-design.md / 計画=plans/2026-06-15-tank-switch-provoke.md。**残=本番実機検証(下記)。**
