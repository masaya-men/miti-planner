...
- **🚧 同期安定化 Step1+2 実装済・未デプロイ (2026-06-03 本セッション)**: 業界水準のソフトデリート(墓標)+墓標ベースマージを TDD で実装。「別端末で消失/削除→復活」を根治。**全テスト緑(同期23/全体1379)+build緑だが commit/push/デプロイは未実施 (オフピーク段階投入はユーザー判断)**。新規 `src/lib/mergePlans.ts`、`planService.ts`(deletePlan墓標化/fetchPlansAndTombstones/fetchAndMerge/migrate/updatePlan/syncDirtyPlans)、`types`、`firestore.rules`(plans update 分岐)。詳細・**デプロイ手順(rules先→push→実機検証)**・残タスクは **[docs/.private/2026-06-03-realtime-collab-and-sync-notes.md](./.private/2026-06-03-realtime-collab-and-sync-notes.md) Phase5**。残=Step3 unload確実化 / `_deletedPlanIds`永続化 / Step4 onSnapshot(共同編集土台) / 墓標GC cron。
