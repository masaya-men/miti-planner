...
## 現在の状態 (次セッションはここから読む)
### ✅ 画像品質バグ3件 = 根因特定・修正済(2026-07-20・未push)
`alwaysKeepResolution:true`(解像度極小化を防止)+ WebP非対応時はJPEGへフォールバック(PNG画質劣化を防止)+ 圧縮3段階化(413によるカバー画像消失を構造的に防止)。詳細→[TODO_COMPLETED.md](./TODO_COMPLETED.md)。unit test 5件+build+vitestフルグリーン確認済。**古いSafari実機では未検証**、次の登録ラッシュでPNGフォールバック登録が0件になるか数日ようすを見る。

**次(ユーザー要望)**: 複数投稿URL機能(Batch 2)完了後、登録UIで「URL経由登録」を理由付きで目立つように推奨する方向へ改修(直接アップロードよりURL経由の方が画質・枚数・動画対応で優位なため)。着手時はbrainstormingから。
