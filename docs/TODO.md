...
1. 🔴🔴 **ハウジング大規模耐性ハードニング + 住所非公開機能** (超大物・設計完成・承認済)
   ...
   - **🛑 P2(認証コスト削減)=コード完成・マージ可能・G4手前で停止(2026-07-15 深夜)**: ブランチ`feat/appcheck-lazy-init`(5コミット・未マージ/未push)。App Check を eager→遅延初期化(匿名閲覧で reCAPTCHA 発火せず・書込/ログインで初期化)。build緑/vitest 3315pass 0fail。全SDDゲート通過(最終Opusレビューで匿名到達3経路の403回帰を検出→fix→再レビュー Ready to merge: Yes)。**明日=ユーザー go でマージ+push→Claude curl確認→G4実機(匿名reCAPTCHA無+ログイン書込+今回修正3経路)**。手順・G5(App Check強制)・follow-upは全て`.superpowers/sdd/progress.md`の「🛑現在の状態」節。**残フェーズ**=P3(住所非公開・G6デザイン承認/G7住所漏れ確認)→発表(Discord淡々テンプレ)。
