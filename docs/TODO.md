...
- **ブランチ**: main直接
- **段階1+2完了**: Phase/Label startTime統一リファクタリング完了（ビルド・テスト通過）
- **未push**: 段階2全コミット（まとめてpush→デプロイ1回で済ませる）
- **注意**: ENFORCE_APP_CHECK=true、Vercel関数7/12、月100ビルド制限
- **同期設計**: 5分クールダウン(自動のみ)、初回editは即push、タブ切替/離脱/手動は即push、競合時は両版コピー保存

### 次セッションでやること
1. push → デプロイ確認
2. 本番動作確認（ラベル編集/追加/削除、旧データ自動変換、FFLogsインポート、テンプレート）
3. 次の優先タスクに着手
...
### Phase/Label リファクタリング（完了）
- [x] 設計書作成・承認
- [x] 段階1（フェーズ）実装（11タスク完了、デプロイ済み）
- [x] 段階2（ラベル）実装（12タスク完了、ビルド・テスト通過）
