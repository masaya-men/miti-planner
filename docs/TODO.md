...
## 現在の状態 (次セッションはここから読む)
### 🔴🔴 次セッション最優先(緊急・2026-07-20 実ユーザー報告で発見)
**画像登録が5枚目以降サイレント失敗するバグ**: 登録フォームは12枚まで選べるが、サーバー(`api/housing/_uploadThumbnailHandler.ts`)は`MAX_IMAGES_PER_LISTING=4`のハードコードで5枚目以降を拒否。UIヒント文言も「先頭4枚保存」で古いまま矛盾。**実ユーザーが9枚登録→1枚しか表示されないと報告**(4枚残るはずが1枚のみ・原因未特定)。詳細調査ログ・file:line引用・仮説候補=`docs/.private/2026-07-20-housing-image-upload-4-limit-bug.md`。次回 systematic-debugging から。
