...
- **🟣 進行中=⑦敵攻撃 "or"(2択攻撃)**: TimelineEvent に任意 `altName: LocalizedString` を持たせ「A or B」表示。**名前だけ変わる(ダメージ同じ)確定**。v1=手動編集(EventForm+管理TemplateEditor)+描画(TimelineRow)、新ブランチ。スプシ自動分割は後追い。brainstorming 中(次=設計案提示→spec)。詳細=docs/.private/2026-06-15-feature-ideas-batch.md の⑦。
