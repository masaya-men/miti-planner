...
- **🔴 最優先ブロッカー=Vercel デプロイ失敗 (2026-06-08)**: 詳細・再開手順は **[docs/.private/2026-06-08-vercel-deploy-blocker-handoff.md](./.private/2026-06-08-vercel-deploy-blocker-handoff.md)** に集約(次セッションはまずこれを読む)。要点: ⑤-2a(ルーム管理API+ワーカー結線)を main にマージ(`ada3c69`)したが本番デプロイが失敗中。**型エラーは全部修正済(api/ がローカル tsc 対象外だった根因を特定→`tsconfig.api.json`新設+`npm run build`に統合)。残る失敗は型でなく `Deploying outputs` 段階(Hobby の Serverless Functions 数上限が最有力・未確認)**。本番は半日前の成功版で安定・collab 休眠でユーザー影響なし。**⑤-1 も実は未デプロイだった疑い**(同型エラーを保持してたため。`ada3c69` 成功時に⑤-1+⑤-2a 同時反映)。collab Worker は wrangler deploy 済(`c21b2bd1`)。**次=この失敗の原因特定(Vercelダッシュボードのエラー文確認)→解消** → その後 ⑤-2b(満員拒否)→⑤-3(クライアントUI)。**UI は完成まで非露出継続**。
