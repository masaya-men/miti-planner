...
0-1. **✅ 共同編集: オーナー自動判別+人数変更確定ボタン(6タスク)+reseedEmptyDocFields信頼境界バグ修正(7タスク+最終レビュー指摘の追加修正)+プラン複製3経路のactiveCollabRoomToken引き継ぎバグ修正、全て実装・検証完了・本番push済み(`91f99e65`、2026-08-08)**。背景=オーナー機能の最終レビュー中に見つけた「別プランのデータが共同編集ルームへ不可逆に混入しうる」経路(`reseedEmptyDocFields`が手元データの出処を検証しない設計)は、新機能とは無関係に前から存在する、参加者側でも起こりうる既存バグだった。`_loadedPlanId`永続化/`canTrustLocalDataForRoom`ガード/reissue順序修正/起動時の札食い違い検出、加えて最終レビューで見つかった圧縮済みプランのフェイルオープン・プラン複製時のトークン引き継ぎも修正。詳細=`docs/.private/2026-08-07-collab-data-safety-audit/00-synthesis-and-next-steps.md`(調査)+`docs/superpowers/specs/2026-08-07-collab-reseed-trust-boundary-design.md`(設計)+`docs/superpowers/plans/2026-08-07-collab-reseed-trust-boundary.md`(実装計画)。**実機確認待ち**=共同編集が普段通り使えるか。
0-2. **✅ 学者「マニフェステーション」追加+「アクセッション」潜在力修正、本番push済(`28bd5001`、2026-08-08)**。セラフィズム中に鼓舞激励の策が変化する仕様が未実装だった(公式ジョブガイド日英+複数一次情報源で裏取り、治癒力360/バリア648)。調査中、アクセッションのmockData.ts側shieldPotency(100)が実際の計算式(calculator.ts、432)と食い違っていたのも発見・修正(該当フィールドは管理画面フォーム以外未使用と確認済み・実害なし)。Firestore(`/master/skills`)にも反映済み(マニフェステーションはADDITIVE seed、アクセッションはフィールド単体の個別パッチ)。韓国語名(현시)はユーザー確認済み。**実機確認待ち**=学者Lv100でマニフェステーションが選択・表示できるか。
...
### ✅ 共同編集: オーナー自動判別+人数変更確定ボタン+reseed信頼境界修正、全て実装・検証完了・本番push済み(詳細→最上部0-1番)。実機確認待ち。
### ✅ 学者マニフェステーション追加+アクセッション潜在力修正、本番push済み(詳細→最上部0-2番)。実機確認待ち。
