...
- **🟢 Vercel デプロイ失敗ブロッカー=根本原因確定+修正実装済 (2026-06-08)**: 詳細は **[docs/.private/2026-06-08-vercel-deploy-blocker-handoff.md](./.private/2026-06-08-vercel-deploy-blocker-handoff.md)**。**確定した根因=Vercel Hobby の Serverless(Node) Functions 12個上限**(実測: 直近Ready=Node12個ぴったり / Error=room.ts追加で13個)。Edge4個(og/og-fetch/tweet-meta/tweet-video)は対象外。**修正=`api/collab/{load,save,room}.ts`を`index.ts`1関数に統合(housing/admin/template と同型・本体ロジック無変更)+`vercel.json` rewriteで旧URL維持→ワーカー無改修。Node 13→11個**。検証=api型チェックEXIT=0/collab30緑/フルビルド緑。**直近Ready=`a49f95b`(⑤-1は本番済)→今回のpushで⑤-2a初反映。src差分は型+テストのみ=ユーザー可視変更ゼロ・collab UI非露出継続**。次=push→デプロイR-eady確認→その後 ⑤-2b(満員拒否)→⑤-3(クライアントUI)。
...
- **段取り⑤(③の後)=共同編集の実入口 [設計書化済 2026-06-05・⑤を3分割]=[specs/2026-06-05-...stage5-collab-entry-design.md](./superpowers/specs/2026-06-05-realtime-collab-stage5-collab-entry-design.md)**: 核心=共有はコピーで別ID化([MitigationSheet.tsx:209](./../src/components/MitigationSheet.tsx#L209))→現状plan ID部屋鍵では繋がれない。解消=**`collabRooms/{roomToken}`対応表(token→planId)**。確定方針=常設リンク(案A・無期限)/**オーナーのみ失効・再発行・配布**/最大人数(既定8=フルパーティ)/緊急停止/**編集ログイン必須(段階導入・公開条件にサーバ認証含む)**/ジョイナーは一時ビュー(自分の一覧に増えない)/注意=初回モーダル+常時赤バナー/パスワード無し。コピー共有(ShareModal)無傷。**完成までUI一切非表示厳守**。順序: ③→⑤→②-b→②-c。
  ...
  - **次=⑤-2b**(満員拒否): `onBeforeConnect`(routePartykitRequest option)で接続前にDO在室数を照合し上限超過は upgrade 拒否。**調査確定=`onConnect`は101返却後に呼ばれ拒否不可**、DO外のfetch層で在室数往復が要る。**その後⑤-3**=クライアントUI(オーナーパネル/ジョイナー一時ビュー §8最難関/注意モーダル+赤バナー/ログインゲート)+実データ往復検証。
