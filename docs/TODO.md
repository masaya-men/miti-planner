...
2. **🟡 表示メンバー絞り込みスイッチ = 実装完了・ユーザー実機確認OK・未コミット(2026-08-19)**。詳細は「現在の状態」の未コミット3件まとめを参照。次=軽減表スプシモード(このスイッチを流用ではなく、同じ仕組みが両方で使えるだけ)。詳細=`docs/.private/2026-08-05-collab-header-and-spreadsheet-mode.md`。**共同編集参加者のヘッダー開閉解禁は保留**(このスイッチとは無関係な別項目)。
...
## 現在の状態 (次セッションはここから読む)
### 🟡 2026-08-19 未コミット3件まとめ(全てtsc/build/新規テスト通過・**まとめて本番反映予定**)
①**表示メンバー絞り込みスイッチ**: 実装完了・ユーザー実機確認OK。詳細=`docs/.private/2026-08-19-party-visibility-switch-handoff.md`。
②**ハウジンガー短縮共有URL**: `/housing/housinger/<64文字ハッシュ>`(93文字前後)→`/h/<名前>-<識別コード8桁>`。名前は飾りのみ・新データ保存無し・旧URL後方互換・Xカード表示も対応。Playwrightで「見つからない」経路のみ実機確認済み、**「見つかる」経路(実在ハウジンガー)の実機確認が残**。詳細=`docs/.private/2026-08-19-housinger-short-url-handoff.md`。
③**Allmarksまとめてインポート**: Allmarks(マイコラージュ)のタグ絞り込み共有リンク(最大100件・Allmarks側は無改修)を一時ツアーへ一括インポート。`EPHEMERAL_POOL_LIMIT`も50→100に引き上げ(コスト影響無し、一時listingはFirestore/Storage不使用)。**実際のAllmarks共有での動作は本番デプロイ後にしか確認できない**(ローカルdevはAPIを本番へproxyする仕組みのため、今回追加したサーバー機能はまだ本番に無い)。詳細=`docs/.private/2026-08-19-allmarks-import-handoff.md`。
