...
1. 🔴🔴 **ハウジング大規模耐性ハードニング + 住所非公開機能** (超大物・設計完成・承認済)
   ...
   - **▶ P3(住所非公開`unlisted`)=着手中(2026-07-15・ハウジングリリースの本丸)**: plan=`docs/.private/2026-07-14-housing-p3-private-listing-plan.md`(全8Task・TDD)。「画像は見せる・住所は守る」三重防御(窓口射影+rules get public限定+表示フォールバック)。visibility 2値→3値。**G6=3択UIデザイン承認(ユーザー・モックアップ外の新規設計)** / **G7=住所漏れ確認(devtools/他人視点で住所文字列ゼロ)**→ Discord淡々テンプレで告知。orchestration §3.5 確定1/2(詳細main窓口フォールバック・galleryAdapter unlisted緩和+MockListing optional)は P3 担当。
