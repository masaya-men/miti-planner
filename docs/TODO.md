...
## 現在の状態 (次セッションはここから読む)
### ✅ 2026-08-14 スマホのスクロール重さ = 大筋解決・本番反映済み、残るのは「初動だけの微かなカクつき」
①**ハウジング探すページ**: `mask-image`をスクロール本体からスクロールしない親へ移動して解消。ユーザー実機確認OK。
②**軽減表モバイル 連動エフェクト棒**: `@property --mobile-effect-bar-progress`(型付きプロパティ登録)で連動モードの重さが静止/常時比3.7倍→1.3倍に改善、さらにスクロール中は`transition:none`で指の位置に直結(離した後だけ400ms easingで戻る)に変更。**両方ともユーザー実機確認OK・本番反映済み**(経緯・不採用にした案の詳細は[TODO_COMPLETED.md](./TODO_COMPLETED.md)参照)。
**🟡 次セッション最優先: 「連動アニメの初動だけかくつく」大幅改善・ユーザー実機確認は軽くなったと確認済み**。詳細・調査経緯は`docs/.private/2026-08-14-mobile-scroll-stutter-investigation.md`。**根本原因は2つ特定**: ①`.timeline-scroll-container`に登録された5つの`scroll`関連リスナー(RecastRowのCD表示更新/フェーズ見出し/エフェクト棒/ヘッダー同期/スムーズホイール)の累積コスト(5つ全部を同時に軽量化して初めて重いフレームがほぼ消えることを実測で確認、1つずつでは効果ゼロ・未着手)。②`.mobile-row-dim-text`(行の文字濃淡)が`[data-mobile-scrolling="1"]`属性セレクタでON/OFF切替され表全体を横断する再評価が発生(ユーザー実機報告「常時表示の方が軽い」から発覚)。**②を本採用(常時濃い状態に固定)→実測で重いフレーム6件→2件まで激減・ユーザー実機確認済み「軽くなってる」**。これを受けて「連動」モードをFABピッカーへ復活し、エフェクト棒の伸縮変身アニメ(一時的にopacityのみへ簡略化していた)も元に戻して本番反映済み。①(5リスナー統合)はまだ未着手、残り2件の重いフレームが気になるようなら次回着手。
...
①既存統合版下書き=`docs/.private/2026-08-12-discord-full-update-draft.md`(投稿待ち、ハウジング13項目+共同編集3件+軽減/学者3件)。②2026-08-14分の新規下書き=`docs/.private/2026-08-14-discord-update-draft.md`(モバイル軽減表レイアウト刷新+競合表示+Discordログインキャンセル修正+探すページ見た目改善)。アイコンアップロード不具合の再発有無は、実際に不具合報告をしたユーザー自身が次にアップロードを試した際の反応を待つ方針(こちらから確認を仕掛けない)。修正自体(Storageルール)は完了済み。
