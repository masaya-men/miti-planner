...
- **最新セッション（2026-05-01・PiP（Floating Timeline）復活）**: 過去に「Chrome の Document Picture-in-Picture API は OS レベル透過不可」で UI 非表示にしていたカンペビューを復活。仕様変更 6 点: ①透過機能完全撤去（将来 Chrome 透過対応で再検討）②単一選択 → 多選（個別/全員/任意組合せをジョブピッカー多選で統合）③自ジョブ未設定時 全員フォールバック ④透過の代わりに背景カラーピッカー追加（テーマ別デフォルト dark=#0F0F10 / light=#FAFAFA + localStorage `pip-bg-color` 永続化、HTML5 native input[type=color] + LoPo 風小色丸ボタン 16px）⑤PC 起動ボタン disable 撤廃（自ジョブ未設定でも開ける）⑥モバイル FAB「カンペ」項目復活。設計書 `docs/superpowers/specs/2026-05-01-pip-revival-design.md`、実装プラン `docs/superpowers/plans/2026-05-01-pip-revival.md`、subagent-driven-development で 8 task + 最終レビュー進行。純粋関数 3 つ（`computeCueItems` / `computeInitialSelection` / `getDefaultBgColor`）を `src/utils/pipViewLogic.ts` に TDD 抽出（16 tests）し、PipView.tsx 本体改修。UI 復活ポイント 2 箇所（Timeline.tsx の `false &&` 撤去 + MobileFAB.tsx のコメントアウト解除）も完了。i18n 4 言語で 3 キー追加（pip_select_all / pip_deselect_all / pip_bg_color）/ 2 キー削除（pip_open_disabled / pip_opacity）。最終 328 tests PASS（既存 312 + 新規 16）、tsc clean、`npm run build` 成功。最終レビュー Important 1 件（select-all/deselect-all の cursor-pointer 欠落）fix 済み、Minor 3 件（aria-label vs title / img alt 欠落 / 浮動小数点比較リスク）は実害ゼロで本番実機確認で判断、Nice-to-have（PipView スモークテスト / mode prop 完全削除）は次セッション対応可。**「自分のプラン」バッジ機能（前セッションで実装した管理画面の ✓ バッジ）は個人特定回避方針で諦め決定** — 共有 API が ownerId を Firestore に書き込んでいないため動作不能、今後は公式ロゴ OGP で自分のプランを識別する運用に。
...
### 次にやること（優先順）
- **PiP（Floating Timeline）復活 実機確認（次セッション最優先）**: PC（Chrome）でタイムライン上部の PiP ボタン → 別ウィンドウ起動 → 多選 Popover の全員/解除/トグル動作 → 軽減アイコンが選択メンバー全員分マージ表示 → 背景カラーピッカー（小色丸ボタン）クリック → OS 標準ピッカー → 色変更で背景反映 → ウィンドウ閉じて再度開いて localStorage 色永続化 → ダーク/ライトテーマで初期色分岐（localStorage クリア後）→ 自ジョブ未設定で全員初期選択 → 軽減ゼロのプランで空状態メッセージ → メモ機能（攻撃名ダブルクリック編集・永続化）。スマホでも FAB → カンペ項目 → フルスクリーン起動 → 同様に多選/カラーピッカー/メモが動くか
