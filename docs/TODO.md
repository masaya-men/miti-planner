...
- **🔴 高 (データ破壊・2026-06-12 実害発生)**: **キャッシュ全消し直後、起動時に現在プランの中身を mitigation store へ読み込まない**ため画面が空のまま → その状態でプラン切替/操作すると Sidebar の「切替時に現在の画面を currentPlanId へ保存」([Sidebar.tsx:351](../src/components/Sidebar.tsx#L351)) と Layout 自動保存([Layout.tsx:253](../src/components/Layout.tsx#L253)) が**空スナップショットを各プランへ保存→Firestore まで上書き**(version++/updatedAt=now で timestamp ガードも貫通)。Cloudflare 検証中にユーザーがキャッシュ全消し→「固定」が空(軽減0/ジョブ空/lvl80)で上書きされる実害。**復旧済**(削除済み墓標 06-10 の 129 軽減を `scripts/restore-fixed-plan.ts` で live doc に復元・他プラン無傷をスキャン確認)。バックアップ=`docs/.private/plan-backup-*.json`(gitignore)。**根治案=①起動時に currentPlanId の data を loadSnapshot ②空(全ジョブ空/中身ゼロ)スナップショットで非空プランを上書きしないガード ③PITR 無効なので有効化検討**。調査スクリプト=`scripts/inspect-plan-contents.ts`・`probe-fixed-plan-history.ts`・`backup-user-plans.ts`。
