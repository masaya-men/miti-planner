...
- **🟡 2026-06-17 共同編集中の Undo/Redo (CRDT化・②-c) 実装完了+3回の敵対監査でデータ安全確認・branch `feat/collab-undo-redo`・未push/未マージ・🔴実機2タブ検証待ち**: 「共同編集中は Undo/Redo が効かない(オーナーも戻せない)」を `Y.UndoManager`(per-user undo)で解消。`trackedOrigins:['local']` で**自分の編集だけ**を戻す(リモートは origin=provider オブジェクトで構造上捕捉不可)。scope=solo と同じ5型。store の undo/redo は collab 中 handlers 経由で UndoManager へ委譲(set 外)、ボタン活性は `_collabCanUndo/Redo` フラグで連動。新規=`src/lib/collab/planUndoManager.ts`+`collabUndoIntegration.test.ts`、改修=collabProvider/collabTypes/useMitigationStore/Timeline。設計=specs/2026-06-17-collab-undo-redo-design.md / 計画=plans/同名。
  - **🛡 MAX effort/Ultracode で 3 回の多エージェント敵対監査を実施しデータ消失を根絶**: ①reseed が undo スタックに乗る Critical(planUndo.clear で根治) ②undo/redo が set() 内 handler 呼びで store↔Y.Doc 恒久 desync する Critical(set 外委譲へ・71c6797) ③exit/enterCollabMode が _history 残留→revoke/disconnect 後 Ctrl+Z で入室前データへ巻き戻り Firestore 恒久上書きする Critical(enter/exit で履歴 clear・71c6797) ④編集者ジョイナー undo 不能 Important(ガードに `&& !_collabActive`) ⑤テストギャップ Important(実 Y.Doc 統合回帰テスト追加) ⑥**既存バグ** confirmAetherflow/AstrologianDrawChain が collab 委譲を持たず次の observeDeep で消えるデータ消失(upsertItems 委譲で根治・09efc66) ⑦テスト flaky(afterEach 追加・e9fa1ac)。最終検証=dataSafe=TRUE/新規 Critical・Important ゼロ。全体 1803 pass(既知 housing5 のみ fail)/build EXIT0。
  - **残(非ブロッキング・既存条件)**: FFLogs importBulk 再 seed 時の Firestore 射影食い違い(Minor・データ消失ではない・コードが安全側に倒す既知トレードオフ) / 連鎖 confirm の実 Y.Doc 往復テストは未追加(機構は検証済 add 経路と同一)。
  - **🔴 残=ユーザー実機2タブ検証**→OK なら main マージ&デプロイ。**チェックリスト**(捨てプランで・本物データに触れない): (a)自分の配置を Ctrl+Z で自分のだけ消える/Ctrl+Y で戻る (b)**タブAで配置→タブBで Ctrl+Z しても A の配置は消えない**(per-user の核心) (c)タブAの Undo がタブBにライブ反映 (d)履歴無いとき Undo/Redo ボタンがグレーアウト (e)閲覧専用ジョイナーは Undo/Redo 無効/編集権ありジョイナーは自分の操作を戻せる (f)フェーズ/ラベル/ジョブ変更/イベントも Undo で戻る (g)**入室前に solo 編集→入室→編集→turn-off→Ctrl+Z で collab 編集が消えない**(Critical②修正) (h)SCH/AST で連鎖確定後リロード/相手画面で連鎖軽減が残る(既存バグ⑥修正) (i)検証後テストプラン削除。
