...
ハウジング投稿(登録ページ/一時ツアークイック追加パネル、編集ページ含む複数箇所)にYouTube動画のURLを貼ると、現状は動画IDからサムネイルURLを組み立てて物件画像として使うだけで、概要欄のテキストは一切取得していない。X(Twitter)・OGP対応サイト(housingsnap等)は既に投稿本文/ページテキストから住所(DC/サーバー/エリア/区画/サイズ)を自動抽出しているが、YouTubeだけこの恩恵がない。
...
| 論点 | 決定 |
|---|---|
| 対象範囲 | **住所自動入力(概要欄取得+抽出+失敗表示)は登録ページ(`RegisterPage`のcreateモード)と一時ツアークイック追加パネル(`EphemeralAddPanel`)の2箇所のみ**。編集ページは対象外(下記「編集ページの扱い」参照、2026-08-17ブレスト中に方針転換) |
| 編集ページの扱い | 調査の結果、編集ページ(`RegisterPage` mode='edit')はSNS URLを貼り直しても**現状Twitter/OGPも含め住所には一切触れておらず**(`HousingEditSourcePanel`/`commitEditSnsFetch`は画像・動画フィールドのみを扱う)、住所自動入力は編集ページには元々存在しない機能だった。これを新設するのは「編集時に既に確認済みの住所を、貼り直した動画の概要欄で意図せず上書きしてしまう」リスクがあり危険なため、**編集ページは今回も対象外のまま据え置く**(ユーザー確定)。ただし`HousingRegisterSnsUrlField`(YouTube欄の共通部品)を変更するため、編集ページのYouTube欄も内部的には概要欄を取得するようになるが、`HousingEditSourcePanel`側がその値を無視するため実害はない(サムネイル添付のタイミングがTwitter/OGPと同じ「取得完了を待ってから表示」に揃う程度の副次的な変化のみ) |
...
| 登録ページの失敗UI(新規追加) | 現状はTwitter/OGPも含め完全に無言(住所欄が空のまま何も表示されない)。今回、一時追加パネルの既存文言と同等の案内を登録ページ(createモード)に追加する。**対象はYouTubeだけでなくTwitter/OGPも含む全SNSソース**(ユーザー判断で範囲拡大)。編集ページは上記の通り対象外 |
| 登録ページでの表示条件 | 既存の`applyExtractedResult`が`fills.length === 0`(=1項目も自動入力できなかった)の場合にのみ表示する。既存の「`ambiguity`があっても area/ward/plot/size 等の一部項目は入る」という現状の寛容な挙動自体は変更しない(下記「非対象」参照) |
...
- `src/components/housing/pages/RegisterPage.tsx` — `applyExtractedResult`(623-686行、`fills.length===0`で無言return)と`handleYoutubeFetched`(788-810行、現状descriptionを扱っていない)。descriptionを`applyExtractedAddress`に渡す変更+`fills.length===0`時の失敗フラグ管理を追加する。**この経路は`mode='create'`でのみ使われる**(下記の通り編集モードは別経路)。
- `src/components/housing/edit/HousingEditSourcePanel.tsx` / `commitEditSnsFetch`(`RegisterPage.tsx:1305`) — 調査の結果、**住所ロジックを一切持たず画像/動画フィールドのみを扱う独立した経路**と判明(`RegisterPage`のcreateモード用ハンドラとは別物)。今回は**変更不要**(上記「編集ページの扱い」参照)。
...
### 5. 変更なし: `HousingEditSourcePanel.tsx` / `commitEditSnsFetch`
今回の対象外(上記「編集ページの扱い」参照)。`HousingRegisterSnsUrlField`の型変更(`description`フィールド追加)は伝搬するが、このファイル側では単に無視すればよく、ロジック変更は不要。
...
- 登録ページ用の「読み取れませんでした」文言(新規キー、`housing.ephemeral.parse_error`と同内容を`housing.register.snsUrl`配下に追加)。
...
- 登録ページの「`ambiguity`があっても area/ward/plot/size 等の一部項目は入る」という既存の寛容な挙動を、一時追加パネル水準(`ambiguity`があれば全項目拒否)まで厳格化することは今回はやらない。既存のTwitter/OGP経由の動作を変える意味を持つため、影響範囲が広く別スコープとする。
- 編集ページへの住所自動入力の新設(上記「編集ページの扱い」参照)。
...
- `RegisterPage`(createモード): YouTube URL貼付→概要欄取得成功→住所自動入力、のケースをmock fetchで追加。`fills.length===0`時に失敗表示が出るケース、Twitter/OGPでも同様に失敗表示が出るケースを追加。
- `RegisterPage`(editモード)/`HousingEditSourcePanel`: 既存テストが回帰しないことのみ確認(住所ロジックへの変更は無いため新規テスト不要)。
