...
0. **🔴 次セッション最優先(2026-08-26 ユーザー確認済み・4件)**: 軽減表周りのブラッシュアップ。
   - 0-1. 🆕(要brainstorming) PC: Timelineのエフェクト棒にホバーで、LoPo流の綺麗なツールチップでスキルアイコン表示。「どのエフェクト棒がどのスキルか把握しづらい」との声。現状のホバー挙動を要確認。
   - 0-2. ✅(2026-08-26修正済・要実機確認) バリア計算バグ、2件。共有リンクの実データで再現・修正後の数値も検算済み。
     - ①展開戦術(バリアをコピーする技)のコピー計算(`calculator.ts` calculateLinkedShieldValue): 「ランパート等の自己バフ(scope:self)は本人にしか効果がない」というチェックが丸ごと抜けていた。タイムラインは誰が使ったか(ownerId)が既にデータにあるため、UI変更なしでロジックのみ修正。
     - ②「最大HPの◯%」固定タイプのバリア6つ(ディヴァインヴェール/ブラックナイト/アルケインクレスト/シェイクオフ/テンペラグラッサ/インプロビゼーション)に、回復効果アップ・確定クリティカルが誤って乗ってしまう不具合(`Timeline.tsx`)。回復魔力(ポテンシー)から算出するバリアだけが対象になるよう修正、`scholarShieldRules.ts`に`isPotencyBasedShield`として切り出し。全28バリア技を実データで網羅検証(回帰テスト40件)。8.0新規バリア追加時もvalueType登録だけで自動的に正しく判定される設計。
     - 検討したが見送り: フェイイルミネーション(パーティ全体に配られる自己バフ、位置情報を持たないアプリの既存の割り切りと整合済み・現状維持が正)/EventForm.tsx(イベント編集画面の手動チェックリスト。技ごとの使用者データを持たないため直すにはUI変更が要り、実害小さいため見送り)。
     - tsc -b・既存テスト全てgreen。**ユーザーの実機再確認待ち**。
   - 0-3. 🐛 スマホ軽減表: メンバー表示切替のタップがドラッグ&ドロップと競合してオンオフしづらい。
   - 0-4. 🐛 スマホ: メモは非表示にすべき(2026-08-14実機報告①と同一事象の可能性、あわせて対応)。
