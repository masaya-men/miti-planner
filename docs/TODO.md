...
## 現在の状態 (次セッションはここから読む)
### 🔴 次セッション最優先: モバイル軽減表 ①は実機確認済み・②に未解決バグ1件あり push保留中
①レイアウト刷新は実機iPhoneで確認完了。②競合(CDかぶり)表示も大部分は実機確認済みだが、**未解決バグが1件残っておりpush保留中**(詳細・解決済みの派生バグ4件は[TODO_COMPLETED.md](./TODO_COMPLETED.md)の2026-08-14エントリ参照)。

**🐛 未解決: スクロール連動(連動)モードで競合中のアイコンだけ画面右にはみ出て表示される**(2026-08-14ユーザー実機報告・未解決)。ユーザーの言葉: 「スクロール連動のときに競合表示が画面右になぜか漏れ出て表示されています」「競合しているスキルだけが漏れ出て表示されてる感じです」(=通常のアイコンではなく、黄色パルス中のアイコンだけがこの症状)。
調査で`.mobile-effect-bar-icon`の中央寄せ(`transform: translateX(-50%) scale(...)`)がPlaywright実測(WebKitヘッドレス)で本当に中央寄せされておらず画面右にはみ出す不具合を発見・`margin-left:-11px`方式に分離して修正・コミット済み(`4fcaaa67`)。**しかしユーザー実機での再テストでは「何も解決してない」と否定された**。**重要な手がかり**: ユーザーは「競合しているスキルだけ」と明言しており、通常アイコンは正常に見えている可能性が高い。つまり①このPlaywright実測で見つけた中央寄せバグは実在するが②ユーザーが実機で見ている不具合の真因ではない、または③conflictの`ring-1 ring-amber-400`スタイル固有の何かが影響している可能性がある。次セッションでは「本当に競合中のアイコンだけが漏れるのか、通常アイコンは漏れないのか」の切り分けから再スタートすること。Playwrightの合成再現(JSで`--mobile-effect-bar-progress`を直接書き換える手法)は今回実機と食い違ったため、可能なら実機スクリーンショット/動画を先に集めるか、実際のタッチスクロールイベントに近い形で再現すること。
**次セッションの最初にやること**: 上記バグの根本原因を特定→修正→実機確認 → push前ゲート(build+全テスト)→push→デプロイ→本番実機で①②一式の最終確認。
