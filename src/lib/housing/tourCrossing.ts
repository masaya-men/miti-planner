...
export type TourCrossing =
  ...
  | { kind: 'world'; world: string; dc: string; area: string; ward: number | null } // ワールド訪問(同DC・別ワールド)。dc は表示側の displayWorldName(dc, world) 用。area/ward = 着地先(次の家)のハウジングエリア区画名+区番号(未確定時は空文字/null)
  | { kind: 'dc'; dc: string; world: string; area: string; ward: number | null }   // DCトラベル(別DC・同リージョン)。着地ワールドも持つ。area/ward は world と同様
  | { kind: 'region' };                          // 別リージョン(通常はブロックで来ない・防御表示)

type Loc = Pick<MockListing, 'region' | 'dc' | 'server' | 'area' | 'ward'>;
...
export function crossingBetween(prev: Loc | null, current: Loc): TourCrossing {
  ...
  if (prev.region !== current.region) {
    ...
    if (prevGroup === 'GLOBAL' && currentGroup === 'GLOBAL' && (prev.region === 'OCE' || current.region === 'OCE')) {
      return { kind: 'dc', dc: current.dc ?? '', world: current.server ?? '', area: current.area ?? '', ward: current.ward ?? null };
    }
    return { kind: 'region' };
  }
  if (prev.dc !== current.dc) return { kind: 'dc', dc: current.dc ?? '', world: current.server ?? '', area: current.area ?? '', ward: current.ward ?? null };
  if (prev.server !== current.server) return { kind: 'world', world: current.server ?? '', dc: current.dc ?? '', area: current.area ?? '', ward: current.ward ?? null };
  return { kind: 'none' };
}
