...
## 現在の状態 (次セッションはここから読む)
### 🔴 次セッション最優先: モバイル軽減表レイアウト刷新 ①②とも実機iPhone確認完了・push待ち
...
**実機FBで別バグ発覚→根治(2026-08-14)**: 「ランパートだけCD赤表示にならない(ディヴァインヴェールは正常)」というユーザー報告から調査。**根本原因**: ランパート等の一部技はレベル帯で内部idが分かれる(例: Lv93以下`rampart_pld`/Lv94以上`rampart_v2_pld`)。過去の別セッションで「同id版違いバグ」(idが完全重複して低Lv版に解決される不具合)を`_base`/`_v2`別id化で修正した際、`getSharedCooldownIds`(競合CD判定の共有グループ関数)がid単純一致のままだったため、**旧idで置いた軽減が現在のレベルの新idと同じ技として認識されずCD競合判定から漏れる**副作用が残っていた(ディヴァインヴェールは単一idのため無事だった)。今回のモバイル競合表示追加でこの穴が初めて可視化された(PC版は元々目立ちにくかった可能性)。対象はランパート/リプライザル/アドル/牽制/オーロラ/インターベンション/秘策/展開戦術/タクティシャン/トルバドゥール/ピュシス/ゾーエ/コンシテーション/ユークレイシア・プログノシス/アスペクテッド・ヘリオス/アサイラム/テトラグラマトン/守りのサンバ(計18技+原初の血気ライン特例)、全てのjobで発生しうる規模。**修正**: `getSharedCooldownIds`を「同一jobId・同一icon=見た目も効果も同じ技」で自動グルーピングする方式に変更(ハードコード列挙不要・将来追加される同種技にも自動対応)。原初の血気(bloodwhetting)⇄原初の猛り(nascent_flash)ラインは別名・別アイコンでも同一CDを共有する特例のため、`raw_intuition`/`nascent_flash_base`も含めて4id全てを明示グループ化(旧実装は2idのみでこれも同じ穴があった)。回帰テスト4件追加(`versionSkewConflicts.test.ts`)、既存resourceTracker系vitest計50件確認済み・tsc確認済み。Playwright実機相当で「旧id配置→現idで確認」の完全再現→修正前は赤くならない/修正後は正しく赤+パルスを確認済み。
**実機確認で②の3点を追加修正(2026-08-14)**: ①画面外シェブロンがモバイルのフェーズ名バーに被る→`ConflictOffscreenArrows`に`topInset`prop追加、モバイル+フェーズありなら`MOBILE_TOKENS.header.phaseBarHeight`分だけ上矢印を下にずらすよう`Timeline.tsx`から指定。②エフェクト常時/連動表示モードで競合パルスが出ない→専用行アイコン(`.mobile-miti-icons`)はこのモードだとopacity:0で隠れ、代わりに`MobileEffectBarLayer.tsx`の棒アイコンが表示されているのにそちらへパルス配線を忘れていたのが原因、`conflictingIds`propを追加して同じ`animate-conflict-pulse ring-1 ring-amber-400`を適用。③軽減追加シートのバツボタンが押しにくい→`MobileBottomSheet.tsx`の×ボタンに`-inset-2`の透明な当たり判定拡張レイヤーを追加(見た目は変えず判定だけ8px拡張)。3件ともPlaywright実機相当で確認済み・tsc確認済み。
**「置いた直後は片方しか光らない」問い合わせ→仕様確認で解決(2026-08-14)**: PC版も同じ挙動と実測確認(既存パルス除外ロジックは「直前配置分だけ除外・リロードで両方光る」がPC側の元々の仕様、モバイルは完全に同じロジックを共有)。ユーザーがプランをリロードして両方光ることを実機確認済み、コード変更なし。
**下矢印がボトムナビに埋もれる不具合を追加修正(2026-08-14)**: 実測の結果、矢印のsticky土台がスクロールコンテナのpadding-top(ヘッダー分52px)だけ画面上端からずれた位置を基準にしていることが判明(上矢印はこのズレとフェーズバー位置がたまたま一致し無問題だったが、下矢印は「画面下端からの距離」計算にこのズレが含まれておらず約52px分ナビに埋もれていた)。`bottomInset`にボトムナビ高さ分に加えてこのpadding分も加算するよう修正、Playwright実機相当で6px余裕を持って非重なりを確認済み。
**次セッションの最初にやること**: push前ゲート(build+全テスト)→push→デプロイ→本番実機で①②一式の最終確認。
