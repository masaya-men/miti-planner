...
- **最新セッション（2026-04-29・プラン切替リセット）**: ユーザー報告「別プランを開いても横スクロール位置と空行非表示トグルが引き継がれる」を修正。`Timeline.tsx` のプラン切替 `useEffect` が `scrollTo({ top: 0 })` で縦のみリセットしていた点を `top: 0, left: 0` に拡張、加えて `setHideEmptyRows(true)` を呼びコンパクト表示に戻す。フェーズ列・ラベル列の折りたたみはユーザー操作の状態を維持する判断で意図的に触らない。build+test 119 PASS、デプロイ済み、実機確認は次セッション。
...
### プラン切替時のスクロール/空行非表示リセット 2026-04-29 — 実機確認待ち
- [ ] プラン A で右にスクロール → プラン B を開く → 横スクロールが左端に戻る
- [ ] プラン A で空行非表示をオフ（全行展開） → プラン B を開く → コンパクト表示に戻る
- [ ] プラン A でフェーズ列を折りたたむ → プラン B を開く → 折りたたみ状態が維持される（仕様）
- [ ] プラン A でラベル列を折りたたむ → プラン B を開く → 折りたたみ状態が維持される（仕様）
