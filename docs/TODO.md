...
- 改善(ユーザー提案・実装済み・未push): DC/ワールド跨ぎの案内文言を実操作+着地区画名入りに強化(①DCトラベルはログアウトが要ると判明し明記②着地先のハウジングエリア区画名+区番号を追加③中央カードと右パネルの重複文言を右パネル側撤去で解消④カード枠をハニーゴールドで視線誘導点滅+DC/ワールド/区画名を強調表示⑤左上フル住所を2段化し最寄りエーテライト表示+拡張街バッジ追加)。詳細な調査根拠・文言before/after・実装ファイル一覧 → `docs/.private/2026-08-10-tour-crossing-wording-revision.md`。スマホもPCと同一コンポーネントのため反映済み(実機見た目確認はユーザー側で予定)。
- 経路データ修正(コミット済み): エンピレアム21号地(本家・拡張街=51号地)の自動計算ナビが最短距離でなかった件、`/housing/dev/routes`(経路お絵かきツール)でユーザーが手直し・保存。[[project_housing_tour_route_fix_policy]]どおり個別override方式。同様の報告が来たら同じ手順(devサーバー起動→ツールで直してもらう→保存後にgit diffで反映件数を確認→コミット)で対応する。
- バグ修正(コミット済み): ハウジンガーページ「まとめてツアー」が探す/お気に入りで組んでいたツアートレイを無視して上書きしていた不具合を修正。トレイ+この人の家をマージしてから開始する形に統一(BrowsePage.commitStartと同型)。同じ理屈のTourNavPage空状態パネルも合わせて修正。全開始経路(5箇所)を洗い出し、他に同種バグが無いことを確認済み。
