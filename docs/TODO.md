...
## 現在の状態 (次セッションはここから読む)
### 🔴 次セッション最優先0: ハウジング全タスクの棚卸し(2026-07-23ユーザー要望)
「ハウジングアプリでやるべきタスクを全部一度整理したい」。TODO.md本ファイル+TODO_COMPLETED.mdの残作業+`docs/.private/`の未消化議論を横断して一覧化する回。同時に出た改善アイデア3件も検討対象に含める:
①**スマホ・PCともに表示までの読み込み速度改善は可能か**(体感速度の改善余地を調査)
②**コスト面の再チェック**(既存の「最優先1.5: Twitter動画コスト0化」l.31と合流して検討)
③**タグでの検索機能を追加すべきか(AND検索?)**(現状フィルターはtheme kindのみ絞り込み対象・公式/季節/個人タグは未対応)
...
### ✅ 実機フィードバック3件(2026-07-21・ブレスト完了) = 全項目 実装・main反映済み(2026-07-22)
1+3. 探すページの表示順ランダム化+一覧スクロール位置復元。2. 初心者向けタグ「ハウジング若葉」。詳細=TODO_COMPLETED.md。**残(次回ユーザー実機確認)**=下記チェックリスト参照。
...
- **タグ仕様全面刷新**: **Phase A+B+C 全て本番稼働済み**(公式23+季節12+テーマ12+初心者1+個人タグ)。計画書=`docs/superpowers/plans/2026-07-10-housing-tag-overhaul-plan.md`。
