...
- **🟢 取込フロー v2(本体6/23本番済→[TODO_COMPLETED.md](./TODO_COMPLETED.md))=残作業のみ**: ③スプシ取込の攻撃に対象(MT/ST)が入らない→テンプレ持ち越し=**未着手・brainstormingから**(マッチング曖昧性あり・詳細`docs/.private/2026-06-23-import-flow-v2-brushup.md`③)。②プレビュー画像=**検証完了/本番OK確定**(対応不要)。②途中取込=spec§7繰越・別タスク。ウィザードfollow-up(任意・別タスク)=step4で`no_phases`時の理由非表示(旧来同挙動)/skipped表示のamber直値→トークン化。
