...
- **ブランチ**: main 直接
- **最新本番デプロイ**: セッション 18 末 (リキャスト専用行 ツールバー統合版完成)
...
1. **【復活方針】 効果中スキル最上行残し** — 一度諦めたが復活。 リキャスト行と相互影響なし (リキャスト行は独立行として実装済) → 単独で brainstorming 可
2. **【バグ】 互い違い配置時のチラつき** (セッション 17 で発見、 本番では再現せず保留) — 同時刻配置で一瞬完全に重なってから横にずれる。 配置前に重なり計算して最初から正位置に置きたい。 Timeline.tsx:2700-2810 のアイコン配置ロジックが対象。 ローカル dev で再現するか要確認
