...
## 現在の状態 (次セッションはここから読む)
### 🔴 次セッション最優先 (2026-07-20 更新)
00. ✅ **コスト・ハードニング=コード6タスク+Cloudflare運用2件+housing公開窓口のmax-age欠落バグ(実機FB④根本原因)、全部本番反映・実証済(2026-07-20)**。詳細=TODO_COMPLETED。
0-1. ✅ **実機FB9件のうち②③⑥⑦⑧を修正・build/test確認済(2026-07-20・未push)**。⑦⑥⑧はユーザー実機確認希望。**残る2件**=⑤YouTube概要欄住所自動入力は要ユーザー判断(公式API導入が必要)/⑨アカウントモーダル位置ずれは原因未特定(実機のcomputed style待ち)。詳細=TODO_COMPLETED。
...
- **実機FB9件**: ①④は対応不要/済。②③⑥⑦⑧は2026-07-20に修正済。残る⑤⑨は上記「現在の状態」参照(詳細=TODO_COMPLETED)。
