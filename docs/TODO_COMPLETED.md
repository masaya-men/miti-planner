...
5. **スクロール連動モードで競合アイコンが画面右にはみ出す**: `.mobile-effect-bar-icon`の中央寄せを`transform:translateX(-50%) scale(...)`の1行で書いていたところ、実機で translateX の%指定が効かず(0扱い)アイコンが中央寄せされずスロット位置によっては画面外にはみ出す不具合が発覚。中央寄せを`margin-left:-11px`(固定px)に分離しscaleだけtransformに残すことで解消(位置決めと拡縮の責務分離)。
...
**次セッション**: push前ゲート(build+全テスト)→push→デプロイ→本番実機で①②一式の最終確認 → 完了後は③競合(CDかぶり)表示の後続として計画していた項目は無し、次は別タスクへ。
