...
- **ブランチ**: main 直接
- **最新本番デプロイ**: セッション 17 末 (表エリア全幅化完了: T/H 151px、 マージン 2.9px、 縦スクロールバー非表示)
...
1. **【新機能】 リキャスト専用行** — フェーズ│ラベル│時間 の下にリキャスト表示行 (折り畳み可、 デフォルト表示)、 ゲーム内本格表示、 パフォーマンス影響なし (過去議論済)。 T/H 6 アイコン対称化により並べやすくなる副次効果あり
2. **【復活方針】 効果中スキル最上行残し** — 一度諦めたが復活、 上記 1 と相互影響
3. **【バグ】 互い違い配置時のチラつき** (セッション 17 で発見) — 同時刻配置で一瞬完全に重なってから横にずれる。 配置前に重なり計算して最初から正位置に置きたい。 Timeline.tsx:2700-2810 のアイコン配置ロジックが対象
...
- **中**: ラベル名が管理画面で取得できない (スプシヘッダー問題?) (※ 互い違いチラつきは「次セッション最優先 3」 に昇格)
