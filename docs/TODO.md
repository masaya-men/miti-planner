...
- **🟢 現在の作業スレッド=リアルタイム共同編集 (branch `feat/collab-stage4b2-live-cursors`・held・push/deploy保留・UI非露出)**: **④-b-2(P2P live カーソル)実装完了 (2026-06-10)**。spike確定=signaling は既存WS awareness相乗り(新DO/依存ゼロ)+自前最小WebRTCメッシュ。既定OFFオプトイン+ON時IP正直説明+OFFで接続クローズ。カーソルはタイムライン上のみ(座標=既存Memo流用)。全15タスクTDD・root1572緑(既知5失敗のみ)/build緑/yjs遅延境界維持。**次=⑤-3d+④ 統合プレビュー実機検証 (login要・localhost auth不可・2ブラウザ)**。詳細は下記 collab バックログ + memory `project_realtime_collab_status`。
