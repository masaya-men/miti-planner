...
**①-a 2行構成レイアウト化 = 実装済・コミット済(未push)**: `MobileTimelineRow.tsx`で行を「時間+攻撃名+ダメージ」+「軽減アイコン専用行(メンバーごとグルーピング)」に分割、`Timeline.tsx`にヘッダー直下スクラブ式フェーズ名バー追加。詳細はgit log参照。**未決**: 軽減アイコンサイズ18px⇄22pxまだ最終決定していない。

**①-b エフェクト棒(スクロール中だけ出る演出) = 実装済・実機調整中で一時停止**(2026-08-12ブレスト→設計書→計画書→SDD実装、詳細=`docs/superpowers/plans/2026-08-12-mobile-effect-bar-toggle.md`・進捗=`.superpowers/sdd/2026-08-12-mobile-effect-bar-toggle/progress.md`): スクロール中だけ軽減アイコン専用行が消え、PC版と同じ「アイコン+下に伸びる色の棒」が現れる演出(優先度順に詰めて右寄せ・入りきらない分はDPSから非表示・行の文字より奥に描画・スクロール中は薄く)。実機FB4件反映済(アイコン潰れ/重なり順/薄さ/指保持中のガタつき)。**視差風の遅延追従は実装→実機確認→ユーザーの好みに合わず不採用で削除済**。**次回検討事項**: 止まっている時のアイコン表示も右寄せ・グループ分けなしに統一するか(ユーザーは統一希望だが、単純な右寄せでは行ごとに位置がズレるため、本当にやるなら`MitiIcons`を廃止して`computeMobileEffectBars`と同じ土台に一本化する必要あり。詳細は上記progress.md)。
