...
1. **🔴 ハウジング次の着手順(2026-07-24決定・24日再整理)**: ①コスト面 = ✅**完了**(Twitter動画0化は既に本番稼働済みと判明・画像Storage egress対策は設計書`docs/superpowers/specs/2026-07-24-housing-image-cloudflare-cache-design.md`通り実装+本番デプロイ+既存79件231枚の移行まで完了。Cloudflareキャッシュ動作確認済み。**数日後にFirebase使用状況で効果測定予定**) ②**マイページ作成 = ✅実装完了・push待ち→デプロイ後ユーザー実機確認待ち**(詳細は下記) ③タグAND検索機能(要brainstorming) ④繁体字対応(要brainstorming) ⑤YouTube概要欄住所自動入力(API自体は無料・1日1万クエリ枠で問題なしと判明済み) ⑥30日物理削除cron実装(housing_listingsに家主削除済みだが未物理削除のドキュメントが38件残存確認・実害小だが公開前に直したい) ⑦運営からのアップデート告知をハウジング側にも(既存の通知ベルマーク活用・要brainstorming)。全項目の詳細=`docs/.private/2026-07-23-housing-task-inventory.md`。
2. **軽減編集タイムラプスのSNS投稿**(大物・要brainstorming)

## 現在の状態 (次セッションはここから読む)
### 🔴 ②マイページ作成 = 実装完了・push直後・ユーザー実機確認待ち(2026-07-24)
brainstormingから実装まで完了。自分のハウジンガーページ(`/housing/housinger/自分のuid`)を拡張し、`/housing/mypage`もこれに接続(自分のuidにフォールバック、未ログイン時はログイン案内)。①左右2カラムレイアウト刷新(プロフィールsticky固定・一覧5列維持・自己紹介欄168px固定高で内部スクロール) ②自分の全物件(公開/住所非公開/完全非公開)を一覧表示するよう変更(旧: 公開分のみ) ③各カードにワンクリック公開状態切替+確認モーダル(新API `update-visibility`) ④設定ウィンドウをシンプル化(アイコン/名前/管理画面/ログアウト/退会のみ)、自己紹介文・SNSリンク編集はマイページへ移動 ⑤表示名編集欄に「ハウジンガータグとして使われます」説明追加 ⑥スマホ下部ナビのアカウントボタンが「マイページ/アカウント設定」2択メニューに ⑦公開期限切れ後の挙動をユーザー選択制に(登録・編集フォームに「期限が来たら住所非公開/完全非公開」の選択肢追加、裏側cron`apply-expired-visibility`が日次で実際に切替、Firestore複合インデックス追加済み)。ビルド・型チェック・関連テスト全通過。**次セッション最優先=デプロイ後の実機確認**(チェックリストはユーザーに送付済み)。
...
00. ✅ **コスト・ハードニング全部+実機FB9件中②③⑥⑦⑧⑨+ボタン高さ統一/スマホ下端余白/お気に入りアニメ強化、全部本番反映済み・ユーザー実機OK(2026-07-20)**。詳細=TODO_COMPLETED。**残る1件**=⑤YouTube概要欄住所自動入力(2026-07-24: 実装する方針決定・上記「次の作業順」⑥として着手待ち)。
