...
## 現在の状態 (次セッションはここから読む)
### ✅ 2026-07-31: タグ検索〜繁体字〜OGPカード作り込み+YouTube修正+WebP画質根治+スマホ一覧見出し修正、全て本番push・デプロイ済(`f81a7049`)
3大バッチ(①タグAND検索 ②繁体字(台湾)対応 ③ハウジンガーOGPカード作り込み)+個別不具合8件(`cd2d95f3`)に続き、同日中に3件追加で本番反映:
- YouTube動画サムネ404の修正(`abde9fd5`)。
- **OGPカードのWebP画像品質・恒久対策**(`9c1ee98b`): アバター・物件写真ともWebP+PNG併存保存化(物件写真は拡張子違いの兄弟ファイル方式、Firestoreスキーマ変更なし)。既存データのバックフィルも本番実行済み(アバター148件/物件写真197枚、失敗0件)。詳細=`docs/.private/2026-07-31-ogp-webp-png-backfill.md`。
- **スマホのマイページ一覧見出しが縦潰れする不具合の修正**(`f81a7049`): 当初「ひとこと説明文」の不具合と誤認していたが、実機スクショで判明した真因は`ListingGrid`のツールバー(見出し+並び替え)がflexboxで幅を奪い合い、日本語テキストにmin-widthの下限が無いため1文字ずつ縦積みに潰れていた。
build+vitest OK、既知の無関係failure=EphemeralAddPanel 7件のみ。Firestore直書き換えを伴うため、反映にはCloudflare「Purge Everything」実行が必要(ユーザー実行済み)。

### 🔴 次セッション最優先(2026-07-31 ユーザー指摘・月曜以降着手)
**OGPカードのデザイン自体がまだ不十分**: WebP不具合は直り、Discordのリンクプレビュー実機確認でアイコン・物件写真は表示されるようになったが、以下は未解決:
- **背景のぼかし画像がそもそも使われていない**: 当初設計(`docs/superpowers/specs/2026-07-31-housinger-ogp-card-redesign.md`)は「代表作の先頭1件を背景ぼかしに使う」想定だったが、実装では背景に画像が一切出ておらず単色パネルのまま。
- **背景に使う画像を能動的に選べない**: 「先頭にチェックした1件が背景」という仕様だと、ユーザーが意図して背景用の画像を選べない。UI含めて設計し直す。
- **画像グリッドの並べ方が雑然としている**: 画像サイズがバラバラ(小さい正方形+大きい縦長パネル混在)で統一感がない。並べ方の方針を決める。
月曜以降、brainstormingから着手。
