...
- **🔴 探す地図フィードバック: ①③④✅ / ②機能=ユーザー実機OK・パネル角=解決(実機OK)→リリース待ち** (2026-07-12)。詳細=`docs/.private/2026-07-12-browse-map-feedback.md`。**左上の四角い出っ張りの真因=パネル裏の区画ピルバー(明るい選択ピル)が半透明パネル越しに透けていた**もの(前セッションの「backdrop-filter 角丸クリップ不良」は誤診)。解決=**ぼかし復活+パネル角丸を裏ピルバーと同一半径に**(共有トークン `--housing-mapctl-areas-radius:9px` を `.housing-mapctl-areas` と `.housing-bmap-roompanel` 両方が参照)。build✅・ユーザー実機OK。この後は big3 全体のリリース前検証へ(下記 2 行)。
  ...
  - **残(リリース前)**: **②ヘッダー「ツアーを組む」タブ削除**(`/housing/plan`=今ComingSoon空・探すから組めるので不要・独立/すぐ) / **④ツアーのリージョン・DC・ワールド跨ぎ処理(リリース必須・未実装)**=`orderTourStops`は並べるだけ→要件「リージョン越え不可/DC・ワールドはテレポ指示」・要計画(3点未決=別リージョンは追加時block?開始時警告?/指示は移動フェーズの行き方枠に足す?/一時のワールド不明時は指示なしで流す?) / **PF(ハウジンガーPF)は本番デプロイ後にログインで実機**(login本番専用)。**リリース=big3全体をmain反映+`firebase deploy --only firestore`(rules+indexes)+ダミー150件削除+push**(②④PF後・要ユーザーGO)。**隣接課題(別)**: 詳細「他の部屋」が limit20/50 で実最大(90/512)未満に truncate。
