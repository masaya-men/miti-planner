...
0. 🏠 **ハウジング公開前 残タスク**(網羅=`docs/.private/2026-07-15-housing-release-remaining-tasks.md`):
   ...
   - **必須大物**: スマホ対応(詳細以外7面・雑でも動く優先) / モデレ /admin 完結確認(BAN/quota0/一括削除UI) / en翻訳確認 / **ツアー主催機能=Phase1完了(2026-07-15・ブランチ feat/housing-shared-tour-sync・未push/未デプロイ)。spec=`superpowers/specs/2026-07-15-shared-tour-sync-design.md` / plan=`superpowers/plans/2026-07-15-shared-tour-sync.md` / 進捗台帳=`.superpowers/sdd/progress.md`。Phase0=App Check Unenforced→方式(a)匿名onSnapshot。Phase1(型/純関数TDD/発行API/rules/client/幹事直書き/参加者フック=1.1-1.8)全task review clean・build+test緑。次=Phase2 UI(招待パネル/住所露出警告/参加者ページ/閲覧専用・housingトンマナ適用・要デザイン相談)→Phase3(悪用ガード/GC cron)→Phase4実機。⚠(a)将来Firestore Enforced化で匿名参加が壊れる→方式(b)へ / ⚠設計§7の別端末幹事復帰(whereクエリ)は計画にタスク未在(公開判断時に要検討)。ビジョン=`.private/2026-07-08-synced-shared-tour-vision.md`** / 中韓公開(公開前候補に格上げ) / Discord告知(P3=告知可)。
