...
1. **【新機能】 占星術師ドロー chain prompt** — ユーザーが手動で `astral_draw` / `umbral_draw` を 1 個置いた時に「以降 60 秒毎に交互配置しますか?」 と確認するモーダル。 学者の `AetherflowChainPromptModal` を流用、 違いは「交互ロジック」 のみ。 実装規模 ~150 行、 1 セッション完結見込み。 詳細は session 18 末の議論参照
2. (その後) **ハウジング Sub-spec 2B** (Gallery & Search) — 並行プロジェクト
