...
- **🔵 取込フロー v2前半(①+③)=設計+実装計画 完了・未実装**: spec=`docs/superpowers/specs/2026-06-23-import-flow-v2-phase1-design.md` / plan=`docs/superpowers/plans/2026-06-23-import-flow-v2-phase1.md`(6タスク・TDD)。①満杯時削除=既存`LimitResolutionSheet`流用(max_total横断も既存対応)・③コンテンツ選択前段化=`currentContentId`誤紐付け根治。共通化=`contentSelection.ts`/確定ゲート=`importWithLimitCheck.ts`新設・`LimitResolutionSheet`マウントをLayoutへ一元化。**次=subagent-driven-development で実装**。②途中取込(パーティ統合/再アンカーが重い)はspec §7に繰り越し・別タスク。
