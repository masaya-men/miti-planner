...
②マイページ一覧カードの公開状態バッジのメニューが**押しても表示されない(4巡目のcreatePortal化の退行・次セッション最優先)**。ビルド後CSS確認+分離環境での再現テストでJS/CSSロジック自体は正常と判明。実機で1回目の診断スクリプトが「NOT_FOUND」(メニュー要素がDOMに無い)という結果だったが、DevTools開く動作(右クリック等)が誤ってメニューを閉じた可能性があり再検証中。MutationObserverで自動検知する2回目のスクリプトをユーザーに依頼済み・結果待ち。
詳細・診断スクリプト・調査ログ = `docs/.private/2026-07-24-mypage-card-overlay-and-scrollbar-handoff.md`(「2026-07-24 追加調査」セクション)。ユーザーから明示指示: 「セッションを区切って新しいセッションで、文脈を失わずに確実にやること」。
