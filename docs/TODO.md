...
- **🚧 2026-06-19 進捗トラッキングHUD イテレーション2 実装完了・実機総点検待ち (branch `feat/progress-tracking-hud`・HEAD `59672f4`・未push/未merge/未deploy=エンドユーザー不可視)**: 全15タスク TDD + 全体最終レビュー(opus) + マージ前必須fix3件 完了。**A**記録ハイライトがフェーズ/ラベル列で切れる根治(記録モード時のみオーバーレイ透過) / **B**記録トースト(走査線ホログラム起動→文字デコード→数字0カウントアップ・4秒・更新/未更新文言4言語・%=チーム最高到達点÷全長で後退でも減らない) / **C**記録ドロワー(開=記録モードON / 光の道=フェーズ比例配置クリックで時間ジャンプ / 活動ドラッグスクラブ16px低感度 / 踏破=文字のみ / 直前undo=アイコンのみ / 記録一覧・記録開始ボタン廃止・脱ピル) / **D**ホバーでグラフアニメ停止根治(cornerX/Y/points useMemo・canvas不変)。build EXIT0 / vitest 1837 pass・既知housing5件のみ赤(新規fail 0)。設計=specs/2026-06-19-progress-hud-iteration2-design.md / 計画=plans/2026-06-19-progress-hud-iteration2.md / 経緯=[.private/2026-06-18-progress-hud-iteration2.md](./.private/2026-06-18-progress-hud-iteration2.md) / ledger=`.git/sdd/progress.md`。軌跡canvas/お祝いは試作`4c0b94b`1:1移植([[feedback_keep_liked_prototype_visuals]])。**✅ 記録ドロワー右ズレ根治(2026-06-19)**: 真因=開演出WAAPI `el.animate({transform:translateY},fill:forwards)` が完了後 `translateY(0)` を保持し中央寄せの `translateX(-50%)` を上書き→半幅(240px)右ズレ(measurementは正しかった)。中央寄せをtransformから数値left(中心−半幅)へ分離。実VP(1489/DPR2.58)Playwright計測で drawer.centerX≈band.centerX 確認。memory `reference_waapi_transform_clobbers_centering`。**🔴 残=ユーザー実機総点検(A/B/C/D + 4言語 + 記録モードON/OFF回帰 + collab viewer表示) → 問題なければ main merge=本番デプロイ(デフォルトON・ヘッダー直撃のため実機確認必須)。実機判断項目=PCDrawer縦溢れ/top:92px位置/トースト字間/スクラブ色/ko語彙。次フェーズ=スマホ記録対応(既存「スマホ最適化」タスク)・Plan2 collab同期。**
