...
- **今セッション完了**:
  - PCハンドル黒化修正（outline-none、--glass-border修正漏れ）
  - Fキーフォーカスモード改善（常に両方オープン⇔フォーカスのトグル）
  - スマホヘッダーfixed化（テーブルが透過ヘッダー裏をスクロール）
  - ヘッダー/ボトムナビ透過強化（10%背景+blur-md、Apple風）
  - ヘッダーコンパクト化（72→52px、LOPO/コンテンツ名+プラン名の2段）
  - カード行高さ縮小（80→60px）、区切り線/左縦罫線常時表示、時間列固定幅
  - ビルド成功・テスト116全パス

### 次セッションでやること
1. **FAB言語切替の展開式セレクター**
   - 言語ボタン押下→左方向にspring staggeredアニメーションで4言語（日英中韓）展開
   - ICS Media記事（https://ics.media/entry/260402/）のFABパターン参考
   - ネイティブアプリ風の軽快でかわいいモーション
