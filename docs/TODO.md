...
- **🔴 探す地図フィードバック: ①③④✅ユーザーローカル確認OK / ②設計確定→明日実装**。詳細・全設計=`docs/.private/2026-07-12-browse-map-feedback.md`。①③④はコミット済(integration/housing-big3)。②完了後に実機最終確認→main 反映→`firebase deploy --only firestore`(PF rules+indexes先)→push。
  - ④✅ゲート文言ステージ別 / ①✅`setDC`でservers クリア(回帰テスト3件) / ③✅北コンパス+操作ヒント(`BrowseWardMap`)。
  - **②明日ここから**: カード配置刷新=**案B確定**。(1)カードを地図と同じズーム面へ移し**家と完全一体でパン/ズーム**(=カサカサ根治+家と一緒にズーム)、区画中央配置・三角しっぽ廃止・flip廃止。基準サイズは今のまま。(2)ホバー/フォーカスは**必須維持**=必ず読めるサイズにpop・ボタン押下可(screen固定へ一時pop)。(3)別途「1スポット大量部屋(アパ/FC個室)」は専用スクロールパネル(既存 `findChambersInPlot`/`findApartmentRoomsInWard` 流用・UI未実装)。**構造変更なので実装後 clamp/flip/pointer capture/onBlankClick/click拡大/ツアー追加/複数nav/Esc を実機総点検**([[feedback_structural_refactor_runtime_audit]])。まず brainstorming の design doc 化→writing-plans→実装。
