...
- **リアルタイム共同編集 (✅段取り①=部屋+WS骨組み 本番デプロイ済→次は段取り②)**: 1軽減表を複数人同時編集。方式確定=**Yjs(CRDT)+Cloudflare Durable Objects (ライブラリは y-partyserver/partyserver。PartyKit CLI は買収され後継)**、カーソル=P2Pで$0、保存=Firestoreへ間引き書き戻し。**Workers無料プラン$0ハードストップ前提でコスト有界化**(編集8席/閲覧20/同時30/緊急停止)。**DOは無料枠要件でSQLite-backed (new_sqlite_classes) 必須**。設計書=[specs/2026-06-03-realtime-collab-design.md](./superpowers/specs/2026-06-03-realtime-collab-design.md) / 段取り①計画=[plans/2026-06-03-realtime-collab-step1-room-skeleton.md](./superpowers/plans/2026-06-03-realtime-collab-step1-room-skeleton.md)。**段取り①実装=`workers/collab/` (本体src/非干渉・別Worker)、本番=https://lopo-collab.masaya-maeno0106.workers.dev (空の部屋。本体未統合なのでユーザー影響ゼロ)、5テスト緑**。**段取り②-a 実装済(branch `feat/collab-stage2a`・未マージ)=[plans/2026-06-04-realtime-collab-stage2a-mitigations-sync.md](./superpowers/plans/2026-06-04-realtime-collab-stage2a-mitigations-sync.md)**(設計書 [specs/...2a-design.md](./superpowers/specs/2026-06-04-realtime-collab-stage2a-mitigations-sync-design.md) §9 改訂が正典)。**次=ユーザーが2ブラウザで実機確認→OKならマージ**。実装サマリ: サーバ `Room` を `YServer` 化 + `hibernate:true`(idle非課金=$0) + 在室数 `getConnections()` 化 + index.ts で `x-partykit-room` フォールバック(本番/Miniflare両対応)。本番デプロイ済+node 2クライアントで同期/late-join/同時add両方残るを実証。クライアント: `src/lib/collab/`(yjsMitigations変換 + collabProvider=cascade込みhandlers+observeDeep)、store に委譲分岐(yjs非依存)、Firestore自動保存抑制(Layout)、footer に最小トグル。**遅延ロード=collabProvider は動的import(gzip 28.7KBの別チャンク、ソロ初期bundle非混入)**。⚠onLoad/onSave 未実装=全員退室で揮発(§5許容、恒久保存は③)。⚠既知の無関係事象: usePlanStore テストは複数同時実行で vmThreads 汚染により失敗(単独緑・私の変更前から存在)。次段取り=②-b(他要素同期)/②-c(Undo CRDT化)/③(Firestore保存・onError在室減算)。
