...
- **🔴 探す地図フィードバック: ①③④✅ / ②機能=ユーザー実機OK・パネル角=A案実装済→実機確認待ち** (2026-07-12)。詳細=`docs/.private/2026-07-12-browse-map-feedback.md`。**パネル左上角のはみ出し=A案(薄い半透明ベール)で解消**: `.housing-bmap-roompanel` から backdrop-filter/clip-path 撤去→`--housing-bmap-panel-veil`(rgba(17,23,37,0.38))のベタ背景に。ぼかし矩形が消えたので border-radius:14px だけで角丸クリップ確定(台帳末尾に詳細)。build✅。次セッション: **ユーザー実機確認**(DEVハードリロード・角のはみ出し消滅/ベールの濃さ好み/角丸)→OKで**リリース**(main反映→`firebase deploy --only firestore`[PF rules+indexes先]→push→②ダミー150件削除 `seed-housing-overlap-dummy.mjs --clear`+script削除)。
