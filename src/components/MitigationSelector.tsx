...
import { getAetherflowStacks, getAddersgallStacks, canUseSummonSeraph, getRemainingCharges } from '../utils/resourceTracker';
...
export const MitigationSelector: React.FC<MitigationSelectorProps> = ({ isOpen, onClose, onSelect, jobId, position, activeMitigations = [], selectedTime = 0, schAetherflowPattern = 1 }) => {
    ...
    const getResourceStatus = (m: Mitigation): { available: boolean; warning?: boolean; message?: string; badge?: string; badgeColor?: string } => {
        ...
        if ((m.id === 'summon_seraph' || m.id === 'seraphism' || m.id === 'fey_illumination') && !canUseSummonSeraph(selectedTime, schAetherflowPattern)) {
            // Fey Illumination can be placed pre-pull (t<=0) as a special exception
            if (m.id === 'fey_illumination' && selectedTime <= 0) {
                // Allow - pre-pull usage
            } else {
                const minTime = schAetherflowPattern === 1 ? 31 : 44;
                return { available: false, message: `${minTime}s${contentLanguage === 'en' ? '~' : '以降'}` };
            }
        }
