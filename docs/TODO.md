...
0. **✅ 共同編集participant側クラッシュ(ジョイナー経由): 2026-08-07 実スタックトレースで確定・修正・本番デプロイ済み(ユーザー実機確認待ち)**。`Timeline.tsx:1265`の`plan.data.timelineEvents`が、参加者自身の圧縮済み(7日未オープン)個人プランを未ガードで読んでいたのが原因。修正=`plan.data?.timelineEvents?.length ?? 0`。詳細=`docs/.private/2026-08-05-collab-participant-timelineevents-crash.md`。**注**: 2026-08-05報告分(サイドバー経由の通常利用中に発生)は別トリガーの可能性があり真因未確定のまま(再発したら別件として調査)。修正確認中に副次発見2件(下記「現在の状態」参照): ①スマホのジョイナー画面はLayout.tsxを通さない設計のためMobileFAB自体が構造的に存在しない(参加者の展開/フェーズジャンプ等の操作手段が無い) ②圧縮済みプランを参照した場合、そのプランの空判定が今回の修正で「空」確定するため無関係な共同編集ルームの行表示が強制展開される(旧ロジックのまま・クラッシュしなくなって初めて可視化された挙動)。
0-1. **🔴 オーナーが自分のリンクを踏むと参加者扱いになる問題**: 2026-08-07朝に設計書・実装計画は作成済み(`docs/superpowers/specs/2026-08-07-collab-owner-link-and-live-capacity-design.md` / 実装計画は同日コミット`9474aad9`)、未実装。今回のクラッシュ修正とは無関係の既知バグ(ユーザーが同日実機で再確認)。次はTDDで実装計画どおり着手。
...
## 現在の状態 (次セッションはここから読む)
### 🟡 共同編集participantクラッシュ(ジョイナー経由分)は原因特定・修正・本番デプロイ済み(詳細→最上部0番)。実機で「外部共有リンクからの参加」「オーナー自身のリンクを踏んだ場合」の2パターン確認待ち。
