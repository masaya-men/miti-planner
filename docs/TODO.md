...
- **🔴 探す地図フィードバック: ①③④✅ / ②機能=ユーザー実機OK・パネル角=解決(実機OK)→リリース待ち** (2026-07-12)。詳細=`docs/.private/2026-07-12-browse-map-feedback.md`。**左上の四角い出っ張りの真因=パネル裏の区画ピルバー(明るい選択ピル)が半透明パネル越しに透けていた**もの(前セッションの「backdrop-filter 角丸クリップ不良」は誤診)。解決=**ぼかし復活+パネル角丸を裏ピルバーと同一半径に**(共有トークン `--housing-mapctl-areas-radius:9px` を `.housing-mapctl-areas` と `.housing-bmap-roompanel` 両方が参照)。build✅・ユーザー実機OK。**次=リリース(要ユーザーGO)**: main反映→`firebase deploy --only firestore`[PF rules+indexes先]→push→②ダミー150件削除 `seed-housing-overlap-dummy.mjs --clear`+script削除。
  - ④✅ゲート文言ステージ別 / ①✅`setDC`でservers クリア(回帰テスト3件) / ③✅北コンパス+操作ヒント(`BrowseWardMap`)。
  - **②=案B実装完了**(subagent-driven Task1-7・全緑 build+housing1326・未push)。spec=`docs/superpowers/specs/2026-07-12-browse-map-card-plane-design.md`/計画=`docs/superpowers/plans/2026-07-12-browse-map-card-plane.md`/台帳=`.superpowers/sdd/progress.md`末尾。カードを地図面へ(カサカサ根治)+逆スケールpop+大量部屋パネル。**次=ユーザー実機総点検**([[feedback_structural_refactor_runtime_audit]]・plan Task4 Step8+Task6 Step5=12項): カサカサ消滅/家と一体ズーム/hover pop読める&ボタン押下/端clamp/空白閉じ/単発詳細·複数パネル/ツアー追加/Esc/**backdrop-filter崩れたら報告**(→ソリッド地へ)。**隣接課題(別タスク)**: 詳細ページ「他の部屋」が `findApartmentRoomsInWard` limit(20)/`findChambersInPlot` limit(50) で実最大(90/512)未満に truncate(パネルは spot.listings 使用で無影響)。
