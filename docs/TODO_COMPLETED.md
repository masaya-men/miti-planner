...
### ✅ 2026-08-19 軽減表: 賢者「リゾーマタ」・リーパー「アルケインクレスト」追加(コミット済み・未push)
リゾーマタ(SGE Lv74/リキャスト90秒/アダーガル1スタック獲得)をmockData.tsに追加。アルケインクレスト(RPR Lv40/リキャスト30秒/最大HP10%バリア5秒→完全吸収で自身+周囲15mに回復力50のHP継続回復15秒)も追加したが、ユーザー判断で`hidden: true`(セレクターに非表示、データ自体は温存)。効果値・ja/en/zh/zh-Hant/ko名は公式ジョブガイド5サイト(日本語・英語・韓国語・簡体字中国語・繁体字中国語)を直接確認して確定(韓国語・簡体字サイトはJS描画SPAでWebFetch不可→Playwrightでブラウザ操作して取得)。二次情報源との食い違い2件を公式側で訂正(アルケインクレストの回復力=50が正・50/100の混在あり／中国語名=神秘纹〈繁体字:神秘紋〉が正・二次情報源の「奥秘纹章」は誤り)。tsc/build/関連テスト通過。アイコンは`public/icons/Rhizomata.png`・`Arcane_Crest.png`配置済み、コミット`46c4dab4`。
**残(全体共通・TODO.md参照)**: main push→deploy後に`seed-icons.ts`→`seed-skills-stats.ts --dry-run`→本実行の順でFirestore反映必須。

### ✅ 2026-08-19 表示メンバー絞り込みスイッチ = 実装・ユーザー実機確認OK(未push、次のまとめpushで一緒に反映)
詳細=`docs/.private/2026-08-19-party-visibility-switch-handoff.md`。

### ✅ 2026-08-19 ハウジンガー短縮共有URL = 実装・実機確認OK(未push、次のまとめpushで一緒に反映)
