...
- **🟣 進行中=⑦敵攻撃 "or"(2択攻撃)**: TimelineEvent に任意 `altName: LocalizedString` を持たせ「A or B」表示。**名前だけ変わる(ダメージ同じ)確定**。v1=手動編集(EventForm+管理TemplateEditor)+描画(TimelineRow)、ブランチ `feat/event-or-attack`。スプシ自動分割は後追い。**spec 確定→実装計画作成済**=`docs/superpowers/plans/2026-06-22-event-or-attack.md`(6タスクTDD)。次=subagent-driven で実装。spec=`docs/superpowers/specs/2026-06-22-event-or-attack-design.md`。
