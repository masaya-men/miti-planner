...
- **🔴 探す地図フィードバック: ①③④✅ / ②機能=ユーザー実機OK・パネル角=解決(実機OK)→リリース待ち** (2026-07-12)。詳細=`docs/.private/2026-07-12-browse-map-feedback.md`。**左上の四角い出っ張りの真因=パネル裏の区画ピルバー(明るい選択ピル)が半透明パネル越しに透けていた**もの(前セッションの「backdrop-filter 角丸クリップ不良」は誤診)。解決=**ぼかし復活+パネル角丸を裏ピルバーと同一半径に**(共有トークン `--housing-mapctl-areas-radius:9px` を `.housing-mapctl-areas` と `.housing-bmap-roompanel` 両方が参照)。build✅・ユーザー実機OK。この後は big3 全体のリリース前検証へ(下記 2 行)。
  - **一時ツアー実機フィードバック対応済 (このsession・全commit済/build+test緑/ユーザー実機OK)**: 「+住所から追加」を HousingPanelModal 化(トレイ直置きの overflow バグ根治)+一時上限30→50+背景 frost(暗幕やめ薄ぼかし)+住所入力を登録と同じ構造化フォーム(`RegisterSectionAddress` variant='tour'・DC/サーバー込み・フリーテキスト廃止)+select▼左スライド不具合根治(background→background-color・登録画面も直る)+select pointer+モーダル色をトンマナ半透明ネイビーへ。詳細=`docs/.private/2026-07-12-big3-release-verification-checklist.md`。
  - **残(リリース前)**: **②タブ削除=✅完了**(`573eaa24`) / **④跨ぎ処理=✅実装完了+2026-07-13実機FB一巡(注記DC直下+ロック/使い捨て説明を上部/フィルターフェード+DC→データセンター/建物タイプ公式表記/自作▲▼ステッパー/ヘッダー検索撤去/号棟横並び/roomKind配線漏れバグ)→**dev目視セッション2: ①③⑥OK / ②がたつき=保留(非ブロッカー) / ④地域最上部+DC連動=実装済(ユーザーOK) / ⑤ヘッダー横断検索=実装完了(dev確認待ち・独立レビューでCritical/Important修正済) → 詳細は同docの「セッション2」節**・個室ツアーはフォローアップ・詳細=`.private/2026-07-12-big3-release-verification-checklist.md`**(commit `3ff3dde`〜`485c33d9`・全vitest3120緑+build緑+最終レビューCritical0。目視項目=`docs/.private/2026-07-12-big3-release-verification-checklist.md` の ④節。設計/計画/台帳=`docs/superpowers/{specs,plans}/2026-07-12-housing-tour-region-crossing*`+`.superpowers/sdd/progress-tour-crossing.md`。決定=リージョン越え不可(追加時block+開始時net)/DC=DCトラベル指示/ワールド=ワールド訪問指示+中央マップぼかし案内/一時追加もワールド必須化) / **PF(ハウジンガーPF)は本番デプロイ後にログインで実機**(login本番専用)。**リリース=big3全体をmain反映+`firebase deploy --only firestore`(rules+indexes)+ダミー150件削除+push**(④dev目視+PF後・要ユーザーGO)。**隣接課題(別)**: 詳細「他の部屋」が limit20/50 で実最大(90/512)未満に truncate。
  - **🧹 ②確認用ダミー稼働中**: 本番Firestore Mana/Anima Mist1区に150件(`ownerUid=dev-dummy-overlap`・区画1-30家×1/区画5FC個室30/アパ棟1満室90)。②完了・確認後に `node scripts/seed-housing-overlap-dummy.mjs --clear` で削除+script削除。
...
- **✅📐 plot→size 表 / 住所抽出v2 / 住所誤爆の根治 / 行き方データ整備 — 全て本番反映済 (2026-07-10)**。詳細→[TODO_COMPLETED.md](./TODO_COMPLETED.md) + `docs/.private/2026-07-10-plot-size-table-and-address-v2.md`。要点=**確定表の再調査は不要** / **辞書 (`masterData.ts`) に短い ASCII 略称を足すな** ([[feedback_no_speculative_alias_data]]) / **行き方の正典は `directions-src/*.csv`、スプシは引退** ([[reference_housing_directions_csv_canonical]])。残る低優先の限界2点は上記 private doc に集約。
