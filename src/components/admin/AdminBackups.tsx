  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-lg font-bold">{t('admin.backups_title')}</h1>
        <button
      ...
      {!loading && filtered.length === 0 && (
        <p className="text-xs text-app-text-muted">{t('admin.backups_no_data')}</p>
      )}
        ...
        <div className="border border-app-text/10 rounded divide-y divide-app-text/5">
          {filtered.map((entry) => {
            ...
            return (
              ...
              >
                {/* タイプバッジ */}
                <span className="shrink-0 text-[10px] border border-app-text/20 rounded px-1.5 py-0.5 font-mono">
                  {typeToFilterKey(entry)}
                </span>

                {/* 情報 */}
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-medium truncate">{entryLabel(entry)}</div>
                ...
                {isConfirming ? (
                  <div className="shrink-0 flex items-center gap-2">
                    <span className="text-[10px] text-app-text-muted max-w-[160px] text-right leading-tight">
                      {t('admin.backups_restore_confirm')}
                    </span>
                    <button
                      onClick={() => handleRestore(entry)}
                      disabled={restoring}
                      className="px-2 py-1 text-[10px] border border-app-text rounded bg-app-text text-app-bg hover:opacity-80 transition-opacity disabled:opacity-50"
                    >
                      {restoring ? '...' : 'OK'}
                    </button>
                    <button
                      ...
                      disabled={restoring}
                      className="px-2 py-1 text-[10px] border border-app-text/30 rounded hover:bg-app-text/10 transition-colors"
                    >
                      ✕
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="shrink-0 px-3 py-1 text-xs border border-app-text/30 rounded hover:bg-app-text/10 transition-colors"
                  >
