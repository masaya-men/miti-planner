...
- 改善(ユーザー提案・実装済み・未push): DC/ワールド跨ぎの案内文言を実操作+着地区画名入りに強化(①DCトラベルはログアウトが要ると判明し明記②着地先のハウジングエリア区画名を追加③中央カードと右パネルの重複文言を右パネル側撤去で解消)。詳細な調査根拠・文言before/after・実装ファイル一覧 → `docs/.private/2026-08-10-tour-crossing-wording-revision.md`。
