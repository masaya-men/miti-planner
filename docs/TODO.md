...
### 🔴 次セッション最優先2: 実機フィードバック3件(2026-07-21・ブレスト完了・実装待ち)
1+3. **探すページの表示順ランダム化+一覧スクロール位置復元** = **設計完了**(`docs/superpowers/specs/2026-07-21-housing-browse-random-order-scroll-restore-design.md`)。デフォルトをランダム表示に変更(住所順は廃止・新着/古い順は選択肢に残す)、アプリ内遷移では再シャッフルせず実リロード/シャッフルボタンのみ再抽選。スクロール復元は探す/お気に入り/ハウジンガープロフィールの3画面が対象。
2. **初心者向けタグ「ハウジング若葉」** = **設計完了**、既存タグ計画書のPhase Cとして追記済み(`docs/superpowers/plans/2026-07-10-housing-tag-overhaul-plan.md`末尾)。kind registryに1件追加するだけの小規模作業。
...
- **タグ仕様全面刷新**: **Phase A+B 本番稼働済み**(2026-07-21ブレスト中に判明・TODO_COMPLETED未記載だった実態を確認。`src/data/housingTags.ts`に公式23+季節12+テーマ12+個人タグの新レジストリ実装済み、個人タグAPI・admin通報機能も稼働中)。計画書=`docs/superpowers/plans/2026-07-10-housing-tag-overhaul-plan.md`。**Phase C(初心者タグ「ハウジング若葉」)を2026-07-21追加決定**、同ファイル末尾に追記済み・次回実装。
