...
describe('commitImportedPlan: 取り込みは共同編集中でも「取込データ」で新規プランを作る (Bug① 止血)', () => {
  ...
  // ユーザー報告(2026-06-30): 絶アレキ(Lv80)を選んで取込したのに Lv100 スキルが全部出る。
  // 原因 = currentLevel が 100 ハードコードでコンテンツの level を無視していた。
  // NewPlanModal / Sidebar は content.level を使う = それに合わせる。
  it('★コンテンツの level を currentLevel に反映する (tea=絶アレキ Lv80)', () => {
    const newId = commitImportedPlan(makeImportResult(), { contentId: 'tea', title: 'スプシ取込' });
    const newPlan = usePlanStore.getState().plans.find((p) => p.id === newId)!;
    expect(newPlan.data.currentLevel).toBe(80);
  });

  it('contentId が null(コンテンツ未選択)なら currentLevel=100 にフォールバック', () => {
    const newId = commitImportedPlan(makeImportResult(), { contentId: null, title: 'スプシ取込' });
    const newPlan = usePlanStore.getState().plans.find((p) => p.id === newId)!;
    expect(newPlan.data.currentLevel).toBe(100);
  });
});
