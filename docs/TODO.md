...
②マイページ一覧カードの公開状態バッジのメニュー非表示 = ✅**2回の修正を経て解決・本番反映待ち**。1回目(--housing-*変数をhousing-workspace等の共有セレクタに追加)で不可視バグは直ったが、その共有セレクタにページ全体のシェルレイアウト(width:100%/height:100vh等)も同居しており、メニューが画面いっぱいの板(実測819×673px)になる新バグを作った。2回目で共有セレクタへの追加をやめ、専用の小さいルールで必要な変数だけ個別再定義して解決。ユーザーに本番での最終確認を依頼中。
