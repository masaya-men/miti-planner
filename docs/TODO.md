...
- **🚧 2026-06-18 表の情報列固定(PC横スクロール時) 実装中・branch `feat/timeline-frozen-info-columns`(未push)**: 横スクロールで左の情報列ブロック(フェーズ/ラベル/時間/敵攻撃/元/軽減後ダメージ)を固定、スキル列だけスクロール(Excel枠固定相当)。常時固定/トグル無/モバイル対象外。設計=specs/2026-06-18-timeline-frozen-info-columns-design.md / 計画=plans/2026-06-18-timeline-frozen-info-columns.md(4フェーズ7タスク・subagent-driven)。**進捗=Task1-3完了(全コミット済・build/テスト緑)**: ①固定ペイン用CSS(`.timeline-info-pane`/`--scrolled`) ②ヘッダー/コントロールバーtranslateXをスキル領域限定(`#timeline-header-skill`/`#timeline-controls-skill`・純関数`applyHorizontalScrollSync`) ③情報列を`TimelineInfoColumns.tsx`へ抽出(見た目不変)。**核心リスク=content-visibility×軽減バー別レイヤーのstacking → Task4のz:60不透明sticky情報ペインで解消予定**。**次セッション最初(Task4着手前)に対応**=Task3レビュー指摘: (a)`DamageInfo`型の二重定義を1箇所集約 (b)不要re-export除外(PcTypeToggleのみ残す) (c)swapMarkers/contentLanguageを内部フック取得に変更済→Task4で行ごと購読倍増のperf確認。台帳=`.git/sdd/progress.md`。**残=Task4(本体)/5(影)/6(hover)/7(QA)+最終レビュー**。
