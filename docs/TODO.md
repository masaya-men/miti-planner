...
**🔴 次セッション最優先: 「連動アニメの初動だけかくつく」根本原因を特定済み・本命修正は未着手**。詳細は`docs/.private/2026-08-14-mobile-scroll-stutter-investigation.md`。**結論**: `.timeline-scroll-container`に登録された5つの`scroll`関連リスナー(RecastRowのCD表示更新/フェーズ見出し/エフェクト棒/ヘッダー同期/スムーズホイール)が同じスクロールイベントの中で毎回まとめて実行され、それぞれが起こすスタイル無効化コストの**合計**が初動の重さの正体(実測で確定: 5つ全部を同時に軽量化して初めて重いフレームがほぼ消えることを確認、1つずつ軽量化しても効果ゼロ)。次回は5つを個別に温存したまま直すのではなく、**1つの統合ハンドラにまとめる**方向で設計し直すこと(詳細な実装方針も上記docに記載済み)。
**✅ 暫定対応2件・本番反映済み**(2026-08-14、根本修正が済むまでのユーザー判断): ①エフェクト棒の「伸びる/縮む」演出を廃止・opacityクロスフェードのみに変更。②その上でユーザー判断により**「連動」モードをFABピッカーの選択肢から一時除外**(`icon`/`bar`の2択に変更、`useThemeStore`に`migrate`追加で既存`scroll`選択ユーザーは`icon`へ自動移行)。**🆕 ユーザー実機報告で追加発見**: 「barモードの方が軽い、iconはFAB消える瞬間にカクつく」→調査したところ`.mobile-row-dim-text`(行の文字濃淡)の`[data-mobile-scrolling="1"]`属性セレクタが独立した主犯の1つと判明(実測: iconモードでこのルールだけ無効化すると重いフレームがほぼ半減)。詳細・次の設計方針は上記docに追記済み。**次セッション最初にユーザー実機確認を聞いてから根本修正(5リスナー統合+この属性セレクタ対応)に着手**。
