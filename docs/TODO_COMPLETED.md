...
### ✅ 2026-08-27 バリアの重なり方ルール + iOS表示ズレ共通フック + シールド棒の早期終了 = 全てデプロイ・ユーザー実機確認OK
(1) **バリアの重なり方ルール** (Task1〜6・コミット `a7e5ff30`・Firestore `--apply`済/28スキル/dataVersion++): FF14実測の固定消費優先順位表(パッチ7.31)を再現。純関数チェーン `barrierStacking.ts`(`resolveBarrierConflict`/`resolveContextShields`/`stepShieldAbsorption`/`shieldCoverageContext`) → `contextShieldEntries.ts` → `Timeline.tsx` `damageMapResult` の2フェーズ吸収ループ。鼓舞系は大きい方が残る・エウクラシア系の一方通行・二重削り修正。🛡️列の意味を「残バリア量」→「その被弾で吸収した量」に変更(吸収0の被弾は🛡️バッジ非表示)。(2) **iOS表示ズレ** (①): 共通フック `useIOSViewportFix` に集約(Layout/HousingShell/CollabJoinerPage)。(3) **シールド棒の早期終了** (②): バリア吸収し切り時刻(`shieldExhaustedAt`)で棒をPC/スマホ早期終了、全体バリアは全体攻撃でのみ終了(`shieldCoverageContext`)。①⑥②フォロー(スマホ展開戦術バー解禁/設計書typo/削れたEディアへの鼓舞上書き)も対応済(〜`4c4c9509`)。ユーザー実機確認: 鼓舞→意気軒高で意気軒高がすぐ終わる/容量超ダメージで棒が切れる/ハイマ・パンハイマは1層壊れても続く/意気軒高にタンクバスターだけでは切れない、全てOK。iPhone表示ズレもOK。
**残った小課題(別件・急ぎなし)**: Task7=上書き負けバリアのグレー2色棒(方式c・見た目の追加情報のみ・やるなら要指示) / 上書き勝敗が被弾イベント時にしか確定しないper-hit近似(根本対応は詠唱時刻で先に全解決する大改修) / EventForm逆算がまだ加算式(同グループ2枚で過大評価) / tempera_grassa に `scope:'self'` が無く全体扱い(吸収量には影響なし・要承認)。詳細=`docs/.private/2026-08-27-barrier-stacking-handoff.md`。

### ✅ 2026-08-26〜27 軽減表ブラッシュアップ6件+バリア計算バグ2件、全てpush済み・ユーザー確認OK
①PCエフェクト棒ホバーでスキルアイコンだけ浮かぶツールチップ(棒クリックは配置マス目へ転送、当たり判定は左右3pxずつ拡張)/②スマホFABの表示切替タップ不安定(`active:scale-90`とframer-motionのtransform競合、2026-08-13にメインFABボタンで解決済みの同一原因)/③スマホでメモが表示されてしまう(`MemoOverlay`にモバイル判定なし)/④表示切替・並び替えが自分の別プラン/無関係な共同編集ルームに持ち越される(`PlanData`の一部としてプランごとに独立保存、共同編集ルームは入室毎にリセット)/⑤共同編集の参加者ページで表示切替・並び替えがグレーアウト/見せかけボタンだった(部屋のデータには影響しないローカル設定と確認済みのため解禁)/⑥表示切替パネルの空席がロール名表示になり全表示・全非表示・反転の3ボタンを追加。全てtsc/build/vitest通過・ユーザー「OK」確認済み。
### ✅ 2026-08-26 軽減表バリア計算バグ2件、本番反映・ユーザー実機確認OK
展開戦術のコピー計算でランパート等の自己バフが本人以外にも乗るバグ、および最大HP%固定タイプのバリア6種(ディヴァインヴェール/ブラックナイト/アルケインクレスト/シェイクオフ/テンペラグラッサ/インプロビゼーション)に回復効果アップ・確定クリティカルが誤って乗るバグを修正(`isPotencyBasedShield`をscholarShieldRules.tsへ切り出し、全28バリア技を回帰テストで網羅)。コミット`1dbc75ef`。

### ✅ 2026-08-24 管理者がお気に入り物件のNEWリボンを期間限定で手動固定表示できる機能・push→デプロイ済み(コミット f67b07a7/ea071e42/b0d17ccb)
`publishUntil`(公開期限)と同じ「期限(epoch ms)・未来なら有効・遅延評価、cron不要」設計で`pinnedNewUntil`をHousingListingに追加。/adminに新規ページ「ハウジング NEW リボン固定」(物件URL/ID検索→日数指定→固定/解除、`resource=housing_new_badge`)を追加、探すページのListingCardは自動NEW判定(投稿からN日以内)にORで対応。
**デプロイ後の実機報告→即修正(2件)**: ①「固定したのに探すページに反映されない」: 管理画面での保存(Firestore書き込み)自体は成功していたが、公開窓口(`api/housing/public`)の配信経路に**2段の許可リスト**があり両方に新フィールドの登録漏れ(Firestoreクエリ側の`.select(...SELECT_FIELDS)`、および出力側の`projectPublicListing`の`SAFE_FIELDS`)。加えてバージョン番号(`housing_meta/public.version`)ベースのキャッシュがコード修正だけでは切り替わらず古いキャッシュを配信し続ける仕組みだったため、手動でバージョンを1つ進めて解消。本番APIを直接叩いて実データを確認しながら特定。②「NEWは付いたが上に固定されない」: 探すページのランダム表示モードの並び順を決める`shuffleWithNewPinned`(既存の自動NEW固定表示機構)が投稿日ベースの`isNewListing`判定しか見ておらず、新設した`pinnedNewUntil`を考慮していなかった(リボン表示ロジックと並び順ロジックが別関数だったための漏れ)。`isPinnedNew`を`isNewListing`と同じ条件でOR結合するよう修正。全てTDD(失敗テスト先行)+本番API直接確認で検証、tsc/build/テスト通過。
...
③**物件詳細ページのYouTube動画が再生終了後に関連動画オーバーレイで隠れる不具合**。loop/playlistパラメータが無く、再生が終わるとYouTube純正の「関連動画」オーバーレイが動画を覆っていた(カードのambient動画=`HousingCardVideoOverlay`では既にloop対策済みだったが物件詳細ギャラリー側は未対策)。loop=1&playlist=(自身のID)を追加。
**調査の副産物(対応しない方針で決着・TODO.md参照)**: 探すページのカード上でYouTube動画自動再生・ループ復帰のたびに、YouTube側が強制的にタイトル/コントロール画面を3〜4秒表示する仕様をPlaywrightで実機再現・特定。`controls=0`では止められずJS版プレーヤーAPIへの切り替えが必要な規模のため今回は着手しないとユーザー判断。
...
### ✅ 2026-08-19 軽減表: 賢者「リゾーマタ」・リーパー「アルケインクレスト」追加 = push・デプロイ・Firestore反映済み
リゾーマタ(SGE Lv74/リキャスト90秒/アダーガル1スタック獲得)をmockData.tsに追加。アルケインクレスト(RPR Lv40/リキャスト30秒/最大HP10%バリア5秒→完全吸収で自身+周囲15mに回復力50のHP継続回復15秒)も追加したが、ユーザー判断で`hidden: true`(セレクターに非表示、データ自体は温存)。効果値・ja/en/zh/zh-Hant/ko名は公式ジョブガイド5サイト(日本語・英語・韓国語・簡体字中国語・繁体字中国語)を直接確認して確定(韓国語・簡体字サイトはJS描画SPAでWebFetch不可→Playwrightでブラウザ操作して取得)。二次情報源との食い違い2件を公式側で訂正(アルケインクレストの回復力=50が正・50/100の混在あり／中国語名=神秘纹〈繁体字:神秘紋〉が正・二次情報源の「奥秘纹章」は誤り)。tsc/build/関連テスト通過。コミット`46c4dab4`、2026-08-20 push→デプロイ→`seed-icons.ts`→`seed-skills-stats.ts`実行済み、Firestore(`master/skills`)の内容を直接読み出して2件とも意図通り(アルケインクレストのみhidden:true)であることを確認済み。

### ✅ 2026-08-19 表示メンバー絞り込みスイッチ = 実装・ユーザー実機確認OK・2026-08-20 push・デプロイ済み
詳細=`docs/.private/2026-08-19-party-visibility-switch-handoff.md`。

### ✅ 2026-08-19 ハウジンガー短縮共有URL = 実装・実機確認OK・2026-08-20 push・デプロイ済み
`/housing/housinger/<64文字ハッシュ>` → `/h/<名前>-<識別コード8桁>`。ユーザーが実際の自分の
プロフィールで短縮リンク(`https://lopoly.app/h/Masaya-MenLoPo-d34d9c56`)を確認し「いい感じ」
とOK。「見つかる」経路の実機確認も完了、残タスクなし。詳細=`docs/.private/2026-08-19-housinger-short-url-handoff.md`。

### ✅ 2026-08-19 Allmarksまとめてインポート = 実装・実機バグ修正済み・2026-08-20 push・デプロイ済み(リージョン混在の実リンク検証のみ保留)
一時ツアーへAllmarks(別プロジェクト)のタグ付き共有リンクを貼ると、含まれる物件URLを一括で読み取り自動追加。「1件も取り込めない」不具合(Allmarks側レスポンス形が`{share:{cards:[...]}}`なのに`data.cards`を直読みしていた)を実リンクで発見・修正・再検証済み。リージョン混在時は全件取り込み後に「どちらをツアーにするか」を選ばせる方式に決着。進捗インジケーターはユーザー提供の波形バー+跳ねるボール+右端の家に衝突するデザインで最終決着・ユーザー確認「完璧」。詳細=`docs/.private/2026-08-19-allmarks-import-handoff.md`。
**残**: 2リージョン混在の実共有リンクが無いため、選択肢UI自体の実機検証は未実施(ロジックはテストで担保済み、本番利用時に問題があれば対応)。

### ✅ 2026-08-16 ハウジング探すページ「NEW」演出 + 管理画面日数設定 = 本番反映済み(2026-08-17push、実機の見た目確認はまだ)
投稿7日以内の物件を、ランダム表示時のみ先頭固定+左上NEWリボン+縁を1周光るビーム演出(画面内に入るたび再生・カードごとに0〜3秒ずらす)で表示。日数は`/admin/config`(要ログイン)で1〜90日の範囲で変更可能(`master/config.newListingWindowDays`)。ビームCSSはmask-composite方式、`::before`にはReact styleが効かないため`--beam-delay`カスタムプロパティ経由。

### ✅ 2026-08-18 YouTube概要欄住所自動入力(2026-08-17新機能)の実機FB2件を修正 + ハウジング多言語の訳し漏れ一括解消 = 本番反映済み
①番地(plot)は取れているのに建物タイプ未選択のまま残る不具合(サイズ表記頼みの判定だった`RegisterPage.tsx`を、番地が取れた事実で個人宅を自動選択するよう修正)/ 重複チェック警告を住所確認ボタン直下にも表示。②housing.*未翻訳330件(英74/韓121/簡体中135)/スプシ取込プレビューのzh-Hant非対応/コンテンツ`dmu`の韓国語名欠落、を一括解消。コミット`e84b845b`/`44d32862`/`b8398116`。

### ✅ 2026-08-17 モバイル軽減表エフェクト表示バグ修正 = 本番反映済み(コミット`48ce4983`)
`mobileEffectBar.ts`の優先順位アルゴリズムを時系列スイープ方式に修正(優先順位の高い人の終盤使用で枠が誤って埋まり判定されるバグ)。

### ✅ 2026-08-17 SEOソフト404対策 = 実装・レビュー完了・本番反映済み
ハウジンガー/共有プラン/ツアー招待/物件詳細の4動的ページで、データ不存在時は真の404を返し、存在時はサーバー側で可視テキストスナップショットを`<div id="root">`に埋め込むよう修正。レビュー過程で本番リスクのある重大バグ4件(NodeNext拡張子欠落+JSON直import/タイトル未入力でSEOタイトル重複/ハウジンガー公開件数の誤表示/24hブラウザキャッシュでデプロイ直後白画面リスク)を発見しその場で修正済み。**デプロイ後のインフラ設定が別途残っている(TODO.md参照)**。

### ✅ 2026-08-14 ハウジング探すページ(PC/スマホ)見た目改善 + モバイル軽減表全面刷新・競合表示 = 本番push/デプロイ済
ハウジング探すページ(中央パネルのツールバー1段集約・常時区切り線・パネル下端フェード・モバイル右余白修正)、モバイル軽減表レイアウト刷新+競合(CDかぶり)表示、Discordログインキャンセル修正、全て実機確認済みで本番反映済み。

### ✅ 2026-08-14 スマホのスクロール重さ調査 = ①ハウジング②軽減表 とも根本原因特定・修正・本番反映完了

ユーザー実機報告「軽減表もハウジングもスクロールが重い(連動エフェクト表示のとき)」の調査。systematic-debuggingで進行。

**①ハウジング探すページ**: 同日追加した下端フェード演出が、スクロールする本体要素`.housing-listing-grid`に直接`mask-image`を掛けていたのが原因(iOS Safariでスクロール毎のマスク再合成が重い典型パターン)。マスクをスクロールしない親`.housing-listing-grid-wrap`へ移動して解消(wrap下端=grid下端が一致することはPlaywrightで実測確認、見た目は完全不変)。ユーザー実機確認OK。

**②軽減表モバイル 連動エフェクト棒**: 複数回の誤仮説を経て根治。
1. 1回目仮説「画面外の棒がスクロール毎に再計算されている」→`MobileEffectBarLayer.tsx`に`content-visibility:auto`+`contain-intrinsic-size`を追加したが、ユーザー実機検証で重さ改善せず、かつ競合アイコンがつぶれる新規不具合が発生→即差し戻し。
2. 2回目仮説「`syncRecastRow`が`hideEmptyRows`時に`timeToYMap`を毎スクロールforEach全走査」→ソート済み配列+二分探索(`sortedTimeYRef`)に置き換え(旧実装との出力一致は40,000ケースのランダム検証済み)たが、ユーザー実機再検証で改善せず。「共同編集じゃなくても重い」との情報でcollab固有ではないと確定。
3. 3回目、ローカルでPlaywright+CDPを使い「軽減149個・900秒戦闘」のテストデータを注入し4倍CPUスロットルで計測。CPUサンプリングプロファイラでは`handleScrollSync`が支配的という手がかりを得たが、内部で呼ばれる`coords.ts`の`sortedEntries`(全走査+毎回ソート)が真因という解釈は追加検証(単体マイクロベンチマーク)で誤りと判明・撤回。ただし`sortedEntries`のWeakMapキャッシュ化(単体56倍高速化・45,000ケースで正当性検証)と`handleScrollSync`内`querySelector`のキャッシュ化は安全な改善として実装・反映(効果は限定的と正直に説明済み)。
4. ユーザーから決定的な情報「静止/常時はさくさく、連動だけ重い」「メモがスマホ共同編集表に見えている(別件)」「エフェクト保持のはずが指を離す前にアイコンに戻る(別件)」を得て方針転換。CPUサンプリングはノイズが大きく信頼性不足と判明したため、Chrome DevTools Protocol の`Tracing`(実イベント記録、スタックトレース付き)に切り替えて再調査。`mobileEffectBarMode`を'icon'/'bar'/'scroll'で切り替えて同条件A/B比較したところ、'scroll'モードのみ`UpdateLayoutTree`(スタイル再計算)コストが他の3.7倍という数値で完全再現に成功。
5. **根本原因確定**: 連動モードのみ`--mobile-effect-bar-progress`カスタムプロパティがスクロール毎に異なる値へ書き換わり(静止/常時は値が固定)、それを参照する多数の要素(棒・アイコン・専用行アイコン)のスタイル再計算が走っていた。未登録のCSSカスタムプロパティは値の型が不定(`syntax:'*'`)なため、値が変わるたびにブラウザが広範囲を保守的に再計算する。
6. **修正**: `src/index.css`に`@property --mobile-effect-bar-progress { syntax:'<number>'; inherits:true; initial-value:0; }`を追加(CSS Properties and Values API、Tailwind自身も内部で使用している標準機能)。型を明示することでブラウザが軽量な再計算経路を使えるようになる。実測で連動モードの重さが静止/常時比3.7倍→1.3倍まで改善。計算結果(opacity/transform)が完全に同一であることも数値検証済み、見た目は一切不変。ユーザー実機確認OK。
7. **業界標準調査**(ユーザー依頼): `will-change`先回り付与は2026年時点でむしろ非推奨(ブラウザの自動判定に任せるのが現行ベストプラクティス、MDN確認済み)と判明し見送り。本命の`animation-timeline: scroll()`(CSS純正スクロール駆動アニメーション)はSafari 26ベータでようやく対応開始した段階(WebKit公式ブログ確認)で正式版はまだのため、今回は見送り。将来Safari正式対応後の移行検討メモを`docs/.private/2026-08-14-scroll-driven-animation-future-migration.md`に記録。
8. **追加報告「全体は軽くなったが連動アニメ自体だけFPSが低い」**: スクロール中も80〜400msの「追いつき」transitionを掛けていたため、進み具合の書き換え頻度(約16〜20msおき)にtransitionが追いつけず常に目標を追いかけ続ける状態だったのが原因。スクロール中(`data-mobile-scrolling="1"`)は`transition:none`にして指の位置に直結、指を離した後の戻り演出だけ既存の400ms easingに任せる方式に変更。transition-durationが意図通り(スクロール中0s→離した後0.4s)であることを実測確認。**ユーザー実機確認OK・本番反映済み**。
9. **さらに残った「連動アニメの初動(スクロールし始めた瞬間)だけかくつく」**: ローカルのCDP Tracingで、スクロール開始2〜4回目のイベントだけ通常の10倍以上(約230ms、影響要素数5,500規模)重い`UpdateLayoutTree`が発生することを発見。「FABを隠す処理(React state更新+Framer Motionアニメ起動)との競合」という仮説を立て、`window.dispatchEvent`を`setTimeout`でマクロタスクへ逃がす修正を実装したが、**実測で効果ゼロと判明・差し戻し**(コードはクリーンな状態に復元済み)。データ注入直後の設定処理待ちを3秒に延ばしても同じスパイクが再現したため「テスト環境がまだ落ち着いていない」線も否定。**根本原因は依然未特定**、ユーザー判断で次セッションへ持ち越し(詳細はTODO.md参照)。ローカルの機械的なスクロール再現(`scrollTop += 20`をrAFループ)がこの細かい現象を正しく模倣できているか自体に疑問が残っている。

**別件として記録(未対応)**: メモ機能がスマホの共同編集表で表示されている(意図的に非表示にしたはずとの認識だったが、`MemoOverlay`にモバイル非表示条件は無く、おそらく新規作成操作のみモバイルで無効化されていた)。連動モードで指を離す前にエフェクト表示からアイコン表示へ戻ってしまう不具合(重さによる取りこぼしの可能性、未検証)。両方とも次回別途調査が必要。

### ✅ 2026-08-14 ハウジング探すページ(PC/スマホ) 見た目改善 = 実機確認完了・本番push/デプロイ完了

ユーザーから「PCスマホともに少しだけ改善したい。1つずつ丁寧に」との依頼。ローカルdevサーバー(LAN公開)で1項目ずつ実装→ユーザー実機確認→次、のループで進行。

1. **中央パネルのツールバーを1段に集約**: 「一覧|マップ|タグ」トグル(`BrowseViewToggle`)+「ハウジング一覧N件・並び替え」が縦に2段だったのを1段にまとめ、カード表示エリアを拡大。`ListingGrid.tsx`に`toolbarPrefix`スロットを追加し、`BrowsePage.tsx`から一覧表示時のみ差し込む方式(他ページのListingGrid再利用箇所や、モバイル(トグル自体がCSS非表示)には影響しないようスコープ)。
2. **常時区切り線**: 集約した分ツールバーとカードの境目が曖昧にならないよう、`.housing-browse-col-center .housing-listing-grid-toolbar`に常時`border-bottom`を追加。当初はスクロール連動で出し分ける実装だったが、ユーザー判断で「常に出しておいていい、シンプルな方」に確定。
3. **パネル下端のフェード**: 実機(1489×679)では行の高さがパネル高さの整数倍にならず、最後の行がタイトル/ボタン無しで画像だけ中途半端に見切れる不具合をユーザー実機スクショで発見。カードを大きくして埋めるのではなく`mask-image`の下端40pxグラデーションで自然に見切れさせる方式に。あわせて中央カラムの下端内側余白(`padding-bottom`)も0にしてパネルを使い切る。
4. **上端のフェードは不採用**: 上端は既に区切り線があり「ヘッダーの下に潜り込む」自然な見え方のため、フェードは追加しない(ユーザー確認済み)。AppleのLiquid Glass的なガラスヘッダー化も検討したが、探すページは2026-07-01に「質感A案」(フラット)へ確定済みのため見送り。
5. **モバイル: 並び替えボタン・区切り線が画面右端に張り付く不具合**: `.housing-browse-col-center`がモバイルで`padding-right:0`(スクロールバーを画面端に出す設計)のため、グリッド外にあるツールバーだけ右余白が無くなっていた。カード側の右12px余白と揃えるため、モバイル専用に`margin-right:12px`を追加(paddingだとborder-bottomの線が右端に残ってしまうためmarginで箱自体を狭めた)。

tsc/build確認済み、関連vitest(BrowsePage.test.tsx 10件、HousingerPage.test.tsx 23件)は単体実行で全通過(フルスイート実行時のみ既知のvmThreads起因の不安定さで無関係な失敗が出ることを確認済み・詳細→本ファイル下部の既知課題)。
...
**実機FBで派生バグ4件を発見・全て根治**:
1. **ランパート等が赤くならない**(根本原因): 一部技はレベル帯で内部idが分かれる(例: Lv93以下`rampart_pld`/Lv94以上`rampart_v2_pld`)。過去セッションの「同id版違いバグ」対策(`_base`/`_v2`別id化)時に`getSharedCooldownIds`がid単純一致のまま放置され、旧idで置いた軽減が現レベルの新idと同じ技と認識されずCD競合判定から漏れていた(単一idのディヴァインヴェールは無事だった)。対象はランパート/リプライザル/アドル/牽制/オーロラ/インターベンション/秘策/展開戦術/タクティシャン/トルバドゥール/ピュシス/ゾーエ/コンシテーション/ユークレイシア・プログノシス/アスペクテッド・ヘリオス/アサイラム/テトラグラマトン/守りのサンバ(計18技)+原初の血気ライン特例。`getSharedCooldownIds`を「同一jobId・同一icon=同じ技」で自動グルーピングする方式に変更(ハードコード列挙不要)。血気(bloodwhetting)⇄猛り(nascent_flash)ラインは別名・別アイコンでも同一CD共有の特例のため`raw_intuition`/`nascent_flash_base`含め4id明示グループ化。回帰テスト4件追加(`versionSkewConflicts.test.ts`)。
2. **画面外シェブロンがフェーズ名バーに被る**: `ConflictOffscreenArrows`に`topInset`prop追加、モバイル+フェーズありなら`MOBILE_TOKENS.header.phaseBarHeight`分だけ上矢印を下にずらす。
3. **エフェクト常時/連動表示モードで競合パルスが出ない**: このモードだと専用行アイコン(`.mobile-miti-icons`)がopacity:0で隠れ`MobileEffectBarLayer.tsx`側の棒アイコンに変身するが、そちらへのパルス配線が抜けていた。`conflictingIds`prop追加で解消。
4. **下矢印がボトムナビに埋もれる**: 矢印のsticky土台がスクロールコンテナのpadding-top(ヘッダー分52px)だけ画面上端からずれた位置を基準にしていたため、「画面下端からの距離」計算にそのズレが含まれておらず約52px分ナビに埋もれていた。`bottomInset`にpadding分も加算。
5. **スクロール連動モードで競合アイコンが画面右にはみ出す**(2026-08-14、2回の調査を経て根治): 1回目は`.mobile-effect-bar-icon`の中央寄せ(`transform:translateX(-50%) scale(...)`)がWebKitでtranslateXの%指定が効かない不具合を発見し`margin-left:-11px`方式に修正(`4fcaaa67`)したが、**ユーザー実機再テストで「解決していない」と否定**。2回目調査でPlaywright合成再現(実`scroll`イベント+実CD競合データ)を徹底したが右はみ出しは再現できず一旦保留、**ユーザー提供の実機スクリーンショット**(競合アイコンの切れ端が画面端で物理的に切れている)から真因を特定: 競合中に付与する`.animate-conflict-pulse`(黄色パルス)がCSSアニメーションとして`opacity`/`transform`を丸ごと上書きし、本来「静止時はopacity:0で完全に隠す」はずの競合アイコンだけ透明度0にならず変身前の待機位置(画面右端そば)にうっすら常時居座っていた。競合中のみアイコンを`.mobile-effect-bar-icon-slot`ラッパーで包み、脈動アニメ(ラッパー側)と進み具合連動のopacity/scale(アイコン側)を別要素に分離、掛け算合成で静止時は完全不可視に戻るよう修正。ユーザー実機で最終確認OK。

「置いた直後は片方しか光らない」問い合わせはPC版も同じ仕様(直前配置分だけ除外・リロードで両方光る)と実測確認、コード変更なし。全てPlaywright実機相当+ユーザー実機iPhoneで確認済み、tsc・resourceTracker系vitest計50件確認済み。

**②競合(CDかぶり)表示フェーズは完全に完了**。次は別タスクへ。

### ✅ 2026-08-13〜14 モバイル軽減表レイアウト刷新(①レイアウト刷新フェーズ) = 実機確認完了・ローカルコミット済(push・デプロイは翌セッション)

発端: モバイル版でCDかぶり競合が画面に一切表示されない不具合。着手順を「①レイアウト刷新 → ②競合表示」に確定し、`npm run dev -- --host`でLAN公開・ユーザー実機iPhoneで1件ずつ指示→実装→確認のループで進行。①は本セッションで完了・ユーザー最終確認OK。

**2行構成レイアウト化**: `MobileTimelineRow.tsx`で行を「時間+攻撃名+ダメージ」+「軽減アイコン専用行」に分割。

**攻撃名が見えなくなる不具合**: ダメージ表記のフル桁化で攻撃名が潰れる問題。初回実装(3行化・pixelsPerSecond 60→72)は「行の高さは変えるな」指示に反していたため撤回。最終形=1行目(時間+攻撃名)そのまま、右端に「元ダメージ→(極小9px)」上段・「軽減率+軽減後ダメージ(通常サイズ)」下段の2段スタック、行高60pxは不変。`mobileEffectBar.ts`の`MOBILE_EFFECT_BAR_ICON_ROW_OFFSET`は実測値38pxに更新。行のダメージ表記は「元ダメージ→軽減率→軽減後ダメージ」(軽減後が常に最右端)に確定、元ダメージはカンマ区切りフル桁表示化(k/M省略の`formatDmg`は廃止・削除)。

**スクロール連動の変身アニメ**: 累積スクロール距離を`MOBILE_EFFECT_BAR_MORPH_DISTANCE_PX`(40→60pxに調整)で正規化した進み具合`--mobile-effect-bar-progress`をCSS変数で更新、棒の伸び(scaleY)・アイコン縮小・横スライドをtransformベースで計算。「戻る時に無アニメ」バグはPlaywright+WebKitで実store/DOMに注入し実測して根治(scroll中は高速追従80ms・戻り時400msに分離)。初速が速いほどゆっくり見せる`--mobile-effect-bar-transition-ms`(80-400ms可変)も追加。イージングは`linear`→`cubic-bezier(0.2, 0, 0, 1)`(MD3 emphasized-decelerate)に変更し「なめらかでない」FBを解消。エフェクト棒の帯とアイコンの濃さを分離(帯0.55/アイコン0.9)。

**3モードスイッチ化**: `mobileEffectBarAnimationEnabled: boolean`(アイコン表示⇄スクロール演出の2択)を`mobileEffectBarMode: 'icon'|'scroll'|'bar'`の3択に拡張(アイコン表示/スクロール演出/エフェクト常時)。FABの操作方式を順送りタップ→言語ボタンと同じfan-out式3択チップに変更(`fanChipX`/`fanChipVariants`として言語チップのロジックを汎用化・再利用)。**ついでに言語チップの実バグも発見・修正**: メインFABボタン(丸の開閉トグル)でメニューを閉じてもチップ用stateがリセットされず選択するまで残り続けていた不具合を`closeAllChips()`新設で根治、言語⇄エフェクトの同時オープンも排他化。**実機FB2件を追加修正**: (1)チップとラベル文字の重なり→`fanChipX`がラベル幅を50px固定と誤仮定していたのが原因、ラベル実測幅(`getBoundingClientRect`)を都度渡すよう修正。(2)エフェクト常時モードでスクロールしてもFAB/Undo-Redoが消えない→「新規スクロール開始」判定が常時モードで恒久的に'1'固定の`data-mobile-scrolling`属性を見ていたのが原因、属性から独立した`mobileScrollSessionActiveRef`で実スクロール開始を追跡するよう分離。

**軽減アイコンのあふれ対策**: `flex-wrap`による2段折り返しでコマからはみ出す不具合。ユーザー判断=**2段は絶対にしない**方針。`sheetWidth`から最大アイコン数を算出(`MOBILE_MITI_ICON_PITCH_PX`=28px保守的見積り)。見せ方は4案を実機FBで往復し最終形に収束=2行目に「+N」バッジ、タップで1行目のダメージ表記エリアがそのままあふれた分のアイコン表示に切り替わる(カード/ポップアップ装飾なし)。`groupAndCapMitigations`共通関数でバッジ数・あふれ分を一致させる。長押し時にiOS標準「範囲選択」ジェスチャーが競合する不具合も根治(`select-none [-webkit-touch-callout:none]`、`.touch-none`は縦スクロールを殺すため不使用)。「+N」バッジの当たり判定を22px→40x36pxへ拡張。軽減アイコン(2行目)が下端罫線に付きすぎるFBも`pt-[7px]`→`pt-[4px] pb-[3px]`再配分で解消(行高60px不変)。

**その他**: ボトムシートのドラッグハンドル当たり判定44→64px拡張(`MOBILE_TOKENS.touchTarget.min`)/ボトムシート系(メニュー・軽減追加・イベント編集)のナビ隙間統一・共通下スワイプ閉じ(`useSwipeToDismiss`)/FAB配置をMaterial Designのスクロール中非表示パターンに統一、framer-motionとCSSのtransform競合によるアニメぎこちなさも根治/軽減アイコン専用行の横スクロール化は縦スクロールと競合し実機NGで撤回(`flex-wrap`に復元、あふれ対策は上記「+N」方式で別解決済み)。

**次セッション最優先**: ローカルコミット済・未push。push前ゲート(build+全テスト)→push→デプロイ→本番実機確認。完了後は②競合(CDかぶり)表示に着手(`conflictingIds`はPC同様リング/バッジ想定・検出ロジックは既存流用可)。
