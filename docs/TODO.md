...
**🔴最優先: M1「ツアー中(Nav)ページ」= 実機ゲート実施済→要対応あり・merge保留 (2026-07-04)**。ブランチ=`feat/housing-tour-nav-m1`(**未push・実機ゲート前**)@`9d7ce03a`。**Task1〜9完了・全review✅・最終ブランチレビュー(opus)完了**(凡例↔ルート色不一致=Important 1件を修正済・triage 5件裁定済)。`npm run build` EXIT0 / 全体`npm test`=既知legacy5failのみ(新規ゼロ・parity緑)。`/housing/tour` が ComingSoon から実ページ化。**実機ゲート実施済(2026-07-04・Claude・store注入で全状態を1489×2.58目視)**=①地図/②前後連動/③報告モーダル/④完了→戻る/⑤空状態/⑥混在map_pending は全部表示動作OK・凡例↔ルート色一致(honey)も実機確認。**ただし要対応あり=merge保留(下記a〜e)**。spec/plan=`docs/superpowers/{specs,plans}/2026-07-04-housing-tour-nav-page*`、台帳=`.superpowers/sdd/progress.md`(✅完了状態+ゲート手順)、議論=`docs/.private/2026-07-01-housing-tour-rebuild.md`。
  ...
  - **▶本物のナビ化: P1完了 + P2+P4統合完了(2026-07-04・全10タスク・全review✅・要視覚承認/実機ゲート)**: spec=`...specs/2026-07-04-housing-tour-real-navigation-design.md`、plan=`...plans/2026-07-04-housing-tour-nav-p2.md`、台帳=`.superpowers/sdd/progress.md`(P2セクション)。**P1(言葉ナビ本体)**=右パネルに最寄りエーテライト実名+言葉ナビ(300件`wardDirections.generated.json`)。**P2+P4(commit a49edc37→c3df3a11・未push)**=①`orderTourStops`自動並べ替え(region→DC→server→area→ward→plot)を探す/お気に入り両入口に配線 ②中央地図を全5エリア対応(`resolveWardMapRef`+`WARD_MAP_LOADERS`遅延ロード・Mistハードコード撤去) ③**実エーテライト起点データ生成**(元SVG`docs/housing-maps-src/`からシャード座標抽出→`wardAetherytes.generated.json`・全300区画・本街/拡張クロス0を自動テスト保証・`getPlotOriginNode`) ④**どの家でも本物の最寄りエーテライトから家までゴージャス経路アニメ**(glow下地+core+comet+起点pulse+目的地beacon)。**レビューで座標バグ1件(SVGパスH/V/C無視で69%破損)を発見→修正済(正しいpathPointsトークナイザ+基準座標テスト)**。`npm run build` EXIT0 / `npx vitest run`=5 fail(既知legacy TopBar4+HousingWorkspace1のみ・**新規ゼロ・parity緑**)。**残=視覚ゴージャス承認(実機1489×2.58スクショ)+最終ブランチレビュー→merge**。**Goblet拡張街(31-60)は言葉ナビ本文空**(スプシ未記入・テレポ先のみ・masaya記入待ち)。**次フェーズ=P3(節目の移動ナビ:区/エリア/ワールド/DCのtransitionBetween画面)+小物b-e**。
