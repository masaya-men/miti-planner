# LoPo 開発ToDo

> 完了済みタスクは [TODO_COMPLETED.md](./TODO_COMPLETED.md) に移動済み
> 確定した設計方針は [DESIGN_DECISIONS.md](./DESIGN_DECISIONS.md) を参照
> 管理者手順は [ADMIN_SETUP.md](./ADMIN_SETUP.md) を参照

---

## 現在の状態（次セッションはここから読む）

- **ブランチ**: main直接
- **今回の作業**: チュートリアル根本修正 + FFLogs精度改善 + スマホUI大幅改善 + ノッチ対応
- **デプロイ済み**: 33f60b2（暫定修正）
- **デプロイ済み**: 4b6b00f（チュートリアル根本修正+スマホUI大幅改善）
- **未push**: myMemberIdスナップショット共有（87d85b4）
- **同期設計**: 5分クールダウン(自動のみ)、初回editは即push、タブ切替/離脱/手動は即push、競合時は両版コピー保存
- **注意**: ENFORCE_APP_CHECK=true、Vercel関数7/12、Vercel月100ビルド制限
- **既知の制限**: FFLogs翻訳はキルログのみ対応、中韓は手動入力
- **デバッグログ**: 除去済み

---

## 完了（未デプロイ）: チュートリアルによるプランデータ消失の根本修正

3点セット実装済み:
1. ログイン後のプラン自動読み込み廃止（Layout.tsx）
2. スナップショットをsessionStorageに保存（useTutorialStore.ts）
3. チュートリアル中のlocalStorage永続化停止（useMitigationStore.ts）

---

## 進行中: FFLogsダメージ精度改善

**目的**: 新コンテンツ実装直後（スプシ未整備）にFFLogsから実用的な精度のタイムラインを自動生成する
**既存零式・絶**: スプシデータをテンプレートに取り込む（FFLogsより正確）
**参照データ**: `docs/reference-m3s-damage.tsv`（有志スプシM3Sタイムライン）
**テストログ**: `Mypg2dzkfJHj3wPQ` fight=4

### 今回完了した改善
- [x] cast（即発動）イベントの取得（begincastのみ→begincast+cast 2パス方式）
- [x] AoEダメージを中央値方式に変更（max→median）
- [x] AoE中央値からタンクを除外（防御力差による低ダメージ排除）
- [x] 同名技のダメージ統一（中央値から20%以内なら統一）
- [x] AoE波のパケット分離マージ（タンク先行到着問題）
- [x] 両タンク被弾をTB扱い（ウェポンバスター等）
- [x] playerDetailsレスポンスのdata.playerDetailsネスト対応
- [x] 未ログイン時FFLogsモーダルのz-index修正（createPortal化）
- [x] ログインボタンでLoginModalが開かない問題修正

### 残課題
- [x] ダメージ値がスプシより全体的に低い → 多数派multiplier+max+5%バッファに改善
- [x] デバッグログの除去
- [x] TB同名技のダメージ未統一 → MT/STを同一グループにして最大値で統一
- [ ] DSRフェーズ: フェーズ区切り・ボス名が正しくない（テンプレート対応予定、一旦無視）
- [ ] 英語ログ警告: 未確認
- [x] `POST /api/template?action=auto-register 400` → name型チェックがLocalizedString非対応だった

## バグ・不具合（要修正）

### 高（ユーザー体験に直接影響）
- [x] スプシからダメージが取り込めない → TSV自動検出・ヘッダー多言語対応・ダメージカンマ除去で修正
- [x] 初回ログイン画面（WelcomeSetup）にキャンセル手段がない → ×ボタン+キャンセルリンク追加（ログアウトして戻る）
- [x] logsインポートがおかしい（複数件マージされている？）→ V5.0キャスト起点に全面書き換え
- [x] 「保存されない場合があります」→ saveSilently後は不要なので削除

### 中（特定環境・管理者向け）
- [ ] テンプレート手動登録の反映が大幅に遅延する（タイミング不明）
- [x] スマホ版: リキャスト違反時に警告が出ない → MobileMitiIconsに競合ハイライト追加
- [x] 言語切替: zh/ko選択時にコンテンツ種類ボタン等でJPが黒塗りになるバグ
- [x] PWAノッチ対応 → MobileHeaderにsafe-area-inset-top追加（bottom既対応）
- [ ] ラベル名が管理画面で取得できない（スプシヘッダー問題？）

### 低（動作影響なし・エッジケース）
- [ ] LP: THREE.Clock非推奨警告 — THREE.Timerに移行
- [ ] FFLogsインポート: 英語主言語ログで言語取得できない
- [ ] FFLogsインポート: 無敵/リビングデッド中ダメージの反映
- [ ] オートプラン: 無敵はなるべく同じ技に使いたい
- [ ] パルス設定: カスタムカラーのスライダー初期位置が端に寄る（軽微）

---

## 未着手（次にやること）

### 多言語
- [ ] コンテンツ名・軽減モーダル等のデータ系zh/ko対応（LocalizedString拡張）
- [ ] 管理画面: zh/ko列・ギミック列追加
- [ ] コンテンツ種類ボタン・コンテンツ名の中韓翻訳
- [ ] ランディングページのLangToggle（2言語→4言語対応）
- [ ] ハウジングツアーページの言語対応

### 管理画面改善
- [ ] スキル管理画面の実装（管理画面でスキル追加・編集ができない）
- [ ] AA一括対象指定（MT/ST等を一括で設定。現在は1つずつで手間大）
- [ ] 技名ソート・フィルタ（AAのみ表示→一括指定等）
- [ ] AA名統一: 英語も"AA"に変更（中韓も同様）

### 保存・同期
- [ ] 保存の確実性向上（終了時「保存しますか？」確認）

### その他
- [ ] 本番動作確認（ギミックグループ・フェーズ編集・翻訳伝播・ダメージインポート）
- [ ] shared_plansクリーンアップ（アカウント削除時logoBase64残留）
- [ ] CSP unsafe-inline除去（β後、reCAPTCHA/Firebase Auth依存）
- [ ] テスト基盤（planService.ts等の純粋関数から）
- [ ] エラー監視（Sentry無料枠 or Discord Webhook）
- [ ] ヒールスキル追加（テトラ、ディグニティ等のoGCDヒール）
- [ ] スマホ対応追加改善（モーダル最適化、タブレット）
- [ ] セキュリティ: 認証方式のプライバシー調査（メアド保存範囲・Anonymous認証検討）
- [ ] セキュリティ: localStorage認証トークン / Google Fonts SRI / Firestoreパス検証

## 未着手（将来）

### 新機能: Floating Timeline (PiP)
- Document Picture-in-Picture APIで別窓タイムライン表示
- 自分のジョブだけの軽減を簡潔表示、フルパーティ表示も選択可能
- 透過対応でゲーム画面に重ねて表示

### FFLogsインポート精度向上
- 敵の全攻撃データを取得→自動判定の精度を上げる
- テンプレート昇格: 完璧なタイムラインを管理者承認でテンプレート化
- FFLogs API制限解除申請（Public化後に検討）

### その他
- [ ] 古いプランの自動アーカイブ（30件超過時）
- [ ] SA法オートプランナー改善
- [ ] 詠唱バー注釈機能
- [ ] AI APIでオートプラン
- [ ] ハウジングツアープランナー（要件定義済み、Pretext採用決定）
- [ ] public/icons/ 削除（バンドル2.1MB削減）
- [ ] チートシートモード検討

## アイデア・やりたいこと
- ~~敵攻撃セルにホバーで攻撃名ツールチップ表示（省略時のみ）~~ → 実装済み
- 新規作成でコンテンツ選択時、テンプレタイムラインを入れずコンテンツ名だけにして空の表にする。空の表はデフォルト展開
- スキル互換性配置のUI、ログインで使うような透き通った青に変更
- フォーカスモード左右対称化
- YouTube埋め込み（LP・ハウジングツアー・軽減表内に解説動画への導線）
- こだわりのトップページ（AIデザインNG）
- 軽減配置時のフィードバックアニメーション
- UI全般の温度感・アニメーション改善
- オートプラン精度改善（スプシ教師データ・スコアリングモデル）
- YouTube導線: ジョブごとにスキル回し動画URL設定→軽減表にアイコン表示
- ストラテジーボード連携（保留）

## バックログ（運用・品質・検討中）
- [ ] 運用: npm audit定期確認 / a11y / SE利用規約 / GDPR / SEO
- [ ] 検討中: FFLogsアイコン / チートシートMTST分け / フェーズスペース / テンプレ日本語名 / みんなの軽減表 / 軽減モーダルサイズ

## プロジェクト方針
### SNS Build in Public
- 進捗時にJP+ENツイート案を提案（ツリー形式、"Translated by AI" 付記）
- #LoPo #FF14 #BuildInPublic #AISelection
