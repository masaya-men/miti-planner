...
- **🆕📐 plot→size 表と住所抽出v2** (詳細=`docs/.private/2026-07-10-plot-size-table-and-address-v2.md`)
  ...
  - **✅ ゴブレット拡張街 (plot 31-60) の行き方 30件を追加** (ユーザー作成のスプシから取込)。これで全300区画に行き方が入った。**⚠ スプシ側は未修正**: Mist plot30/60・Shirogane plot8/38 が「Ｌハウス」のまま / LavenderBeds plot14・44 の最寄りエーテライト名がリポと違う。**次にスプシから一括エクスポートすると巻き戻る**。
