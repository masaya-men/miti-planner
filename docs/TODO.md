...
- **段取り⑤(③の後)=共同編集の実入口 [設計書化済 2026-06-05・⑤を3分割]=[specs/2026-06-05-...stage5-collab-entry-design.md](./superpowers/specs/2026-06-05-realtime-collab-stage5-collab-entry-design.md)**: 核心=共有はコピーで別ID化([MitigationSheet.tsx:209](./../src/components/MitigationSheet.tsx#L209))→現状plan ID部屋鍵では繋がれない。解消=**`collabRooms/{roomToken}`対応表(token→planId)**。確定方針=常設リンク(案A・無期限)/**オーナーのみ失効・再発行・配布**/最大人数(既定8=フルパーティ)/緊急停止/**編集ログイン必須(段階導入・公開条件にサーバ認証含む)**/ジョイナーは一時ビュー(自分の一覧に増えない)/注意=初回モーダル+常時赤バナー/パスワード無し。コピー共有(ShareModal)無傷。**完成までUI一切非表示厳守**。順序: ③→⑤→②-b→②-c。
  ...
  - **⑤-3を4分割(2026-06-08ブレスト・設計書[specs/...stage5-3a...md](./superpowers/specs/2026-06-08-realtime-collab-stage5-3a-owner-entry-design.md))**: 3a=オーナー入口 / 3b=ジョイナー一時ビュー(§8最難関) / 3c=注意UI+ログインゲート / 3d=実データ往復E2E。
  - **✅⑤-3a=オーナー入口 実装済(2026-06-08・ブランチ`feat/collab-stage5-3a-owner-entry`)=[plans/...stage5-3a...md](./superpowers/plans/2026-06-08-realtime-collab-stage5-3a-owner-entry.md)**: 共有2択(B案コピー/一緒に編集)+ルーム発行/人数/失効/再発行(`collabRoomApi`+`useCollabSessionStore`)+表ツールバー常設チップ(業界水準)+`startCollabSession`roomToken化+オーナーパネル(確定文言・i18n4言語)。**SYSTEM_MAX 28→20**(総参加上限・既定8)。サーバは既存で結線のみ。root1445緑/worker24緑/build緑。**⑤-3aは初のUI露出段→案ア:3a〜3cをブランチに溜め、⑤-3完成+ユーザー承認まで main マージ&push&worker deploy 保留(UI非露出厳守)**。
  - **次=⑤-3b**(ジョイナー一時ビュー): `/collab/:roomToken`ルート+SavedPlan非紐付けの一時ワークスペース+退室クリア。その後3c(注意UI+ログインゲート)→3d(実データ往復)。
