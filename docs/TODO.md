...
- **🚧 スマホ最適化 進行中 (2026-06-15・全てローカル commit 済・未push・デプロイ未)**: 設計メモ=[specs/2026-06-15-mobile-optimization-design.md](./superpowers/specs/2026-06-15-mobile-optimization-design.md)。**済(コミット済)**=①スマホ対象選択バグ(`resolveMitigationTap`・実機OK) ②チュートリアルFキータブレット詰み(キーキャップtap可・実機OK) ③C共同編集参加ヘッダーを本物MobileHeader統一(二重余白解消・**本番確認はデプロイ後**) ④D FAB背景2倍濃く(実機OK)。**次セッション最優先=A ボトムナビ再設計の実装**(spec 参照)=新ナビ5タブ `メニュー/インポート/カンペ/共有/ログイン`(パーティ・MYJOBタブ廃止→メニュー集約 / ツール→インポート改名・Undo/Redoは FAB近く常設・自動組み立てはメニューへ / カンペFAB昇格 / 文言PC一致=共通i18nキー / 支援をメニューで埋もれさせない[あ案])。**その後=B 共有配布フロー設計(別ブレスト)→ A+B 完了でまとめて push/デプロイ**。`writing-plans` から着手推奨。
