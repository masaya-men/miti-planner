...
0. 🏠 **ハウジング公開前 残タスク**(網羅=`docs/.private/2026-07-15-housing-release-remaining-tasks.md`):
   - **✅スマホ対応=実機FB5弾まで反映し本番デプロイ(2026-07-17)**: 基盤(ナビ/FAB/シート/2列/全画面/ツアー横持ち→縦OK化)+FB第2〜5弾(トップ/フィルター▼ヘッダー/シート不透明+つまみ閉じ/行き方地図下部/終了/住所2行/次へ消失根治=sticky hover/トレイバー/カードをナビ下まで/スクロールバー右端/お気に入り2行化+文字ボタン)。#A削除即反映/#Bお気に入り件数重複も修正。PC無変更。**本番確認=全部OK(2026-07-17)**。台帳=.superpowers/sdd/progress.md。✅①#B残ストリップ件数(`d6261a9d`) ②LP導線(`0aec3971`) ③中韓用語CSV(`docs/.private/2026-07-17-housing-terms-ja-en-ko-zh.csv`+デスクトップ)=2026-07-17完了→TODO_COMPLETED。**🔴次**: ④お気に入りPC⇔スマホ非同期=根因確定([useHousingFavoritesStore.ts:26] zustand persist→localStorageのみ・サーバー同期なし)→同期設計はbrainstormingから。**FB第6弾+第7弾=全部実装・push済(2026-07-17・実機確認待ち)**: 同期✅/ツアー順✅(第7弾でピン=「この位置に固定」1個に刷新+行サムネ/タイトル追加) / Xシェア=アニメアイコン(ボタン全体hover発火)+#LoPo+追跡クエリ除去+詳細は本文なし(ハウジンガーは表示名維持) / ハウジンガーOGP=**実装正常を本番curlで実証**・黒ロゴはX側URL単位キャッシュ(確認は`?v=2`付きで貼る)・フォールバック画像の相対URLバグ修正済。詳細=TODO_COMPLETED。
