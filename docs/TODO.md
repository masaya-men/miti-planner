...
- **✅ 2026-06-16 追加修正(同ブランチ・push/deploy保留・コミット済)**:
  ...
  - **🔴 次セッション最優先=B共有タブの「共同編集2択」未表示の調査(ユーザー報告 2026-06-16)**: ユーザー実機で共有タブが「共同編集込み(コピー/一緒に編集の2択)」になっていなかった。**判定ロジック**=[useShareFlow](../src/components/collab/useShareFlow.tsx)→`resolveInitialShareView({user,isOn})`([shareView.ts](../src/lib/collab/shareView.ts)): 未ログイン→`copy`直行(collabはログイン必須なので2択出さない設計)/ ログイン+OFF→`choice`(2択)/ ログイン+ON→`panel`。**仮説=確認時に未ログインだった可能性大**(=copy直行は設計通り・PCと同挙動)。**次セッションで切り分け**: ①スマホで**ログインした状態**で共有タブ→ShareChoiceModal(2択)が出るか②出ないならバグ(MobileShareController の openShareUI 発火 / user 取得 / useShareFlow 配線を実機で追う)。私の前回実機確認は**未ログイン**で copy 起動を見ただけ=ログイン経路は未検証。**この決着まで push/デプロイ保留**。
  - **🚀 上記Bの決着後=本番反映**: `rtk git push origin HEAD:main` → Vercel自動デプロイ → seed 3手順(seed-icons / seed-skills-stats ADDITIVE / seed-charge-min-level --commit)→ 本番でスマホ総点検(共有タブ/パーティ・自動ボタン/ガイド文言/Lv80でDB・星天が1チャージ/深謀遠慮表示)。
