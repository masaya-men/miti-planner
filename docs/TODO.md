...
1. ✅メモURLリンク化=本番済(spec/plan=`2026-06-29-memo-url-linkify*`) 2. ✅PS5リモプ貼り付けアシスト(/stgy)=**本番デプロイ済**(実機OK・PR#2マージ・spec/plan=`2026-06-29-ps5-strategy-board-paste-assist*`) 3. **【次】スプシ取込スマホ本格対応**(私のスマホ実機クリップボード採取が起点) 4. **軽減編集タイムラプスのSNS投稿**(大物・要brainstorming)
...
- **🆕 スプシ取込のスマホ本格対応**(2026-06-26 ユーザーFB・現状は近日公開メッセージで蓋・`3dd4f10b`): **アプリ=Googleスプレッドシート アプリ(2026-06-29確定)**。**🔑実機採取で形式確定(2026-06-29・`/clip`=`ClipboardInspectorPage.tsx`)**: スマホもPCと同じ**TSV(text/plain にタブ3480個/87行)** + 完全な `text/html`(`<table>`,`<tr>`87,`<td>`3427)を返す。**当初仮説「コピー形式が違うから取れない」は外れ→データはスマホでも取得可能・パーサ(`parseGridPaste.ts`)は基本流用可(要検証)**。よって残作業は**スマホ用の貼付取込UI(機能)を作ること**(形式問題ではない)。**🔑原因確定(2026-06-29実機・端末=iPhone/Safari)**: 既存スマホ取込が頼る `navigator.clipboard.readText()`(`SpreadsheetGridImportModal.tsx:302`)はこの端末で **`NotAllowedError` でブロック**=取れない真因。一方 `/clip` の**本物の`<textarea>`+onPaste 経路はフルTSV取得成功**。→**解決方向=API読取を捨て `<textarea>`貼付方式へ**(divのonPaste不可→readText の経緯を、textareaなら回避できる・実証済)。パーサ流用可。次=brainstorming。`/clip` は役目終了後ルートごと削除可 ②フェーズごと貼付が伝わらず未貼付のまま誤作成→スマホ専用ウィザードUX(手順明示/未貼付ガード) ③全選択コピー手順が難しい(アプリ外)→図解で緩和。z-index/dvh/貼付ボタンは対応済(PCのモーダルは動作)。
