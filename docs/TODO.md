...
- **リアルタイム共同編集 (✅ブレスト完了→設計書あり→次は実装計画)**: 1軽減表を複数人同時編集。方式確定=**Yjs(CRDT)+Cloudflare Durable Objects(PartyKit)**、カーソル=P2Pで$0、保存=Firestoreへ間引き書き戻し。**Workers無料プラン$0ハードストップ前提でコスト有界化**(編集8席/閲覧20/同時30/緊急停止)。設計書=[docs/superpowers/specs/2026-06-03-realtime-collab-design.md](./superpowers/specs/2026-06-03-realtime-collab-design.md)。前提メモ=[.private/2026-06-03-realtime-collab-and-sync-notes.md](./.private/2026-06-03-realtime-collab-and-sync-notes.md)。次=writing-plansで実装計画化
