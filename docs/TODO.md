...
- **🟢 現在の作業スレッド=リアルタイム共同編集 (branch `feat/collab-stage4b1-presence-roster`・held・push/deploy保留・UI非露出)**: ④-b 設計完了 + **④-b-1(presence roster=WS awareness)実装完了 (2026-06-10)**。**次セッションはここ→④-b-2(P2Pカーソル・WebRTC・signaling/lib選定の spike から)を設計→実装**。その後 ⑤-3d+④ 統合プレビュー実機検証 (login要・localhost auth不可)。詳細は下記 collab バックログ + memory `project_realtime_collab_status`。
