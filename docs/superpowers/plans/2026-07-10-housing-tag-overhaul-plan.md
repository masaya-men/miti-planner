...
## Phase C: 初心者タグ「ハウジング若葉」(2026-07-21 追加決定)

実機フィードバックで「初心者でも登録しやすくする」ための自己申告タグが欲しいという要望が出た。
Phase A で作った kind レジストリ拡張の仕組み (「レジストリに 1 エントリ足す + ロケール追加」だけで
タブに現れる) がそのまま使えるため、新規設計はせずここに追記する。

- 新 kind: `'beginner'` (1 タグのみ、公式/季節/テーマと同じ「選べば付く」形。個人タグとは別枠)。
- タグ文言: **「ハウジング若葉」** (1 つのみ、A/B 案のうち A で決定)。
- 4 言語: これは FF14 公式用語ではなく LoPo 独自のラベルなので、公式タグのような一次ソース照合は
  不要。EN/KO/ZH は自然な意訳で提案し、ユーザー確認を挟む (「若葉」の初心者マーク的ニュアンスは
  直訳ではなく意味を汲んだ訳にする)。
- 実装は Phase A の registry 拡張パターン (`HOUSING_TAG_KINDS` に `'beginner'` 追加 + タグ定義 1 件 +
  4 言語ロケールキー追加) をそのまま踏襲する。API 側の validation (許可 kind リスト) も追従が必要。
