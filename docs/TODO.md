...
- **🚧 スマホ最適化 A(ボトムナビ再設計)実装中 (2026-06-15・ブランチ `feat/mobile-bottom-nav-redesign`・全コミット済・未push・デプロイ未)**: 設計メモ=[specs/2026-06-15-mobile-optimization-design.md](./superpowers/specs/2026-06-15-mobile-optimization-design.md) / **実装計画=[plans/2026-06-15-mobile-bottom-nav-redesign.md](./superpowers/plans/2026-06-15-mobile-bottom-nav-redesign.md)**(全9タスク・サブエージェント駆動)。前段の個別修正①〜④(対象選択バグ/チュートリアルFキー/C参加ヘッダー/D FAB背景)は別コミットで済(③本番確認はデプロイ後)。
  ...
  - **🔴 未解決=⑤ メニューの☕支援が下端固定で見えない**: MobileBottomSheet の中身は `flex-1 overflow-y-auto pb-20`([MobileBottomSheet.tsx:118](../src/components/MobileBottomSheet.tsx#L118))。Sidebar ルートは3層 h-full([Sidebar.tsx:1234-1248](../src/components/Sidebar.tsx#L1234))だが、メニュー側ラッパー([Layout.tsx:618](../src/components/Layout.tsx#L618))に高さが無く h-full 不解決→シート全体がスクロールし支援が下に流れる。**試した修正=ラッパーに `h-full` 追加(コミット 23c60fa)→ 実機で依然見えず=不十分**(% height が flex-1 スクロール親で解決していない or pb-20 が押し出し)。**次の手の候補**: ①支援を fullWidth 限定で `position: sticky; bottom:0`+背景(sheet-bg)にして pb-20 のギャップを `-mb` 等で詰める ②MobileBottomSheet に「中身を自前で高さ管理」する prop を足し pb-20 を外す ③メニューだけ専用シート化。**いずれも実機で支援がスクロール無しで見えること必須**。
  - **未着手**: Task8 FABからカンペ項目撤去([MobileFAB.tsx:241](../src/components/MobileFAB.tsx#L241)・カンペはナビ昇格済) / Task9 総合確認(build+vitest+4言語+全チェックポイント)。**その後=B 共有配布フロー設計(別ブレスト)→ A+B 完了でまとめて push/デプロイ**(Vercelビルド節約+C本番確認)。
