...
**🔴 次セッション最優先: 残課題「連動アニメの初動(スクロールし始めた瞬間)だけかくつく」原因未特定・ユーザー指示で一旦区切り**。試した仮説(FABを隠す処理との競合→dispatchEventをsetTimeoutで遅延)は実測で**効果ゼロと判明・差し戻し済み**(コードはクリーンな状態)。ローカル検証手法(Playwright+CDP、`container.scrollTop += 20`をrAFループで機械的に回すスクロール再現)では、スクロール開始2〜4回目のイベントだけ通常の10倍以上(約230ms、影響要素数5,500規模)重い`UpdateLayoutTree`が再現するが、原因を1つ潰しても数値が変わらない。**要注意**: この機械的なスクロール再現が実機の指スクロールの「初動」を正しく模倣できているか自体に疑いが出てきている(データ注入直後のReact設定処理待ちを3秒に延ばしても再現するので、そこは違うと切り分け済み)。次セッションはsystematic-debuggingのPhase1からやり直し、可能なら実機によるより近い再現方法を検討すること。
