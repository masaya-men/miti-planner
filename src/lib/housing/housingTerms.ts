...
const isCnKrTw = (dcKey: string) => { const r = regionForDC(dcKey); return r === 'KR' || r === 'CN' || r === 'TW'; };

/** DC 表示名: KR/CN/TW のみ全ロケール辞書名、グローバルは現状表示 (内部キー=英名) を維持。 */
export function displayDcName(dcKey: string, locale: TermLocale): string {
  return isCnKrTw(dcKey) ? termLabel('dc', dcKey, locale) : dcKey;
}

/** ワールド表示名: 所属 DC が KR/CN/TW のときのみ辞書名。 */
