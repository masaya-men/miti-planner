...
- **🆕 機能ブラッシュアップ案9件**(詳細=docs/.private/2026-06-15-feature-ideas-batch.md): ①同時刻3+イベント ②スマホ/タブレット最適化(残=ボトムナビ/FAB) ③軽減競合逆方向警告(✅実装済) ④MAXHP-10%でダメージ黄 ⑤Logsインポート上書き/追記(✅本番済2026-06-20) ⑥有名スプシ取込(✅実装完了2026-06-21=上記🟣・要実機確認) ⑦敵攻撃 or(2択) ⑧管理画面 攻撃ID保持で任意言語翻訳(GUID保持済=ほぼ実装済・仕上げのみ) ⑨メモに動画URL→iframe。⑥は実装完了(要実機確認)。取り込み導線チューザー統合は将来。**🆕列グリッド取込=§9.7改訂の実装待ち(`feat/spreadsheet-grid-import`・未merge・コードは未着手で前回のまま)。実機FB→ブレスト→spec §9.7確定→実装計画作成まで完了(2026-06-24)。§9.7=右パネル廃止しスプシ面集約(フェーズバー/メンバー列ヘッダーにインライン枠/Ctrl+A導線)・攻撃の対象は行ごとselect+テンプレ競合解消(優先=手動>自作>テンプレ)・matrixの列チップ撤去&読めない技だけセル内黄色(同時刻2技は白)・作成は常時可(未追加は自動取込)・自作は正式名称前提(辞書なし)+空白前処理+ジョブ列手動救済。spec=`docs/superpowers/specs/2026-06-24-spreadsheet-grid-import-design.md`§9.7 / plan=`docs/superpowers/plans/2026-06-24-spreadsheet-grid-import-v97-ux.md`(11タスク)。次=subagent-driven で実装→本人実機(plan Task11)→main merge。**
