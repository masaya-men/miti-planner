...
**Files:**
- Modify: `src/components/housing/pages/TourNavPage.tsx`（`:160-191` の左右 `<section>` の中身を入替）
- Modify: `src/styles/housing.css`（`:173` の直後にツアー専用トークン追加、`:6469-6475` の `.housing-tour-page` グリッド差し替え）

**Interfaces:**
- Consumes: 新トークン `--housing-tour-showcase-w` / `--housing-tour-progress-w`。
- Produces: `.housing-tour-page` のグリッドが「ショーケース幅 1fr 進行状況幅」。`data-region="left"` = ショーケース、`data-region="right"` = 進行状況。
...
- [ ] **Step 2: .housing-tour-page のグリッドをツアー専用トークンへ差し替え**

`src/styles/housing.css` `:6471` を差し替え:
```css
  grid-template-columns: var(--housing-tour-showcase-w) 1fr var(--housing-tour-progress-w);
```

- [ ] **Step 3: TourNavPage の左右 section の中身を入替**

`src/components/housing/pages/TourNavPage.tsx` `:160-191` の `return` 内、左右 2 つの `<section>` を次へ差し替え（中央 `<section data-region="center">` は不変）。**左＝ショーケース / 右＝進行状況**:
```tsx
      <section className="housing-tour-page-panel" data-region="left">
        <div className="housing-tour-page-col">
          <TourShowcasePanel
