...
0. **🔴 次セッション最優先**: 軽減表ブラッシュアップ残り + 新発見1件(調査済み・下記参照)。
   - 0-1. 🆕(要brainstorming) PC: Timelineのエフェクト棒にホバーで、LoPo流の綺麗なツールチップでスキルアイコン表示。「どのエフェクト棒がどのスキルか把握しづらい」との声。現状のホバー挙動を要確認。
   - 0-3. 🐛✅実装済 スマホ軽減表: FABメニューのメンバー表示切替がタップでスムーズにオンオフできず「アイコンが動こうとする感じ」になる不具合。**原因特定**: このチップ(`MobileFab.tsx`)はframer-motionがx/y/scaleのtransformを継続制御しているのに、CSS `active:scale-90` も同じtransformを取り合っていた。同一ファイルのメインFABボタンで2026-08-13に全く同じ原因(CSS `:active` とframer-motionのtransform競合)を特定・`whileTap`に一本化して解決済みだったが、2026-08-19に追加されたこのチップだけ同じ直しが漏れていた。同じ直し方を適用済み・実機確認待ち。
   - 0-4. 🐛✅実装済 スマホ: メモがスマホでも表示されてしまう不具合。原因=`MemoOverlay`(`Timeline.tsx`)にモバイル判定が無かった。修正済み・実機確認待ち(2026-08-14実機報告①と同一事象)。
   - 0-5. 🐛✅実装済 メンバー表示切替・並び替えが自分の別プラン、さらには無関係な共同編集プランにまで持ち越される不具合。原因=この2つがブラウザ全体で1個だけの設定として保存され、プラン切替/共同編集入室のどちらでもリセットされていなかった(2026-08-19時点は意図的にそういう仕様だった)。**ユーザー判断で決定**: 自分のプランは`PlanData`の一部としてプランごとに独立保存(Firestore書込コストへの影響は無し=既存の書込タイミングに便乗するだけ)、共同編集ルームは入室のたびに必ずリセット(部屋のデータには元々含まれず各参加者のローカル限定=既存設計通りで無理のある作りではないと確認済み)。回帰テスト4件追加、tsc/テスト全1480件通過・実機確認待ち。
     **後回し(スコープ外・要望あれば着手)**: 共同編集の部屋についても、自分が同じ部屋に再入室した時だけ自分の前回設定を覚えておく(部屋のデータには含めずローカルのみ、部屋IDごとの記録が必要)。
   ハウジングの「新着ハウジング自動ツイート下書き通知」(バックログ参照)と、下記1〜3のタイムラプスSNS投稿/スプシモード/Wiki型タイムラインは、いずれもこのブラッシュアップより後回し(さらに先送り確定)。
...
## 現在の状態 (次セッションはここから読む)
### 🔴 2026-08-26 軽減表バリア計算バグ2件は実機確認OK・完了(詳細→TODO_COMPLETED.md)。0-3/0-4/0-5は実装済み・tsc/テスト通過・**masayaさんの実機確認待ち**。
実機確認OKが出たらコミット→push。残るは0-1(ツールチップ、要brainstorming)のみ。
