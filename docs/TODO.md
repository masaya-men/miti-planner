...
- **ブランチ**: main直接
- **デプロイ済み**: テンプレート保護 + エディタ一括編集 + ツールチップ言語対応 + zh/ko表示全対応 + キャッシュ自動無効化 + フェーズ・ラベル編集UI
...
- **確認済み**: テンプレート保護（管理画面保存→lockedAt自動付与→FFLogs自動登録で上書き防止）
- **確認済み**: テンプレートエディタ一括編集（チェックボックス+AAフィルタ+一括変更ポップアップ）
- **確認済み**: ツールチップ言語対応（z-index修正でモーダル上にも表示）
- **確認済み**: 翻訳管理画面（ジョブ名カテゴリ追加済み、実データ登録はこれから）
- **確認済み**: フェーズ名LocalizedString化（contents.json+TemplateData+スクリプト+変換ユーティリティ）
- **確認済み**: テンプレートエディタのフェーズ・ラベルインライン編集UI（4言語ポップオーバー）
- **実装済み未確認**: フェーズ編集の時刻ベース境界追加/削除
- **実装済み未確認**: テンプレートキャッシュ自動無効化（dataVersion変更時にlocalStorageクリア）
- **実装済み未確認**: 全コンポーネントzh/ko表示対応（11ファイル、getPhaseName統一）
- **実装済み未確認**: カテゴリ/レベル/プロジェクトラベルのzh/ko追加
...
### 管理画面改善
- [x] テンプレートエディタ: フェーズ編集・ラベル編集機能（完了）
