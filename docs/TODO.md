...
**未解決2点(次セッション最優先・このファイルではなくコードを触る前に必ず引き継ぎメモを読む)**:
①左カラムのスクロールバー色 = ユーザー指摘「薄い透き通ったグレーのはず」だが採用した `--housing-text-dim`(実値`rgba(255,255,255,0.78)`=白系)は否定された。名前で決めず実機で色を特定すること。
②マイページ一覧カードの公開状態バッジのポップオーバーメニューが**押しても表示されなくなった(4巡目のcreatePortal化で新規発生した退行バグ)**。実機DevToolsで事実確認してから直すこと(推測禁止)。
詳細な経緯・仮説・調査手順 = `docs/.private/2026-07-24-mypage-card-overlay-and-scrollbar-handoff.md`。ユーザーから明示指示: 「セッションを区切って新しいセッションで、文脈を失わずに確実にやること」。
