...
### ✅ 2026-08-18 ハウジング多言語の訳し漏れ・データ欠落を一括解消 = 実装完了・本番反映済み
タスク台帳(Artifact)の棚卸し項目をユーザーと1件ずつ実機/コードで確認する過程で発見。**①housing.*ロケールの未翻訳330件**(英74・韓121・簡体中135、`apartment_building`の号棟表記+バリデーションエラー文含む)をSonnetサブエージェント3体(言語ごと)に並行翻訳させ、既存訳語との統一性を指示、完了後に本体側で日本語残存ゼロ・JSON構文をそれぞれ独立検証。**②スプレッドシート取込プレビュー(`SpreadsheetGridImportModal.tsx`+`gridRowsFromResult.ts`)がzh-Hant非対応**(言語判定の型が4言語のみでzh-Hant判定漏れ→繁体字ユーザーは常に日本語表示)を修正、回帰テスト追加。**③コンテンツ`dmu`(絶妖星乱舞)の韓国語名が欠落**(他コンテンツは全て`ko`あり)、公式ガイド+韓国主要ゲームメディア複数(inven/gametoc/ruliweb)で一致した「절 요성난무」を採用し追加、`seed-contents.ts`でFirestore本番へ反映済み。i18nParity系テスト・tsc・build全通過。**なお②③はTODO.mdに記載の無かった追加発見分**(①の記録の周辺調査で発覚)。
...
2026-07-31分の訳し漏れ3件(housing.*未翻訳330件/スプシ取込プレビューzh-Hant未対応/コンテンツdmuのko欠落)は2026-08-18に全て解消済み(詳細は上の「現在の状態」欄)。
