...
export const CheatSheetView: React.FC = () => {
    ...
    const EventRow = ({ event }: { event: MergedEvent }) => {
        ...
        const inWindowMitigations = timelineMitigations.filter(m =>
            m.time <= (event.time + event.span) && (m.time + m.duration) >= event.time
        );
        // 挑発/エーテルフロー/ドロー系は軽減ではないためカンペのアイコン表示から除外(表示専用・計算には不影響)
        const activeMitigations = filterCheatSheetMitigations(inWindowMitigations, id => MITIGATIONS.find(d => d.id === id));
