...
0. **🔴 次セッション最優先**: 軽減表ブラッシュアップ6件、全て実装・push済み・**masayaさんの実機確認待ち**(詳細=下記「現在の状態」)。
...
## 現在の状態 (次セッションはここから読む)
### 🔴 2026-08-26〜27 軽減表ブラッシュアップ6件、全てpush済み・**masayaさんの実機確認待ち**
バリア計算バグ2件は実機確認OK・完了(詳細→TODO_COMPLETED.md)。残り6件は実機確認待ち:
①PCエフェクト棒ホバーでスキルアイコンだけ浮かぶツールチップ(棒クリックは配置マス目へ転送)/
②スマホFABの表示切替タップ不安定(`active:scale-90`とframer-motionのtransform競合、同一原因を
2026-08-13にメインFABボタンで解決済み・同じ直しを適用)/③スマホでメモが表示されてしまう
(`MemoOverlay`にモバイル判定なし)/④表示切替・並び替えが自分の別プラン/無関係な共同編集ルームに
持ち越される(`PlanData`の一部としてプランごとに独立保存する仕様に変更、共同編集ルームは入室毎に
リセット)/⑤共同編集の参加者ページで表示切替・並び替えがグレーアウト/見せかけボタンだった
(部屋のデータには影響しないローカル設定と確認済みのため解禁)。全てtsc/build/vitest通過。
