...
- **🔴 探す地図フィードバック: ①③④✅ / ②機能=ユーザー実機OK・見た目微調整で1点残** (2026-07-12)。詳細=`docs/.private/2026-07-12-browse-map-feedback.md`。**残1点=大量部屋パネル左上角だけ backdrop-filter のぼかしが四角くはみ出す**(border-radius/overflow/clip-path 全滅・実DOM調査で祖先transform無し確定・詳細と選択肢A/B/Cは台帳末尾)。次セッション: パネル角を要ユーザー相談(A=薄い半透明ソリッドに振る等)→決着後に main 反映→`firebase deploy --only firestore`(PF rules+indexes先)→push→②ダミー削除。
