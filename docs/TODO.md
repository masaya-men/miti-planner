...
0. **🔴 次セッション最優先(実装準備完了・再調査不要)**: 詳細・実装方針・触ってはいけないもの全て=`docs/.private/2026-08-27-viewport-shift-audit-and-barrier-bar-cutoff.md`に確定済み。**①→②の順で着手**。
   - ①**共同編集の参加者画面(`CollabJoinerPage.tsx`)がiOS Safariのスマホ表示ズレ対策を持っていない**(軽減表本体・ハウジングには対策済みだがここだけ穴だった)。全ルート監査済み・穴はここだけと確認済み。`HousingShell.tsx`の改良版ロジックを共有フック化して3箇所(Layout.tsx/HousingShell.tsx/CollabJoinerPage.tsx)に適用。
   - ②**バリア(シールド)が吸収量を使い切ったら、そこでエフェクト棒をPC・スマホ両方で止める**。ダメージ計算内部で既に「いつ壊れたか」を追跡済みのため配線作業。スタック制シールド(ハイマ等)は壊れても続く場合があるので早期終了させない、という注意点あり(ファイル参照)。
...
## 現在の状態 (次セッションはここから読む)
### 🔴 2026-08-27 次にやる①②(上記参照)は調査・設計とも完了、実装のみ残った状態
