...
- **ローカル削除→即同期で復活 潜在バグ** (2026-04-28、 2026-05-16 部分対応): `deletePlan` が `ownerId === 'local'` のとき `_deletedPlanIds` に追加しない。 「同期成功後にローカル `ownerId` を `uid` に書き換え」 部分は対応済 (syncToFirestore / forceSyncAll / manualSync の 3 経路)。 残り deletePlan 側の対応は別途
