...
### ✅ 2026-08-20 ユーザー実機報告3件、全部修正・デプロイ済み
①アパートの住所自動入力で部屋番号が空欄のまま残る不具合(`housingExtractResultToAddressPatch`がroomNumberを一切コピーしていなかった。一時ツアーの住所追加パネルとAllmarksまとめてインポート両方に影響)。②PC版ツアートレイに「空にする」ボタンが無かった(スマホ版には既にあった)ので追加。③物件詳細ページのYouTube動画が再生終了後にYouTube純正の関連動画オーバーレイで隠れる不具合(loop未設定が原因、カードのambient動画では対策済みだった設定を移植)。詳細=`docs/TODO_COMPLETED.md`。
**🟡 既知の制約・対応しない方針で決着**: 探すページのカード上でYouTube動画が自動再生される瞬間・ループで最初に戻るたびに、YouTube側が強制的にタイトル/チャンネル名/再生ボタンを乗せた画面を3〜4秒間表示する(`controls=0`では止められないYouTube埋め込みの仕様。Playwrightで実機再現・検証済み)。根本的に消すにはYouTube公式のJS版プレーヤーAPIへの切り替えが必要(URLパラメータだけでは不可)で作り込みが要るため、**ユーザー判断で今回は着手しない**。Allmarks(別プロジェクト)でも同じ症状が出ており、こちらはLoPo固有のバグではなくYouTube埋め込み共通の制約と判明。
