...
### ✅ 2026-08-13 ハウジンガー公開が本人にも詰む不具合 = 本番push・デプロイ完了(`7749f831`)

ユーザー実機指摘: 「設定する」でアイコン/名前だけ設定 → MY PAGEを開くと「このハウジンガーは公開されていません」で行き止まり、公開ボタンにたどり着けない循環バグだった。原因は`getHousingerProfile`(housingerProfileService.ts)がisPublishedフィルタを本人閲覧時にもかけていたため(Firestore rules自体は`isOwner(uid)`で本人読み取りを許可済みだったが、クライアント側コードがそれを活かせていなかった)。`isSelf`オプション追加でセルフビュー時はフィルタをスキップするよう修正(キャッシュもself/public別キー化)。あわせて物件登録画面の「設定する」ポップアップ(`HousingAccountModal`)に`HousingerProfileSection`(公開ボタン/ひとこと/SNSリンク)を追加し、その場で公開まで完結できるようにした(物件登録フォーム自体は非ブロッキングのまま=spec §4.1の方針維持)。vitest42件確認済み。

### ✅ 2026-08-13 ハウジンガーひとこと欄プレースホルダーをコミカルな例文に変更 = 本番push・デプロイ完了(`233503b4`)

「例: S字改築が好きです。見学歓迎!」→「例: 動線?知らない子ですね。見学歓迎!」(ja.jsonのみ、他言語は未着手)。

### ✅ 2026-08-12 Vercel Fluid Active CPU 75%アラート対応 = 共同編集Firestore反映を間引く・本番デプロイ済

Vercelから「Fluid Active CPU(月4時間の無料枠)75%消費」のメール→ダッシュボード実測(Observability→Functions)で`/api/collab`が全体の75%以上を占めると判明。8人固定で1-2週間使用しただけで無料枠を月換算で超えるペースだった。原因は共同編集の保存が「DOストレージ(Cloudflare側・無料・こまめな保護用)」と「Firestore反映(Vercel経由・有料枠消費・保険用途)」の2層を同じ15秒debounceで一緒に動かしていたため。`workers/collab/src/server.ts`でDOストレージ保存とFirestore反映を分離、後者だけソロモードの定期バックアップと同じ5分間隔に間引く判定を`saveThrottle.ts`(純粋関数)へ抽出。全員退室時は間引かず強制反映(タブを閉じたら必ず保存、の体験は不変)。ライブ同期には一切影響なし。vitest66件(新規6件)。`wrangler deploy`で本番反映済み(Version ID `2c162875`)。**残**: 数日後にVercelダッシュボードで消費量の減り方を確認。

### ✅ 2026-08-12 スマホ背景動画が読み込まれない(リロードで直る)不具合 = 自己修復ロジックで解消・本番デプロイ済

スマホでページを開いた直後、背景動画が1フレームも描画されず下地の背景色(ダークテーマは`#050814`〜`#0a1230`の紺〜紫グラデ)が透けて見える不具合(ユーザー実機指摘)。production実機でCloudflareのRangeリクエスト挙動をcurlで直接検証しCDNキャッシュ破損は否定。`SceneryVideo.tsx`に読み込み失敗/無応答時の自動リトライが一切無かったのが根本原因。`useVideoLoadRecovery`フックを追加し、アクティブな動画が6秒以内に描画開始しなければ`load()`し直す/`error`イベントでも即リトライ(上限1回)。vitest7件(新規3件)。

### ✅ 2026-08-12 スマホ版ツアー招待のログイン導線UX改善 = 本番デプロイ済

スマホ版はPC版の招待カードがCSSで丸ごと非表示になっており、招待の入口(下部バーのアイコン)に説明が一切なかった(実機指摘)。「PC版パネルをそのまま出す」案は地図右下フロートが下部固定バーと重なるため不採用(過去に一度その形で実装→実機FBで統合済みという経緯がコード上に残っていた)。アイコンをログイン状態だけで決める方式に統一(未ログイン=招待/ログイン済み=リンク)、ログイン済み・未発行時のタップは発行→自動コピーまで1タップに統合、地図下部の行き方帯に案内文を常時表示。5言語i18n追加。vitest71件。

### ✅ 2026-08-12 大型push完了(80コミット・`e27b4429..084a885f`) = 本番push・Vercelデプロイ完了

ツアー計画画面(蛇行グリッド)/ツアー追加フィードバック演出/ハウジングツアー不具合修正一式/小粒ブラッシュアップ4件/セラフィズム自動変化+展開戦術追従修正/タイムライン25分延長、まとめて本番push・デプロイ完了。詳細は本ファイル内の個別エントリ参照。

### ✅ 2026-08-12 未ログイン時ツアー招待案内の文言修正 = 本番push・デプロイ完了(`f076ed79`)

「ツアー作成にはログインが必要です」は、ログインが必要なのは招待リンクの発行だけでツアー自体(閲覧・利用)にはログイン不要という実態とズレていた(ユーザー実機指摘)。タイトルを「ツアーへの招待にはログインが必要です」に修正(5言語)。lead文は元々「招待の発行」を正しく指していたため変更なし。この確認の過程で、スマホ版はこの案内カード自体がCSSで丸ごと非表示になっていることが判明(→現在の状態セクションで別途相談中)。

### ✅ 2026-08-12 ハウジング小粒ブラッシュアップ4件+タイムライン25分延長 = 本番push・デプロイ完了(`084a885f`)
...
③はじめの登録チケット残数表示の下に、使い切った後1日何件登録できるかの説明を追加(チケット残あり時のみ)。
④未ログイン時マイページのログインボタン不一致を修正: `/housing/mypage`を未ログインで開いた時のログイン導線が、登録/ツアー/お気に入りで使う共通コンポーネント`HousingLoginPrompt`(ハニーゴールド主CTA)ではなく`HousingerPage.tsx`内の直書きボタン(色なし)だったため見た目が浮いていた不具合を修正。`HousingLoginPrompt`のcontextに'mypage'追加・i18n5言語・旧キー削除。共通シェルがブロックレイアウト前提のため案内カードがパネル左上に寄る問題も修飾クラスで解消(HousingDetailPage.tsx側は不変)。副産物: 修正過程で発覚した`HousingerPage.test.tsx`の既存不備(`useHousingListingsStore`が`beforeEach`未resetでテスト順依存の汚染が起きうる)も解消。
⑤タイムライン最大表示時間を20分→25分に延長: 「25分までのコンテンツがあるので伸ばしてほしい」というユーザー要望への対応。事前にコスト/パフォーマンスを検討: デフォルト表示(空行非表示)のDOM数はイベント数のみ依存・Firestore保存量も無関係なので普段の動作に影響なし。唯一「表を展開する」操作(既存の重さ課題#59)が行数+25%になる点のみ影響。`Timeline.tsx`の`fightDuration`と、白紙プランのオート挿入上限(astrologian/scholarAutoInsert.ts各2箇所)に重複していた「20分=1200秒」を`src/constants/timeline.ts`の`DEFAULT_FIGHT_DURATION_SEC`に集約し3ファイル5箇所で共用。ユーザー自身が時間を選べる可変長機能は「表を展開する」の重さが本格的なボトルネックになるため保留、真に必要になったら仮想化とセットで検討する方針。

いずれもi18n 5言語対応・token経由。tsc/vitest確認済み。

### ✅ 2026-08-11 ツアー計画画面(蛇行グリッド)実装+ブラッシュアップ完了 = 本番push・デプロイ完了(`084a885f`)

探す/お気に入りの右トレイだけでは50〜100件規模のツアーは並べ替えづらい問題を解決する計画画面(PC=左に詳細パネル+右に蛇行グリッド/スマホ=縦一覧)を実装後、ユーザーと何度も実機往復しながら細部を仕上げた。設計書=`docs/superpowers/specs/2026-08-11-housing-tour-tray-page-design.md`。

主な変更: ①接続線は「角丸ポリライン」方式に収束(直線+曲がる箇所だけ丸くカーブ)/②線の上を流れる光の玉アニメーション追加(SMILのbeginElement強制開始で初回スタック不具合を解消)/③マウスホイール横スクロール対応/④カードのホバー浮き上がり/⑤カードの番号バッジを撤去しタイトル表示幅を拡大(共有部品のためサイドバー/スマホ一覧にも波及)/⑥**ピン留めの設計刷新**: 「ドラッグ=自動ピン留め」「効率順に並び替え=全ピン解除してリセット」の2ルールに単純化し、旧来の「ドラッグ1回で全ピンが無反応になる」バグを解消/⑦招待前ログインボタンの文字が左寄りになる不具合・初回マウント時に線や玉が誤った位置から動いて見える不具合を修正。

**残**: ピンボタンの視覚的な説明(hoverツールチップ)を追加するか未決定のまま。

### ✅ 2026-08-11 プロフィール画像(アイコン)アップロード不具合 = 本番修正済み(実ユーザー報告→即日解決)

根本原因: 2026-07-31のPNG派生版保存機能追加(`b46bf9a4`)でStorageへの書き込みファイルが`avatar.webp`+`avatar.png`の2つになったが、`storage.rules`の書き込み許可パターンが`avatar.webp`のみのままだった。WebP本体アップロード後のPNGアップロードがpermission-deniedで例外→処理全体が失敗しユーザーには「アップロードできない」だけが見える状態に(実際はWebP本体は保存済みだがFirestoreのURL保存まで届かない)。ユニットテストは`uploadBytes`をモックしておりStorageルールを検証しないため検知できなかった。`storage.rules`の正規表現に`avatar.png`を追加し本番デプロイ済み(`firebase deploy --only storage`)・コミット`f64e1640`(2026-08-12にgit push済み)。

**🟡 残課題**: このバグを機械的に検知するテストが無い(`@firebase/rules-unit-testing`未導入)。次にStorageルールを触る時は導入を検討。

### ✅ 2026-08-10 ツアー追加フィードバックアニメーション = 本番push・デプロイ完了(`084a885f`)

探すページのカード/詳細ページの「ツアーに追加」ボタンに、成功時のチェックマーク演出+「追加済み」トグル状態、失敗時(別リージョン)のボタン単体シェイク+吹き出しを実装。設計書=`docs/superpowers/specs/2026-08-10-housing-tour-add-feedback-design.md`・実装計画=`docs/superpowers/plans/2026-08-10-housing-tour-add-feedback.md`。サブエージェント方式(7タスク+最終全体差分レビュー)で実装、本物のバグ計5件検出・修正(hookの未使用import/エラーメッセージ残留、吹き出しrefの型エラー、色トークンがdocument.body配下で未解決、画面端クランプ漏れ、トレイ削除時のピン残留)。既存2ファイル(BrowsePage/FavoritesPageの旧トースト前提テスト)も新仕様に更新済み。

### ✅ 2026-08-10 ハウジングツアーの不具合修正一式 = 本番push・デプロイ完了(`084a885f`)

非ログインユーザーが「みんなを招待」でツアーリンクを発行しようとすると、汎用エラートースト(「招待リンクの発行に失敗しました」)だけが出てログインが必要なことが伝わらず、原因不明の失敗に見えていた不具合を修正。招待発行はログイン必須の仕様(`createSharedTour`)自体は変更せず、未ログイン時は既に用意されていたログイン案内(`HousingLoginPrompt context="tour"`)に表示を差し替えて、原因がユーザーにわかるようにした。テスト2件追加。
...
- 案内文言の修正: `login_prompt.tour.lead` の仮文言を実態(誰でも発行できると悪用されるおそれ)に合わせて5言語とも修正。
- UX改善・新機能: 住所追加パネルsticky footer化+Enter送信/ツアー中パネルに登録者アイコン名前表示/現在地カードにお気に入りハート追加/DC・ワールド跨ぎ案内を実操作+区画名+区番号+視線誘導アニメ+強調表示+住所2段化+拡張街バッジで強化。詳細・文言before/after → `docs/.private/2026-08-10-tour-crossing-wording-revision.md`。
- その他: エンピレアム21号地ナビ経路override修正([[project_housing_tour_route_fix_policy]]方式)/ハウジンガー「まとめてツアー」の既存トレイ上書きバグ修正(全5開始経路確認済み)/OGPカードSQUARE ENIX著作権表記追加(テスト2件)。

### ✅ 2026-07-31 細かい修正一覧 = 本番デプロイ完了(`cd2d95f3`)
...
**追加修正(2026-08-10・ユーザー実機指摘)**: 展開戦術でマニフェステーションのバリアを展開できない不具合。原因は`useMitigationStore.ts`内の`resolveShieldLinks`(展開戦術のリンク自動解決)が`MitigationSelector`/`mitigationTapResolver`とは別の独立実装で、`matchesCopiesShieldSource`統一に追従できていなかったため(リンクしても直後に自動解除されていた)。同関数を使うよう修正しTDDで回帰テスト追加(`66ab70fb`)。2026-08-12にgit push・デプロイ済み。

**同時にスコープ外便乗で修正(告知対象外)**: スマホFAB/Undo・Redoボタンの視認性改善/メモ入力ボックスが画面端で見えなくなる不具合をfixed+portal化で修正(共同編集参加者側も同時に直った)。
...
### ✅ 2026-08-07/08 共同編集: オーナー自動判別+人数変更確定ボタン+reseed信頼境界バグ修正+プラン複製トークン引き継ぎバグ修正 = 本番デプロイ・実機確認完了

オーナー自動判別+人数変更確定ボタン(6タスク)を実装後、最終レビューで「別プランのデータが共同編集ルームへ不可逆に混入しうる」既存の重大データ安全バグ(`reseedEmptyDocFields`が手元データの出処を検証しない設計、参加者側でも起こりうる)を発見。4方向の独立調査→設計書→実装計画(TDDで7タスク)→最終レビューで見つかった圧縮済みプランのフェイルオープン修正まで完了(`91f99e65`)。付随して見つかった「プラン複製3経路がactiveCollabRoomTokenを引き継ぐバグ」(ユーザーの積年の要望「複製したら普通のプランになってほしい」も同時に解消)も修正。詳細=`docs/.private/2026-08-07-collab-data-safety-audit/00-synthesis-and-next-steps.md`(調査)+`docs/superpowers/specs/2026-08-07-collab-reseed-trust-boundary-design.md`(設計)+`docs/superpowers/plans/2026-08-07-collab-reseed-trust-boundary.md`(実装計画)。2026-08-08ユーザー実機確認: 定員/オーナーがリンクを踏んだ時の挙動OK。**軽微な観察**: 1人店員時に別タブでリンクを踏んだ際、一瞬「満員」表示が出たがすぐ自分の軽減表画面に読み込まれた(自己解消・実害なし・再発したら調査)。

### ✅ 2026-08-05 サモン・セラフィム召喚中のフェイブレッシング/エーテルパクト使用不可制約を追加 = 本番デプロイ完了

ユーザーから「サモン・セラフィムの効果時間中に使用できないスキルがあるはず」との質問。公式ジョブガイド+複数の詳細解説記事(1つはパッチ6.0時点の古い情報と気づかず誤って引用し、ユーザーに指摘されて訂正)で裏取りした結果、セラフィム召喚中はフェイブレッシングとエーテルパクト(フェイユニオン)がエオス/セレネ専用のため使用不可、光の囁き/フェイイルミネーションは名称が変わるだけで実際は使用可能と判明。データ変更は不要(既存の`requiresFairy`フラグはダミー化せず活かす)で、配置判定ロジック側の修正のみ。手動配置(`resourceTracker.ts`)と自動プランナー(`autoPlanner.ts`)の2箇所に、既存の転化vsセラフィム排他チェックと同じパターンで制約を追加(`e5f6d6dc`)。手動配置側はTDDで4件のテストを追加、自動プランナー側は内部が焼きなまし法の複雑な最適化アルゴリズムのため単体テストは見送り(既存の類似チェックも同様にテスト無し)とユーザーに正直に共有し了承済み。

### ✅ 2026-08-05 技のレベル版違い(`_v2`)が軽減追加モーダルの最下段に落ちる表示順バグを修正 = 本番デプロイ完了

ランパートのLv94回復効果アップ追加後、ユーザーから「軽減追加モーダルでランパートが一番下になっている」と実機指摘。原因は`rampart_v2_pld`等の新規IDが`MITIGATION_DISPLAY_ORDER`の照合(`getMitigationPriority`)でジョブ接尾辞しか剥がしておらず`_v2`部分が残ったままリスト照合に失敗し、優先度999(最下段)に落ちていたため。テストで検証したところ、既存の`aurora_v2`にも全く同じ潜在バグがあったことが判明(今回の作業とは無関係に以前から存在)。`getMitigationPriority`でジョブ接尾辞に加えて`_vN`接尾辞も剥がしてから照合するよう修正(`b2954d48`)。データ変更・Firestore反映は不要(純粋なコード修正)。

### ✅ 2026-08-05 ハウジング モバイル表示ズレ修正(visualViewportマウント時無条件補正)= 本番デプロイ完了・実機確認は次回

ユーザー実機報告(iPhone SE/Safari)「探すページを開くと最初からズレる、リロードしても直らない、お気に入りページ経由だと直ることがある」を調査。当初「Service Workerのキャッシュが古い」と仮説を立てて実装まで進めたが、ユーザーから「勝手に進めすぎ・仮説止まり」と指摘を受け差し戻し。ユーザーの「既存修正が効いているはずなのに直っていないなら根本原因が残っているのでは」という指摘を受けて再調査した結果、`HousingShell.tsx`のiOS Safari対策(`visualViewport.resize`検知)が**「変化」にしか反応せず、マウント直後の初回描画が既にズレていた場合に直す手段が無い**という実装ギャップを発見。業界標準パターン(CSS-Tricksの`--vh`手法等、複数の技術記事で裏取り)は「読み込み時に無条件で1回同期+以後resizeイベントでも同期」という形が定番であることを確認し、同パターンに合わせてマウント時の無条件補正を追加(`e788fcc1`)。TDDで先に失敗するテストを書いてギャップを実証してから実装。並行して調査していた「SW即時更新チェック」の修正(`serviceWorkerAutoUpdate.ts`)は、今回のバグの原因としては裏付けが取れなかったため破棄。実機確認はユーザー側で次回実施。

### ✅ 2026-08-05 ランパートにLv94特性の被回復+15%を追加 = 本番デプロイ完了・確認済み

ユーザーから「ランパートに回復効果アップは付いているか」との質問を受け、当初はアプリのデータのみで判断し「付いていない」と誤答。「必ず公式情報を確認して」との指摘を受け、公式ジョブガイド(jp.finalfantasyxiv.com/jobguide/warrior/)で裏取りした結果、Lv94特性「ランパート効果アップ」により被ダメ20%軽減に加えて被回復+15%が追加されることが判明(Lv93以下には付かない)。既存の`aurora`/`aurora_v2`と同じレベル分岐パターン(`minLevel`/`maxLevel`で非重複の2エントリに分離)で`mockData.ts`に反映し、`scripts/seed-skills-stats.ts --force-overwrite`でFirestoreへ本番反映(dataVersionインクリメント済・全ユーザーのキャッシュ即時反映)。反映前に対象の全エントリと、ユーザーが直近編集した野戦治療の陣(duration=18)がFirestore側と完全一致することもdry-runで確認済み。

### ✅ 2026-08-04 ハウジンガーOGPカード grid/sidebar抽選の体感頻度を改善 = 本番デプロイ完了

ユーザーから「2パターンは本当に毎回ランダムか」の確認質問を受け、実装(`api/share/_housingerPageHandler.ts`)を直接調査。画像本体は内容ハッシュ単位で永続キャッシュ(名前・自己紹介・写真が変わらない限り再生成なし)されているが、どちらを見せるかの抽選自体はページ全体のキャッシュ(`s-maxage=300, max-age=60`)に巻き込まれ5分に1回しか振り直されていなかった。ユーザーの「画像生成コストは変わらないなら、もっと頻繁に抽選し直したい」という提案を検証したうえで妥当と判断し、`s-maxage=30, max-age=0`に短縮(`ff88e8db`)。抽選のやり直し自体はFirestoreへの軽い書き込みのみでsatoriレンダリングは発生しないため、コスト増は小さい。**注記**: DiscordやX側もリンクプレビューを自前でキャッシュするため、「同じリンクを開くたび毎回変わる」は実現できない(相手側キャッシュが実質的な下限になる)ことをユーザーに事前説明済み。
...
### ✅ 2026-07-24 マイページ ブラッシュアップ 最後の2点 = 実機確認OK・本番反映済み

4巡目の実機FB対応で作り込んだ2つの不具合を、5巡目セッションで実機証拠ベースの調査により解決。

**①左カラムのスクロールバー色**: 「薄い透き通ったグレーのはず」というユーザー指摘に対し、housing.css内の該当トークン(`--housing-text-dim`=白0.78)や他の候補を全数確認したが該当する既存の色が見つからず、ユーザーに直接確認 → 独自の色を作るのをやめてブラウザ標準表示に戻す方針に決定・反映。

**②マイページ一覧カードの公開状態バッジのメニューが押しても表示されない(4巡目のcreatePortal化による退行)**: 実機DevToolsでの事実確認を徹底し、2段階で根本原因を特定。
- 1段目: `btn.click()`を実機で直接実行しaria-expanded/portal存在を確認したところJSロジックは正常。computed styleを取得したところ`zIndex=auto`という異常値を発見 → `--housing-*`CSS変数群は`:root`ではなく`.housing-workspace`等の限定セレクタでのみ定義されており、`createPortal`で`document.body`直下に出たメニューはそのスコープ外で全変数が未解決(z-index/背景/文字色が全てtransparent/auto落ち)になっていたと判明。該当セレクタへの追加で修正。
- 2段目: 1段目の修正をデプロイ後、ユーザーから「メニューが画面いっぱいの板になった」とスクリーンショット付きで報告。実機で computed style を再取得すると`width=819px height=673px`。原因は1段目で追加した共有セレクタに、`--housing-*`トークン定義だけでなくページ全体のシェルレイアウト(`width:100%;height:100vh;position:relative`等)も同居していたため。既存の他3クラスは自前で`position:fixed;inset:0`を持つため実害が出ていなかっただけだったと判明。共有セレクタへの追加をやめ、メニュー専用の小さいルールで必要な8変数だけを個別再定義する形に修正し直して解決。

**教訓**: 実機DevToolsでの事実確認(`btn.click()`直接実行・computed style取得)がなければ、両方とも「コードは正常に見えるが実際は不可視/巨大化」という気づきにくい不具合のまま長期化していた。修正内容はmemory `reference_housing_root_modal_tokens`に「対処法自体が新バグを生んだ」教訓として反映済み。詳細ログ=`docs/.private/2026-07-24-mypage-card-overlay-and-scrollbar-handoff.md`。

### ✅ 2026-07-24 マイページ作成 = 実装完了・本番デプロイ・ユーザー実機確認OK・Discord告知済み(別便)
brainstormingから実装まで完了。旧「ハウジンガータグ(個人タグ)が編集できないバグ」は実態としてはバグでなく編集導線が存在しないだけと判明したため、本タスクに統合。自分のハウジンガーページ(`/housing/housinger/自分のuid`)を拡張し、`/housing/mypage`もこれに接続(自分のuidにフォールバック、未ログイン時はログイン案内)。main push済み(commit `403696ff`)。

**実装内容**: ①左右2カラムレイアウト刷新(パネル分割はせず1枚のまま左右に分ける・プロフィール列はスクロールしても画面に張り付く・一覧5列を左右いっぱいに使う幅で維持・自己紹介欄168px固定高で内部スクロール、ボタン位置が文章量でガタつかない) ②自分の全物件(公開/住所非公開/完全非公開)を一覧表示するよう変更(旧: 公開分のみ) ③各カードにワンクリック公開状態切替+確認モーダル(完全非公開のときだけ警告文言)。新API `POST /api/housing?action=update-visibility`(既存の`update-listing`はフォーム全体バリデーションが必要で軽量切替に不向きだったため新設、Vercel Hobby関数上限は既存`api/housing`ルータへの`action`追加なので影響なし) ④設定ウィンドウをシンプル化(アイコン/名前/管理画面/ログアウト/退会のみ)、自己紹介文・SNSリンク編集(`HousingerProfileSection`)はモーダルからマイページへ移動 ⑤表示名編集欄(モーダル内)に「この名前がハウジンガータグとして使われます」の説明を追加(以前は説明が一切無くユーザーが自覚できていなかった) ⑥スマホ下部ナビのアカウントボタンが「マイページ/アカウント設定」の2択ポップオーバーメニューに ⑦公開期限切れ後の挙動をユーザー選択制に: 他人からの見え方(公開判定)は既存の遅延評価方式のままリアルタイム、登録・編集フォームに「期限が来たら住所非公開/完全非公開のどちらにするか」の事前選択肢を追加し、裏側の日次cron(`apply-expired-visibility`、CRON_SECRET認証)が実際にvisibilityを書き換える(取りこぼしても次回実行が拾う設計)。Firestore複合インデックス(`visibility`+`publishUntil`)を追加。

**ケバブメニュー移動**: 他人のハウジンガーページで「…」通報メニューがヘッダー行にあると40px固定円形ボタンのぶんヘッダーの高さが本人閲覧時とズレる問題を発見・修正。プロフィール右のアクション列(まとめてツアー/シェア/X)に移動しサイズを揃えた。

**ビルド・検証**: 型チェック(`tsc -b`)・4言語ロケールJSON構文・本番ビルド(`vite build`)・housing関連テスト685件、いずれも通過(既知の無関係failure=EphemeralAddPanel 7件のみ)。ユーザー実機確認で全項目OK。

### ✅ 2026-07-22 探すページ表示順ランダム化+一覧スクロール位置復元 = 実装完了・main反映済み
設計書=`docs/superpowers/specs/2026-07-21-housing-browse-random-order-scroll-restore-design.md`、実装計画=`docs/superpowers/plans/2026-07-22-housing-browse-random-order-scroll-restore.md`。subagent-driven-developmentで9タスク+最終whole-branchレビュー(Opus)まで完走、全task reviewでCritical/Important指摘なし。mainにmerge済み(merge commit `2fbada38`)。

**実装内容**: 探すページのデフォルト表示順を「ランダム」に変更(新着順/古い順も選択可・シャッフルボタン付き)。ランダム順のシード値・並び順選択・スクロール位置は新設の非永続zustandストア(`useHousingListOrderStore`)がSPA内遷移(詳細→戻る等)の間だけ保持し、実リロードでリセットされる。スクロール位置復元は探す/お気に入り/ハウジンガープロフィールの3画面共通。

**実装時に発見した設計書との差分**: 設計書は「現在のデフォルトは住所順グループ化」としていたが、実際のコードは既に`.sort()`でcreatedAt降順(新着順)に上書きされており、住所順という選択肢自体がUIに存在したことがなかった(`sortListingsForGallery`の呼び出し結果は後続の`.sort()`で常に上書きされ事実上無効化されていた)。今回の実質的な変更は「ランダムを3つ目の選択肢として追加しデフォルトにする」のみ。最終要件(ランダム既定+新着/古い順選択可)自体は変わらない。

**最終whole-branchレビュー(Opus)で見つかったMinor4件**: うち3件(favoritesエントリのsortMode既定値がrandomで紛らわしい/シャッフル時にstore側scrollTopが同期されない/コメント不足2箇所)を修正コミットで反映。残り1件(router実ナビゲーションを通した end-to-end 結合テストの追加)は計画書自身のテスト戦略で許容範囲と判断し見送り。

**検証**: 全9タスクのtask-level review(各1ラウンドで承認)+最終whole-branchレビューでReady to merge=Yes。フルビルド+フルテストとも既知の無関係failure(EphemeralAddPanel7件)以外クリーン。

**残(次回ユーザー実機確認)**: ①探すページで実リロードのたび並び順が変わる ②詳細→戻るで並び順・スクロール位置が保持される ③シャッフルボタンで再抽選+先頭スクロール ④新着順/古い順切替でシャッフルボタンが消え、選択も詳細往復で保持される ⑤お気に入り・ハウジンガープロフィールでもスクロール位置復元 ⑥モバイル幅でシャッフルボタンの配置確認 ⑦英語/韓国語/中国語モードの表示崩れ確認。

### ✅ 2026-07-22 初心者向けタグ「ハウジング若葉」= 実装完了・main反映済み
タグ刷新計画書 (`docs/superpowers/plans/2026-07-10-housing-tag-overhaul-plan.md`) の Phase C。実装計画=`docs/superpowers/plans/2026-07-22-housing-beginner-tag.md`。新 kind `'beginner'` を1件 (`beginner_sprout`) 追加、4言語ロケール (EN "Housing Newcomer" / KO "하우징 새싹" / ZH "房屋新人") を追加。`HousingRegisterTagPicker.tsx` はレジストリ駆動設計のため**コンポーネント側の変更は不要**(タブ・タグ選択とも自動的に出現)、統合テストでそれを実証。subagent-driven-developmentで2タスク実装、task reviewerとも Critical/Important指摘なしで承認。mainにfast-forward mergeでcommit済み(`7fda2a78`)。ビルド・全テスト確認済み(既知の無関係failure=EphemeralAddPanel 7件のみ)。
**残(次回ユーザー実機確認)**: 登録/編集フォームで「初心者」タブ→「ハウジング若葉」選択→chip表示→×で解除、英語/韓国語/中国語モードでの表示崩れ確認。

### ✅ 2026-07-22 複数投稿URL登録機能 (Batch 2) = 全12タスク+最終レビュー完了・main反映・本番デプロイ・ユーザー実機確認OK
...
**本番確認 (2026-07-22 ユーザー実機・全項目OK)**: ①新規登録で複数URL(Twitterスレッド想定)の画像が合算される ②2本目以降の動画は無視され画像だけ追加される(トースト表示) ③同じURL2回貼りで重複エラーが出る ④既存物件の編集でURL追加時に既存画像が消えない ⑤動画付き既存物件の編集で写真だけの投稿を追加しても動画が消えない(最重要ポイント) ⑥「元の投稿を見る」の複数URLドロップダウン ⑦登録ページのURL優先UI(アップロード折りたたみ) ⑧新規スタイルの見た目 ⑨英語モードの文字崩れなし ⑩既存(Batch2以前)物件が今まで通り動く。Vercel自動デプロイが一時反応しなかった件は原因調査中に自然復旧(2026-07-22、デプロイ自体は正常完了)。
...
- **設計方針(brainstormingで確定)**: 確認ダイアログは一切なし(削除・登録方法切替とも)。操作ごとに即時サーバー反映、保存中は処理中表示→確定、失敗時はロールバック+汎用トースト。「差し替え」専用UIは作らず削除+追加の組み合わせで代替。登録方法切替タブは押した瞬間はローカル表示のみで、実際にサーバーへ反映するのは新しい方に実コンテンツが入った瞬間(誤操作でデータが消えないため)。
- **実装**: subagent-driven-developmentで9タスク実装(共有削除/並び替えグリッド→直接アップロード側パネル→URL側パネル→タブ→統合セクション→RegisterPage配線(旧仕様「方式A」= 編集モードで写真セクション自体を非表示にしていた仕組みを4箇所とも撤廃)→既存テスト更新→push前ゲート)。タスクごとにレビュー→修正サイクルを実施し、Task3で画像サイレント消失バグ(アップロード中の多重クリック)、Task5でi18nキー重複を発見・修正。
- **最終ブランチ全体レビュー(Opus)でCriticalバグ1件を発見・修正**: 編集ページでSNS投稿URLを2回以上貼り替えると、`commitEditSnsFetch`内で使う`buildDraft()`が前回貼り付けた古いSNSデータ(`snsCapture` state)を参照し続けてしまい、古いデータ(例:前回の動画URL)が新しいデータに混入したままサーバーへ送信される不具合。`buildDraft()`の戻り値から画像関連10フィールドを分割代入で除去し、その回のフレッシュなデータだけを信じる形に修正。回帰テスト追加、再レビューでReady to merge確認。あわせてImportant1件(動画プレビューが再貼り替え後に更新されない表示ズレ)も同時修正。
- **教訓**: 「状態を更新する関数自身が、次回呼び出し時の前提を壊す」パターン(1回目の`setSnsCapture`が2回目の`buildDraft()`を汚染)は1回だけのテストでは検出できず、同一操作を連続2回行う結合テストで初めて見つかった。
- mainにmerge・push済み(commit `b45c93ae`)。build/vitest(3612件)ともクリーン(既知の無関係failure=EphemeralAddPanel7件のみ)。**ユーザーが本番で実機確認、動作全部OK**(2026-07-21)。
