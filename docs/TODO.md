...
- **✅ YouTube ライブ配信対応 (2026-06-02 実装・要実機検証)**: `parseYouTubeId` の path 正規表現に `live` を1語追加 (`embed|shorts|v|live`)、`youtube.test.ts` に2ケース追加 (live / クエリ付き live)。時刻ロジック・UI・`useYouTubePlayer` は無改変 (相対方式なので真ライブ=配信開始からの経過秒でもVODでも同じ経路で正しく動く)。CSP 追加不要。テスト12/12緑・build緑・VideoRecorderModal回帰緑。設計追記=specs `2026-06-02-video-recorder-modal-design.md` §11。**残=デプロイ後に参考URL `https://www.youtube.com/live/<id>` を実機で1本通す** (貼付→再生→スタート→＋追加→書込)。
