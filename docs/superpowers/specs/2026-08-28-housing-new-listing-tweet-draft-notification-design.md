...
- 韓国語 / 中国語 (簡体・繁体) のツイート文面の実運用 (定数だけ用意、地域開放時に有効化)。
...
- Musk (2024-12)「ハッシュタグはもう使わないで」。アルゴリズムは本文を AI で読む。
- ハッシュタグのプラス効果はほぼ無く、数を増やすとマイナスに働きうる。
- ただし FF14 ハウジング勢は今も `#FF14ハウジング` / `#FFXIVHousing` の検索タブを
...
- 物件詳細ページの OGP を「写真 1 枚」ではなく合成カード (住所 + タイトル + 複数写真) に
  ...
  流用すれば作れるが、需要を見てから。
- 通知量が増えたら (地域開放後など)、admin 除外だけでなく「1 日 N 件まで」「ダイジェスト化」
  も検討。
