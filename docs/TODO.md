...
- **✅ 本番デプロイ済(詳細→[TODO_COMPLETED.md](./TODO_COMPLETED.md))**: スプシ取込一式(6/22 `a6af27c0`)/⑦敵攻撃or 2択(6/23 `6471e816`)/スプシ取込バグ修正一式(6/23 `ca98bd32`)/**取込フローv2前半+①誘導型ウィザード(6/23 `50c64d7d`)/**③攻撃の対象(MT/ST)テンプレ引き継ぎ+管理ツールバーsticky化(6/24 `276ff4f4..746476ba`・SDD+opus最終レビューReady=Yes・実機OK)**。**後追い候補**=スプシ「A or B」自動分割→altName / **攻撃名見切れマーキー(§5・`2026-06-22-event-or-attack-design.md`)** / マントラ取込直後の一過性非発光(軽微) / ウィザード`no_phases`理由非表示 / skipped表示のamber直値→トークン化 / ②途中取込(spec§7繰越)。
...
- **🆕 機能ブラッシュアップ案9件**(詳細=docs/.private/2026-06-15-feature-ideas-batch.md): ①同時刻3+イベント ②スマホ/タブレット最適化(残=ボトムナビ/FAB) ③軽減競合逆方向警告(✅実装済) ④MAXHP-10%でダメージ黄 ⑤Logsインポート上書き/追記(✅本番済2026-06-20) ⑥有名スプシ取込(✅実装完了2026-06-21=上記🟣・要実機確認) ⑦敵攻撃 or(2択) ⑧管理画面 攻撃ID保持で任意言語翻訳(GUID保持済=ほぼ実装済・仕上げのみ) ⑨メモに動画URL→iframe。⑥は実装完了(要実機確認)。取り込み導線チューザー統合は将来。**🆕列グリッド取込=§9.7改訂 **実装完了・全レビューclean・本人実機待ち**(`feat/spreadsheet-grid-import`・未merge・HEAD 5f0ce89e)。SDD全11タスク+各fix+最終opus=Ready to merge Yes。build EXIT0/vitest新規回帰ゼロ。§9.7=右パネル廃止しスプシ面集約(フェーズバー/メンバー列ヘッダーにインライン枠/Ctrl+A導線)・攻撃の対象は行ごとselect+テンプレ競合解消(優先=手動>自作>テンプレ)・matrixの列チップ撤去&読めない技だけセル内黄色(同時刻2技は白)・作成は常時可(未追加は自動取込)・自作は正式名称前提(辞書なし)+空白前処理+ジョブ列手動救済。spec=`docs/superpowers/specs/2026-06-24-spreadsheet-grid-import-design.md`§9.7 / plan=`docs/superpowers/plans/2026-06-24-spreadsheet-grid-import-v97-ux.md`(11タスク)。次=本人が plan Task11 実機チェックリスト→OKで finishing-a-development-branch で main merge+push。**
