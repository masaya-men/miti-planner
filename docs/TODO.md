...
### 🔴 マイページ ブラッシュアップ = 一部本番デプロイ済だが2点未解決・要実機調査(2026-07-24)
①〜⑦(右カラムのみスクロール/鉛筆その場編集/アイコンモーダル差替/公開文言改善/textarea化/まとめてツアー非表示/公開やめる文言)は解決・本番反映済み。
①左カラムのスクロールバー色 = ✅解決・本番反映待ち。独自の色は作らずブラウザ標準表示に戻した(2026-07-24 ユーザー最終確認)。
②マイページ一覧カードの公開状態バッジのメニュー非表示 = ✅**根本原因判明・修正済み・本番反映待ち**。実機診断の結果、ポータル要素は生成されるが`--housing-*` CSS変数群が`.housing-workspace`等の限定セレクタ内でしか定義されておらず(housing.css:7-10)、`document.body`直下のポータルには継承されず全変数が未解決→z-index/背景/文字色が透明・auto落ちして「存在するが完全に不可視」になっていた。修正済み(該当セレクタに`.housing-card-visibility-menu-portal`を追加)。ユーザーに本番での見た目確認を依頼中。
詳細・調査ログ = `docs/.private/2026-07-24-mypage-card-overlay-and-scrollbar-handoff.md`(「根本原因判明・修正済み」セクション)。ユーザーから明示指示: 「セッションを区切って新しいセッションで、文脈を失わずに確実にやること」。
