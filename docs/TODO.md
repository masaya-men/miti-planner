...
## 現在の状態 (次セッションはここから読む)
### 🔴🔴 次セッション最優先(緊急): 画像品質バグ3件(2026-07-20発見・実利用中に発覚)
ハウジングは既に実質稼働中(104件登録・大半が本日連続登録・16以上の異なるユーザー)と判明。その中で発見:
①圧縮処理が`alwaysKeepResolution`未指定でサイズ優先→複雑な画像は解像度を激しく削り極小画像化(実例: 239×134px) ②一部ブラウザ(古いSafari等)がWebP非対応でPNGにフォールバック→写真圧縮が苦手なPNGはさらに画質劣化 ③「カバー画像だけ消える」不具合が**本日16:48の本番反映(下記✅参照)より後も再発**しており別原因の疑い(PNG勢のみ3件・413拒否が濃厚な仮説)。詳細・実データ・直す方向性=`docs/.private/2026-07-20-housing-image-quality-and-cost-investigation.md`。**このセッションでは調査のみ・未修正**。

**その後の方針(ユーザー要望)**: 上記3件修正+複数投稿URL機能(Batch 2)完了後、登録UIで「URL経由登録」を理由付きで目立つように推奨する方向へ改修(直接アップロードよりURL経由の方が画質・枚数・動画対応で優位なため)。着手時はbrainstormingから。
