...
- **最新セッション（2026-05-08・ハウジングツアー Sub-spec 2A 実装完了）**: 設計書 §18 Sub-spec 2 のうち「登録 (画像なしモード)」を完成。`subagent-driven-development` で 26 タスクを順次実装、各タスクごとに spec/quality 2 段レビュー。中身: タグマスタ 147 件 × 4 言語 i18n / バリデーション・登録枠 D 案・重複住所キー の純粋関数群 / Firestore listings 読取 service / `/api/housing` 3 アクション (can-register / register-listing / check-duplicate、Firestore Admin SDK + AppCheck + RateLimit) / 登録フォーム本体 (HousingRegisterView) + 重複警告ダイアログ + オンボーディングダイアログ + 未ログインプロンプト / `/housing` 3 タブ (探す/回る/登録) 切替 + URL ハッシュ同期。Firestore rules に `addressKey` 検証追加→本番デプロイ済 (`firebase deploy --only firestore:rules`)。`HousingListing` 型に `addressKey: string` 必須フィールド追加。デザインは Tailwind プリミティブのみで仮置き、リキッドグラス・ルーペは 2C で本格適用予定。**437 tests PASS / tsc clean / build 成功 / Playwright 自動チェック 7/7 OK** (3 タブ表示 / オンボーディング初回表示 → 「はじめる」で閉じる → LocalStorage flag → 再訪で出ない / 未ログイン時 `/miti` へのプロンプト / 探す・回るタブのプレースホルダ表示)。**残り手動確認**: 実機ログイン後に登録 1 件成功するか + ward 31 等のバリデーションエラー表示 + 同住所 2 回目で重複ダイアログ。次セッションは Sub-spec 2B (Gallery & Search) のプラン作成。
...
### 次にやること（優先順）
- **【最優先・次セッション】Sub-spec 2B (Gallery & Search) のプラン作成 → 実装**: 登録済みデータをギャラリー表示・検索フィルタ (タグ / DC / エリア / サイズ) ・URL クエリ反映・お気に入り。画像はまだ「画像なし」プレースホルダで OK。リキッドグラスやルーペは Sub-spec 2C 以降。工数想定 3-5 日。
- **Sub-spec 2A 実機確認 (本番)**: lopoly.app/housing にアクセスして以下を確認。①3 タブ切替 ②未ログイン時 `/miti` プロンプト ③オンボーディング初回のみ ④ログイン後の登録フォームで 1 件登録成功 (DC=Mana / Server=Pandaemonium / Area=Shirogane / Ward=3 / Plot=12 / Size=M / タグ 1-2 件) ⑤ ward に 31 を入れるとバリデーションエラー ⑥ 同住所で 2 回目登録 → 重複警告ダイアログ。
