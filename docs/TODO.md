...
- **🚧 2026-06-19 進捗HUD イテレーション2 + ドロワーUI磨き + 共有除外 (branch `feat/progress-tracking-hud`・HEAD `75bcc59`・未push/未merge/未deploy=エンドユーザー不可視)**: イテレーション2本体(A記録ハイライト根治/Bトースト/C記録ドロワー/Dホバー停止根治)完了。本セッションで**ドロワーUIを実機合わせで磨き込み(16コミット)**: 横位置=画面中央 / ライト白基調 / トーストをテーマ対応(light青み抑制) / 開閉ミラー演出(開360ms・閉153ms) / 外側クリック&帯再クリックで閉じる / U型でヘッダー下端ハンドル線に密着(引き出し化) / コンパクト3段+CLEAR!をライン上 / 活動±撤去+ダブルクリック直接編集+固定幅(ガタつき防止)+(任意) / 戻るボタンをドロワー右下に常時表示+LoPo Tooltip(最新打点1つ削除)。設計=specs/2026-06-19-progress-hud-iteration2-design.md / 経緯=[.private/2026-06-18-progress-hud-iteration2.md](./.private/2026-06-18-progress-hud-iteration2.md)。memory `reference_waapi_transform_clobbers_centering`。
  ...
  - **🔴🔴 次セッション最優先=最高モデル+最大エフォートでデータロスト/既存破損ゼロを敵対監査**(ユーザー要望・恐怖記憶): ①自分のプランの progress/memos が保持されるか全経路(getSnapshot/syncToFirestore/pullFromFirestore/loadSnapshot/BackupExport/duplicatePlan)②共有プレビュー・単一/ bundle コピー・OG・popular・template が壊れないか(undefined memos/progress クラッシュ無)③api/share strip は vite dev 非実行のため**デプロイ後E2E必須**(POST保存がクリーン/GET応答がクリーン/旧共有もクリーン)。memory `feedback_dataloss_exhaustive_audit` 準拠で多エージェント監査。**検証OKまで merge/deploy しない。**
