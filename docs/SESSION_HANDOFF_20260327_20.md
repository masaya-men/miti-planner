...
### 1. 人気ページのガラス表現を大幅強化
- empty-liquid-glass風のプロフェッショナルなエフェクトにしたい
- 縁が明るく明滅、光が走る演出。共有カード（1層等）も光っていなくて寂しい
- 軽いページなのでリッチにできる

### 2. LoPoロゴ文字サイズ拡大（軽減表ヘッダー）
- カプセルの大きさは変えずに文字を大きく
- ランキングページのLoPoロゴくらいの存在感にしたい
- 対象: `src/components/LoPoButton.tsx` の `size="sm"` 設定

### 3. スマホ版の表示修正
- 軽減表ページにLoPoロゴが表示されていない
- 人気ページのLoPoロゴが他要素とぶつかって潰れている

### 4. UI温度感の統一（オーバーレイの暗さ等）
...
### 6. プラン名編集のツールチップ
- デフォルトのまま → カスタムデザインに統一

### 7. PWA対応状況の確認
- スマホでホーム画面に追加してアプリとして使えるか確認
- アイコンは作ってある可能性あり。manifest.json等の状態をチェック
