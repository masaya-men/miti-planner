...
- **🔴次=段取り⑤(共同編集の実入口)をbrainstormingから設計**: 重要発見=**共有はコピー作成で別ID化([MitigationSheet.tsx:209](./../src/components/MitigationSheet.tsx#L209))→現状plan IDを部屋鍵にしても他人と繋がれない**。業界標準=「共同編集リンク発行→同じ部屋に参加(コピーでない)→オーナー不在でも編集可」。⑤で**ルームトークンをplan IDと分離・入口は共有(共有)ボタン内・既存コピー共有と別物**。「全員退室後も残す」は③(Firestore保存)必須。**完成までUI一切非表示厳守**(ユーザー指示)。順序候補: ⑤(入口)→③(保存)→②-b(他要素)→②-c(Undo)。
