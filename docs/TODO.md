...
- **今セッション完了**:
  - フッターglass効果: Layout.tsx（軽減プランナー）+ PopularPage.tsx（みんなの軽減表）にサイドバーと同じglass-tier3 glass-frame適用
  - チュートリアルSTEP1-3でサイドバーハンドル右罫線が消えるバグ修正（右側代替ライン追加）
  - 同一時刻イベントの表示順保証: MT→ST→AoEの順で常に表示（Timeline.tsx eventsByTime）
  - ビルド成功・未テスト（要確認: `npm run build` + `vitest run`）

### 次セッションでやること
- ビルド・テスト確認してからコミット＆push
