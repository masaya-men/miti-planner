...
### ✅ ツアートレイのサムネイル画像クリックで詳細ページへ(2026-08-12・未push)
探すページのツアートレイ・ツアー計画画面の蛇行グリッド・スマホ縦一覧で共用する`TourTrayRow`のサムネイル部分だけをボタン化し、詳細ページ(`/housing/listing/:id`)へ遷移するようにした(共有部品のため1箇所の修正で3箇所に反映)。行本体クリック(蛇行グリッドではin-place詳細パネル表示)とは独立して動作、一時listingは対象外。i18nキー5言語追加・CSS追加・関連テストにMemoryRouter配線。tsc・関連vitest(55件)確認済み。
### ✅ 探すページ一覧+ツアー横スクロールにスムーズスクロール追加(2026-08-12・未push)
過去に document 全体へ Lenis を適用してホイールが全画面で効かなくなり撤去した経緯(2026-05-14, `b0f948d8`)があるため、同じ罠を避けて軽減表で実績のある個別要素配線方式(`useSmoothWheelScroll`のバネ補間)を採用。①`ListingGrid`(探す/お気に入り/マイページ共通の一覧)に配線②`TourTrayBoard`の縦ホイール→横スクロール変換を即時反映からバネ補間に置き換え(`smoothScrollLogic.ts`の純粋関数を横方向に転用)。タッチ/reduce-motion環境は従来どおり即時反映。tsc・関連vitest(27件)確認済み、ユーザーのローカル実機確認待ち。
