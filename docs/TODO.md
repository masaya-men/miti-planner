...
### 🟡 マイページ ブラッシュアップ4件 = 実装済・デプロイ待ち(2026-07-24)
実機FBを反映: ①ハウジンガー/マイページ共通の右カラムスクロール不可バグ修正(詳細ページ用「画面内フィット固定レール」レイアウトを誤って継承していたのが原因) ②名前横の鉛筆→その場で名前編集(アイコンにも変更バッジ追加、旧は/housing/mypageへの無意味な自己navigateだった) ③「ハウジンガー公開」説明文を分かりやすく書き換え+「表示名がハウジンガータグとして使われます」を追加(4言語) ④「ひとこと」欄をtextarea化(改行OK・フォーカス/入力量で自動拡大)。①③④はコード確認・実機検証済み、②③④はログイン必須のためローカル未検証→本番デプロイ後にユーザー実機確認予定。**次セッション最優先候補**=③タグAND検索機能(要brainstorming)。
