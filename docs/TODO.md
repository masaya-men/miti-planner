...
0. 🏠 **ハウジング公開前 残タスク**(網羅=`docs/.private/2026-07-15-housing-release-remaining-tasks.md`):
   - **✅スマホ対応=実装完了→本番反映(2026-07-16)**: ボトムナビ/FAB/フィルタ・設定シート・探す/お気に入り2列・詳細/登録/編集/PF全画面・ツアー横持ち(案A)・共有ツアー参加横持ち。PC無変更(≤767pxのみ)。opus全体レビュー済(I-1=没入判定をpath化)。**実機確認待ち(ユーザー)**。設計/計画/台帳=superpowers/{specs,plans}/2026-07-16-housing-mobile-support* + .superpowers/sdd/progress.md。(前デプロイ5件=全部OK✅07-16実機)
