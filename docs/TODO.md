...
## 現在の状態 (次セッションはここから読む)
### 🔴 次セッション最優先: モバイル軽減競合(CDかぶり)表示が未実装(2026-08-12ユーザー指摘・確認済み・未着手)
PC版は`findSameSkillCdConflicts`(`src/utils/resourceTracker.ts`)で検出した競合を`MitigationItem`が黄色脈動(`animate-conflict-pulse ring-2 ring-amber-400`)で表示するが、**スマホ版はこの`MitigationItem`を一切使わず別コンポーネント`MitiIcons`(`src/components/MobileTimelineRow.tsx`)で軽減アイコンを描画しており、`conflictingIds`を全く受け取っていない**(コード確認済み・`docs/TODO.md`の「③軽減競合逆方向警告=✅済」はPC版のみを指しており記録が不正確だった)。ユーザーの見立て(PCのような絶対位置アニメーションが使えないので同じ実装を移植できない)は構造的に妥当: スマホは時刻ごとの行(`MobileTimelineRow`)にアイコンを横並びさせる方式で、PCの「かぶった位置で光る」という表現がそのまま持ち込めない。ただし検出ロジック自体(`conflictingIds`という単純なID集合)は位置に依存しないため、個別アイコンへのリング/バッジ等の代替表現は技術的には可能と見られる。次セッション: UI変更を伴うためbrainstormingスキルでモバイル向け表現を検討してから実装。
