...
- b-2 の難所＝ジョブ変更が `timelineMitigations` に波及（フィルタ／別ジョブ移行／学者・占星の自動挿入。[useMitigationStore.ts:1063-1105](../../../src/store/useMitigationStore.ts#L1063-L1105)）。`timelineMitigations` は②-a が所有するため、b-2 はこの継ぎ目（store 計算結果の mitigation 差分を②-a ハンドラ経由で流すか／b-2 側で mitigations も転写するか）を改めて設計する。
- **b-1 が mitigations Y.Array に触れるのは `importTimelineEvents`（取込＝別の戦闘に切替で mitigations を空クリア。[useMitigationStore.ts:610](../../../src/store/useMitigationStore.ts#L610)）の 1 経路のみ**。これは「全消し」という破壊的全置換で、②-a の id 単位マージとは継ぎ目で衝突しない（バルクハンドラが同一 transaction 内で mitigations Y.Array を `delete(0, length)` する）。それ以外の b-1 操作は mitigations に触れないため b-2 を阻害しない。
