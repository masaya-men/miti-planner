...
- **✅ 本番デプロイ済(詳細→[TODO_COMPLETED.md](./TODO_COMPLETED.md))**: スプシ取込一式(6/22 `a6af27c0`)/⑦敵攻撃or 2択(6/23 `6471e816`)/スプシ取込バグ修正一式(6/23 `ca98bd32`)/**取込フローv2前半+①誘導型ウィザード(6/23 `50c64d7d`・SDD+opusレビュー済)**。**後追い候補**=スプシ「A or B」自動分割→altName / **攻撃名見切れマーキー(ホバー中だけ横スクロール・確定方針=`docs/superpowers/specs/2026-06-22-event-or-attack-design.md`§5)** / マントラ取込直後の一過性非発光(軽微) / 取込ウィザードの`no_phases`時 理由非表示(旧来同挙動)。
- **🟣 取込フロー v2 ③対象引き継ぎ=spec+plan完了・実装待ち(ブランチ `feat/import-target-carryover`・未merge)**: テンプレ在るコンテンツで、スプシ取込の攻撃名→テンプレ技を照合し**対象(MT/ST)だけ**を引き継ぐ。**確定=精度優先**(正規化[括弧除去/NFKC/空白]+スプシ別名のみ・**編集距離の曖昧一致はやらない**=誤付け有害)/対象のみ引継・上書きしない/テンプレ無・未マッチ・等距離衝突は空/管理画面に「スプシ表記」列+「対象マッチ確認」モーダル(照合は取込時と同一関数)。spec=`docs/superpowers/specs/2026-06-23-import-target-carryover-design.md` / plan=`docs/superpowers/plans/2026-06-23-import-target-carryover.md`(4タスク・完全コード)。**次=subagent-drivenで実装→実機(管理:別名保存+マッチ確認 / ユーザー:テンプレ有取込で対象入る)→merge+push**。②プレビュー画像=**検証完了/本番OK確定**(対応不要)。②途中取込=spec§7繰越・別タスク。ウィザードfollow-up(任意・別タスク)=step4で`no_phases`時の理由非表示(旧来同挙動)/skipped表示のamber直値→トークン化。
