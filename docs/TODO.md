...
- **🔴🔴 共同編集=本番公開試行→重大バグ→ロールバック (2026-06-11・最優先・次セッション必読)**: collab を **admin gate**(管理者のみ共有2択表示・一般はコピー直行)で本番公開試行 (main ff510af・worker deploy・Vercel deploy・CSPにworker wss追加)。実機で重大バグ「**共同編集の表を開いてから別の表を開くと壊れが引き継がれる(横に列増殖=partyMembers)**」発覚→**即ロールバック (`vercel promote lopo-r0zqim9ke` で本番を共同編集前に固定・安全)**。**根本原因=`usePlanStore.setCurrentPlanId`([src/store/usePlanStore.ts:183](../src/store/usePlanStore.ts#L183)) が collab を切断しない**(別プラン開きでも `_collabActive` のまま→別表の読込がY.docに流入し増殖。disconnectは`revoke`時しか呼ばれない)。**被害=端末メモリのみ・リロードで直る・Firestore/localStorage無傷=データ破壊ゼロ**(ユーザー確認済)。⚠**git main=ff510af(collab全部+gate+UI改善+バグ3+CSP)・push禁止(次pushでpromote上書き=再発)**。修正方針=collabライフサイクルをプランに紐付け(別表開く/モーダル閉じ/離脱で必ずdisconnect+ローカルrehydrate)→徹底検証してから再公開。残UI課題=①カーソル時間ベース補間(lerp飛び飛び) ②ジョイナーにヘッダー/サイドメニュー+操作は表のみ+説明 ③赤バナー下。**詳細=[docs/.private/2026-06-11-collab-rollback-root-cause.md](./.private/2026-06-11-collab-rollback-root-cause.md) + memory `project_realtime_collab_status`**。
