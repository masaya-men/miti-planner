...
## 現在の状態 (次セッションはここから読む)
### 🟡 2026-08-19 ハウジング3機能+軽減表スキル2件、全部コミット済み・未push(11コミット)。ユーザーがAllmarksをさらに改善中→終わり次第まとめてpush→デプロイ→Discord告知
①**表示メンバー絞り込みスイッチ**・②**ハウジンガー短縮共有URL**は実機確認OK済み→`TODO_COMPLETED.md`参照。
③**Allmarksまとめてインポート**: 本体機能・「1件も取り込めない」不具合修正・リージョン混在時の選択肢UIまで実装完了。**進捗インジケーターは紆余曲折の末ユーザー提供デザイン(波形バー+跳ねるボール+右端の家に衝突)に決着・ユーザー確認「完璧」**(地図描画演出は試作→不採用で完全撤去)。DEV専用プレビュー`/housing/dev/allmarks-animation`で本番と同じモーダルを使って確認可能。tsc/build/全テスト(520ファイル超)毎回グリーン。**vmThreadsハング事故が2回発生→both修正・[[reference_vitest_vmthreads_hang]]に教訓追記済み**。詳細=`docs/.private/2026-08-19-allmarks-import-handoff.md`(追記3〜8)。
**残**: リージョン混在ケースを実際のAllmarks共有リンクで実機確認(2リージョン混在の共有をまだ作っていない)/ インジケーターの本番一時ツアー画面での実動作確認(devプレビューはダミー進捗値)。
**🔵進行中**: ユーザーがAllmarksにさらに改善を加える予定(次回セッションで内容判明)。それが終わってから下記の残タスク+全体pushへ。
**残(Allmarks)**: リージョン混在ケースを実際のAllmarks共有リンクで実機確認(2リージョン混在の共有をまだ作っていない)/ インジケーターの本番一時ツアー画面での実動作確認(devプレビューはダミー進捗値)。
④**軽減表: 賢者「リゾーマタ」・リーパー「アルケインクレスト」追加**(コミット済み・詳細はTODO_COMPLETED.md参照)。
**⚠️全体共通の残・重要**: 上記まとめてmain push→deploy後、`npx tsx scripts/seed-icons.ts`→`npx tsx scripts/seed-skills-stats.ts --dry-run`で差分確認→本実行、の順でFirestore反映必須([[reference_skill_add_rollout]]、seedを先に行わない)。これを忘れるとmockData.tsに追加した2スキルがdev/本番とも表示されないまま放置される。
