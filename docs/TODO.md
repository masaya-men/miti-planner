...
### 次セッションで実機検証すること（致命バグ修正 + 自動採番 2026-04-28 後半）
- [ ] 野良主流ボトムシートから FRU コピー → 雲同期 → プランが消えないこと（最大の確認項目）
- [ ] 同じプランを 2 回コピー → 2 つ目が `タイトル (2)`、3 つ目が `(3)` になる
- [ ] サイドバー「下にコピー」も同コンテンツ内のみで採番されること
- [ ] 別コンテンツに同名プランがあっても採番に巻き込まれないこと
- [ ] PopularConsentDialog がライトモードで白背景になっていること（en/zh/ko も）
