...
### 次にやること（最優先）
- 管理ダッシュボード: ユーザー数・プラン数を表示するシンプルな画面（詳細はTODO下部）
- デプロイ確認: フェーズ名修正（新規プラン要再作成）、コンテンツ追加修正
...
### 管理画面改善
- [ ] 管理ダッシュボード（シンプル版）
  - API: Firebase Admin `listUsers()` でユーザー数、Firestore `plans` 集計でプラン数
  - 表示: ログインユーザー数、総プラン数、ユーザー別プラン数
  - 外部リンク: Firebase Console / Google Analytics / Vercel Dashboard（API取得不可データ用）
  - 注意: 非ログインユーザーはAnalyticsでしか見れない。簡素でOK
