...
- **✅ 本番デプロイ済(詳細→[TODO_COMPLETED.md](./TODO_COMPLETED.md))**: スプシ取込一式(6/22 `a6af27c0`)/⑦敵攻撃or 2択(6/23 `6471e816`)/スプシ取込バグ修正一式(6/23 `ca98bd32`)。**後追い候補**=スプシ「A or B」自動分割→altName / 攻撃名見切れマーキー / マントラ取込直後の一過性非発光(軽微)。
- **🟣 取込フロー v2前半(①+③)=実装完了・実機確認待ち(ブランチ `feat/import-flow-v2-phase1`・未merge/未push)**: 全6タスク+最終レビューfix完了。tsc0/build成功/対象テスト37緑。spec=`docs/superpowers/specs/2026-06-23-import-flow-v2-phase1-design.md` / plan=`docs/superpowers/plans/...phase1.md`。実装=`contentSelection.ts`(NewPlanと共通化)/`importWithLimitCheck.ts`(満杯ゲート=既存`LimitResolutionSheet`流用)/`LimitResolutionSheet`マウントをLayout一元化/取込モーダルにコンテンツ選択UI+onImport async/Timeline配線(誤紐付け根治)。**最終レビューでCritical=満杯解消後にshare取込storeのstatus汚染で空ShareImportSheetが幽霊化→fix済(commit47d1aa54・shareWasIdleガードでclose()復帰+回帰テスト2)+独立再レビュー clean**。**ユーザー実機テストで2バグ→fix済→実機OK確認済(2026-06-23)**: ①picker未プリセレクト→`resolveInitialSelection()`でcontentId優先復元 ②取込先に別コンテンツ選択しても確定時に現在コンテンツへ巻戻り→初期化effectを「開いた瞬間のみ」(dep[isOpen]+ref)に限定。**次=本番前ブラッシュアップ3点(詳細→`docs/.private/2026-06-23-import-flow-v2-brushup.md`)→仕上げ後merge+push(=本番自動デプロイ)**: ①取込モーダル誘導型ウィザード化=**brainstorming完了→spec作成+コミット済(`e816b23d`)・spec=`docs/superpowers/specs/2026-06-23-import-guided-wizard-design.md`**。確定判断=フルウィザード4/3ステップ(①設定→②貼付ループ→③パーティ※軽減も&ジョブ検出時のみ→④確認)/貼り方ガイド常時表示(「A1クリック→Ctrl+A→Ctrl+C→Ctrl+V」)/フェーズ名任意(空→`Phase N`実体化が必須=`buildPlanFromSheets.ts:46`が`s.phaseName`をそのまま使うので空だと生成プランのフェーズ名が空になる)/黄(未追加)赤(未割当)ゲート移植/i18n4言語/**ロジック不変のpresentation刷新**。**実装完了=SDD4タスク全緑(plan→`importWizard.ts`純粋遷移/i18n4言語/モーダルwizard化[既存effect/memo/handler不変をopusレビューでbyte-identical確認]/フェーズ名任意化)・最終全体レビュー(opus)=Critical/Importantゼロ・head`25f07f41`(未merge)。tsc0/build成功/新規32緑(全vitest既知失敗5のみ)**。**次=ユーザー実機E2E1回(軽減も4step/タイムラインだけ3step/空名→Phase1実体化/黄赤ゲート/4言語/既存2バグ非再発)→OKで merge+push(本番自動デプロイ)**。follow-up候補(別タスク)=step4で`no_phases`時に理由非表示(旧来同挙動)。②プレビュー画像=**検証完了/本番OK確定**(dev/本番とも/icons/実測200・本番rewrite配信。devの壊れは存在しないアイコンのindex.html(200/html)フォールバック=本番無影響・merge非ブロッカー)。③攻撃に対象(MT/ST)が入らない→テンプレ持ち越し=**未着手(brainstorming待ち)**。②途中取込はspec§7繰り越し・別タスク。
