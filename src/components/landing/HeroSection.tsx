...
export function HeroSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      entryTl.fromTo(taglineRef.current,
        ...
        { opacity: 1, y: 0, filter: 'blur(0px)', duration: 1.0, ease: 'power3.out' }, 0.9);
      entryTl.fromTo(taglineSubRef.current,
        { opacity: 0, y: 20, filter: 'blur(8px)' },
        { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.8, ease: 'power3.out' }, 1.1);
      entryTl.fromTo(ctaRef.current,
        { y: 40, opacity: 0, scale: 0.9 },
        { y: 0, opacity: 1, scale: 1, duration: 0.8, ease: 'back.out(1.7)' }, 1.2);
      ...
      if (scrollLine) gsap.to(scrollLine, {
        scaleY: 1.5, opacity: 0.3, duration: 1.2,
        ease: 'power1.inOut', yoyo: true, repeat: -1,
      });

      // スクロール駆動
      gsap.to(contentRef.current, {
        scale: 1.3, z: -2000, opacity: 0, filter: 'blur(15px)',
        scrollTrigger: {
  ...
  const logoLetters = t('portal.title').split('');

  return (
    <section ref={sectionRef} className="h-screen w-full flex items-center justify-center overflow-hidden">
      <div
        ref={contentRef}
