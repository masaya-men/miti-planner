...
### ✅ ツアー追加フィードバックアニメーション(2026-08-10・実装完了・未push・実機確認待ち)
探すページのカード/詳細ページで「ツアーに追加」を押しても成功/失敗の手がかりが無い問題への対応。成功=チェックマーク描画+「追加済み」トグル状態(もう一度押すと外れる)、失敗(別リージョン)=ボタン単体シェイク+ボタン直上の吹き出し(画面下中央の汎用トーストは置き換え)。設計書=`docs/superpowers/specs/2026-08-10-housing-tour-add-feedback-design.md`・実装計画=`docs/superpowers/plans/2026-08-10-housing-tour-add-feedback.md`。サブエージェント方式(7タスク)で実装、各タスク後にレビュー+最終の全体差分レビューまで完了(本物のバグ計5件を検出・修正: hookの未使用import/エラーメッセージ残留、吹き出しrefの型エラー、吹き出しの色トークンがdocument.body配下で未解決、画面端クランプ漏れ、トレイ削除時のピン残留。ほかレビュー誤検知1件は実差分照合の上却下)。既存2ファイル(BrowsePage/FavoritesPageの旧トースト前提テスト)も新仕様に更新済み。**次**: ユーザー実機確認(`npm run dev`で探す/詳細ページの成功・失敗パターン)→OKならまとめてpush。
