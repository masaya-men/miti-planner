...
| `src/components/ShareModal.tsx` | updateShareLogo関数追加、変更ボタン廃止、生成中ブロック、エラー時復旧 |
...
| `src/locales/ja.json` | plan_deleted_remotely追加 |
| `src/locales/en.json` | plan_deleted_remotely追加 |
...
**本番デプロイ済み（2026-03-29）。以下の手動テストが未完了:**

### 共有モーダルテスト
- [ ] ロゴなし → ドラッグ&ドロップ → プレビューに反映される
- [ ] ロゴあり → 別画像ドラッグ&ドロップ → 新しい画像に置き換わる
- [ ] 削除ボタン → ロゴが消える → プレビューからも消える
- [ ] 追加ボタン → ファイル選択 → ロゴが表示される
- [ ] ロゴトグルOFF → プレビューからロゴが消える
- [ ] ロゴトグルON → プレビューにロゴが戻る
- [ ] 画像生成中に共有ボタンが無効化されている
- [ ] 共有URLを別タブで開く → OGP画像が正しい

### 端末間同期テスト
- [ ] PCで表を削除 → 別ブラウザをリロード → 表が消えている（復活しない）

---

## コードベースの健康状態（第39セッション調査結果）

| 項目 | 状態 | 備考 |
|------|------|------|
| テストフレームワーク | ❌ 未導入 | vitest導入をTODOに追加済み |
| i18nハードコーディング | ⚠️ 19ファイルに残存 | 全体精査が必要 |
| React.memo | ❌ 大規模コンポ未対応 | Timeline(2482行)等。視覚変更後に対応予定 |
| public/icons/ | ⏳ 削除予定 | 2.1MB削減。全動作確認後に実施 |
| ハウジング機能 | 📋 要件定義のみ | コード実装未着手 |
...
### 残りのタスク（優先順）
1. **デザイン改善 + アクセントカラー** — モーダル・各画面のライトモード修正
2. **パフォーマンス最適化** — React.memo / useMemo（全視覚変更後）
3. **public/icons/ 削除** — 2.1MB削減（最後に実施）
4. **テスト基盤導入** — vitest（タイミングは柔軟）
5. **軽減配置フィードバックアニメーション** — 良いアイデアが出るまで保留
