...
for (const line of lines) {
    ...
    if (line.startsWith('|') && !line.includes('Patch') && !line.includes(':---')) {
        ...
        if (parts.length === 4) {
            ...
            const ja = parts[1];
            const en = parts[2];
            let shortRaw = parts[3].replace(/<[^>]+>/g, '').trim(); 
            let id = shortRaw.toLowerCase().replace(/\s+/g, '_');
            
            let category = 'savage';
            if (en.includes('Ultimate') || ja.includes('絶')) category = 'ultimate';
            
            parsed.push({ id, patch, ja, en, shortNameJa: shortRaw, category, level: currentLevel });
        }
    }
}
...
fs.writeFileSync('src/data/contents.ts', tsContent);
...
for (const c of parsed) {
    ...
    enLocale.content[c.id + '_short'] = c.shortNameJa; 
}

fs.writeFileSync(jaFile, JSON.stringify(jaLocale, null, 4));
fs.writeFileSync(enFile, JSON.stringify(enLocale, null, 4));
