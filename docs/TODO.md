...
- **🟢 現在の作業スレッド=リアルタイム共同編集 (branch `feat/collab-stage4a-server-edit-auth`・held・push/deploy保留・UI非露出)**: ④-a サーバ側編集認証 実装完了 (2026-06-10)。**次セッションはここ→④-b(P2Pカーソル・動く実装+ジョブアイコン+色・実名なし・$0)を brainstorming から設計**。その後 ⑤-3d+④ 統合プレビュー実機検証 (login要・localhost auth不可)。詳細は下記 collab バックログ + memory `project_realtime_collab_status`。
