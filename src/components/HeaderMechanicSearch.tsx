                    ...
                    <div className="p-2 border-b border-glass-border">
                        <div className="flex items-center gap-2 px-2 py-1.5 rounded-md bg-app-bg border border-app-border">
                            ...
                            <input
                                ...
                                className="w-full bg-transparent text-[13px] text-app-text placeholder:text-app-text-muted focus:outline-none"
                            />
                        </div>
                    </div>
                        ...
                        {filtered.length === 0 ? (
                            ...
                            filtered.map(entry => (
                                <button
                                    ...
                                    className={clsx(
                                        "w-full px-3 py-2.5 text-left text-[13px] border-b border-glass-border last:border-b-0 cursor-pointer transition-colors flex items-center justify-between",
                                ...
                                >
                                    ...
                                    <span className="text-[9px] text-app-text-muted flex-shrink-0 ml-2">
                ...
                {selectedMechanic && (
                    <div className={clsx("flex-shrink-0", typeof window !== 'undefined' && window.innerWidth < 768 ? "w-full border-t border-glass-border" : "w-[180px] border-l border-glass-border")}>
                        <div className="px-3 py-2 bg-glass-header border-b border-glass-border">
                            <span className="text-[11px] font-bold text-app-text truncate block">
                                {selectedMechanic}
                            </span>
                        </div>
                        <div className="max-h-[340px] overflow-y-auto">
                            {mechanics.find(m => m.name === selectedMechanic)?.occurrences.map((occ, i) => (
                                <button
                                    ...
                                    className="w-full px-3 py-2.5 text-left text-[13px] text-app-text hover:bg-glass-hover border-b border-glass-border last:border-b-0 cursor-pointer transition-colors flex items-center justify-between"
                                >
                                    <span className="text-app-text-muted text-[9px] truncate">{occ.phaseName}</span>
                                    <span className="font-mono text-[11px]">{occ.formattedTime}</span>
                                </button>
                            ))}
                        </div>
                    </div>
