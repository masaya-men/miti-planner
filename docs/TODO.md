...
- **📌 次セッション最優先 = YouTube ライブ配信対応** (動画モーダル拡張): 参考 `https://www.youtube.com/live/<id>`。**現状 `parseYouTubeId` が `/live/` 未対応** ([src/utils/youtube.ts:20](../src/utils/youtube.ts) の path 正規表現が `embed|shorts|v` のみ) → `live` を追加 + `youtube.test.ts` にケース追加が起点。アーカイブ(配信後VOD)は通常動画として既存ロジックで動くはず／真のライブ中は `getCurrentTime`=配信開始からの経過秒で相対タイマー機能 (DVR窓内のみ seek)。実機で参考URL検証。
