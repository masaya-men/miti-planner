...
## 現在の状態 (次セッションはここから読む)
### 🔴 次セッション最優先: スマホでスクロールが重い(軽減表・ハウジング両方、連動エフェクト表示時)未調査
2026-08-14セッション終盤、ユーザーが実機で「軽減表もハウジングもスクロールがすごい重く感じた(スマホ、連動エフェクト表示のとき)」と報告。**まだ原因調査に着手していない**(症状の記録のみ)。次セッションはsystematic-debuggingスキルで根本原因の特定から開始すること。「連動エフェクト表示」は軽減表モバイルの`mobileEffectBarMode:'scroll'`(スクロール連動でアイコン⇄エフェクト棒が変身する演出、[[reference_playwright_miti_overlays]]参照)を指すと思われるが、ハウジング側で同種の演出は無いはずなので、ハウジング側は別原因の可能性もあり要確認。まず実機で再現条件を切り分ける(どちらのアプリで/どの操作で重いか)。
