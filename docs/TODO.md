...
- **今回の作業**: create-plan/shareチュートリアル完成 + 軽減UIアイコン表示 + 多数のフィードバック修正
- **最優先**: 下記の残チュートリアルバグ修正 → main マージ → デプロイ
- **完了済み**: create-plan（10ステップ、タイプライター演出）、share修正（2ステップ）、軽減UI（アイコン並び）、文言最終調整済み
- **変更点**: add-event廃止→create-planに置換。shareバグ修正。「N Selected」→アイコン。completion簡略化
...
- [ ] チュートリアル: main STEP11 ピル飛行後のバウンスが激しすぎる
- [ ] チュートリアル: 途中でリロードするとテンプレートタイムラインが残る
- [ ] チュートリアル: 開始時に最新レベルを開いていないと壊れる → 自動で最新レベルを開く
- [ ] サイドバー開閉ハンドル右側の縦線アピールが正しく出ない（明るい白にならない）
- [ ] Pモーダル: ジョブアイコンの罫線を消す + 上の説明を日本語時も一行にする（ガタつき防止）

## 進行中
- [ ] チュートリアル全面刷新 — `feature/tutorial-overhaul`ブランチ。残り: 上記バグ修正 → mainマージ → デプロイ
