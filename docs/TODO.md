...
### 🟡 マイページ ブラッシュアップ = 実装済・デプロイ待ち(2026-07-24、3巡)
実機FBを3巡で反映。①右カラムのみ内部スクロール(ツールバー固定・カードだけ動く。1回目はpanel全体スクロールに誤って直し2回目で修正) ②名前横の鉛筆→その場編集+アイコン変更バッジ ③アイコン変更モーダルを housing 独自トンマナの新規 `HousingAvatarCropModal.tsx` に差し替え(HousingAccountModal.tsx 側も統一) ④「ハウジンガー公開」説明文+表示名タグ注記(4言語) ⑤「ひとこと」欄 textarea化(自動拡大) ⑥「まとめてツアー」は本人閲覧時は非表示 ⑦「公開をやめる」→「ハウジンガー公開をやめる」に文言明確化(4言語) ⑧左カラムのスクロールバー見た目調整(余白+細め) ⑨マイページの一覧カードから専用フッター行を撤去し、右上=公開状態バッジ(お気に入りハートと排他)・右下=編集ボタンを画像タイル上に直接オーバーレイ配置(ListingCard.tsx、表示件数増)。型チェック/ビルド/既存テスト/Playwright実機検証済み。ログイン必須部分は本番デプロイ後にユーザー実機確認予定。**次セッション最優先候補**=③タグAND検索機能(要brainstorming)。
