...
- **✅段取り③(Firestore恒久保存)コード実装 main マージ済(2026-06-04)=[plans/2026-06-04-realtime-collab-stage3-firestore-persistence.md](./superpowers/plans/2026-06-04-realtime-collab-stage3-firestore-persistence.md)**(設計書 [specs/...stage3...design.md](./superpowers/specs/2026-06-04-realtime-collab-stage3-firestore-persistence-design.md))。方式=**案B(DOはVercel受付係 `/api/collab/load`・`/api/collab/save` に保存委譲・共有シークレット認証・既存保存ロジック再利用)**。DO `onLoad`(seed)/`onSave`(debounce 5s/15s書き戻し)/`onClose`(最後の退室で最終flush)+**破壊保存ガード**(seed失敗時は保存しない)+**墓標ガード**(削除が勝つ・復活させない)。seedはサーバー(onLoad)が正→client seed撤去。room鍵=plan IDのまま(トークン分離は⑤)。テスト=純粋ロジック9+変換3+永続化HTTP7+既存6 全緑・build緑。**🔴Task10未了=本番稼働化**: ①`COLLAB_SHARED_SECRET`をVercel(sensitive)+Cloudflare(`wrangler secret put`)に同値設定 ②`cd workers/collab && wrangler deploy`(現本番Workerは②-aのまま=onLoad/onSave無し) ③Claude node 2クライアント結線確認→ユーザー2ブラウザ。**シークレット未設定でもapi/collabは401 fail-closedで安全・UI非表示継続**。
- **段取り⑤(③の後)=共同編集の実入口**: 重要発見=**共有はコピー作成で別ID化([MitigationSheet.tsx:209](./../src/components/MitigationSheet.tsx#L209))→現状plan IDを部屋鍵にしても他人と繋がれない**。業界標準=「共同編集リンク発行→同じ部屋に参加(コピーでない)→オーナー不在でも編集可」。⑤で**ルームトークンをplan IDと分離・入口は共有ボタン内・既存コピー共有と別物**・ログイン必須化・人数上限・緊急停止。**完成までUI一切非表示厳守**(ユーザー指示)。順序: ③(保存)→⑤(入口)→②-b(他要素)→②-c(Undo)。
