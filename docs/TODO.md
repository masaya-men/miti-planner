...
- **🔴 探す地図フィードバック: ①③④✅ / ②機能=ユーザー実機OK・パネル角=解決(実機OK)→リリース待ち** (2026-07-12)。詳細=`docs/.private/2026-07-12-browse-map-feedback.md`。**左上の四角い出っ張りの真因=パネル裏の区画ピルバー(明るい選択ピル)が半透明パネル越しに透けていた**もの(前セッションの「backdrop-filter 角丸クリップ不良」は誤診)。解決=**ぼかし復活+パネル角丸を裏ピルバーと同一半径に**(共有トークン `--housing-mapctl-areas-radius:9px` を `.housing-mapctl-areas` と `.housing-bmap-roompanel` 両方が参照)。build✅・ユーザー実機OK。この後は big3 全体のリリース前検証へ(下記 2 行)。
  ...
  - **残(リリース前)**: **②タブ削除=✅完了**(`573eaa24`) / **④跨ぎ処理=✅実装完了+2026-07-13実機FB一巡(注記DC直下+ロック/使い捨て説明を上部/フィルターフェード+DC→データセンター/建物タイプ公式表記/自作▲▼ステッパー/ヘッダー検索撤去/号棟横並び/roomKind配線漏れバグ)→dev目視待ち・個室ツアーはフォローアップ・詳細=`.private/2026-07-12-big3-release-verification-checklist.md`**(commit `3ff3dde`〜`485c33d9`・全vitest3120緑+build緑+最終レビューCritical0。目視項目=`docs/.private/2026-07-12-big3-release-verification-checklist.md` の ④節。設計/計画/台帳=`docs/superpowers/{specs,plans}/2026-07-12-housing-tour-region-crossing*`+`.superpowers/sdd/progress-tour-crossing.md`。決定=リージョン越え不可(追加時block+開始時net)/DC=DCトラベル指示/ワールド=ワールド訪問指示+中央マップぼかし案内/一時追加もワールド必須化) / **PF(ハウジンガーPF)は本番デプロイ後にログインで実機**(login本番専用)。**リリース=big3全体をmain反映+`firebase deploy --only firestore`(rules+indexes)+ダミー150件削除+push**(④dev目視+PF後・要ユーザーGO)。**隣接課題(別)**: 詳細「他の部屋」が limit20/50 で実最大(90/512)未満に truncate。
