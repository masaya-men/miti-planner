...
### 次にやること（優先順）
- **【最優先・次セッション】広告に関する方針議論**: ユーザー方針転換「グレーでも広告入れていいかも」。論点: ①MUL 違反リスク（SE 公式アイコン使用ゆえ寄付はグレー、広告は明確に違反、ただしグレー受け入れる前提でどうか）②業界事例の事実確認（FFLogs / Triple Triad / ACT 等の広告状況）③収益化目的の試算（Vercel Pro $20/月 + 共同編集を将来入れた時の追加コスト）④代替案（寄付強化 / マイコラージュ収益で LoPo 維持 / 別アプリで広告）⑤入れるなら配置・密度・無効化条件・SE 削除依頼時の対応方針。memory `project_lopo_mul_constraint.md` も更新検討
- **マイコラージュ（別プロジェクト）**: ユーザー優先指示継続。広告議論と並行可能（別プロジェクトなので）。完了次第ハウジング着手
- **PWA UX 改善（後回し決定 2026-05-03）**: ①iOS Safari でのインストール案内バナー（現状 beforeinstallprompt も独自バナーも無し）②アプデ通知トースト（自動更新は動作中だが完全終了→再起動が必要なことをユーザーに伝える UI 無し）③ログイン前の注意ガイド（PWA で「Safari で開く」を押すと state mismatch になる旨）。実装規模 2-3 日。広告とマイコラージュ後
