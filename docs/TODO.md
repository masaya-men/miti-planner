...
- **ブランチ**: main直接
- **デプロイ済み**: テンプレート保護 + エディタ一括編集 + ツールチップ言語対応 + zh/ko表示全対応 + キャッシュ自動無効化 + フェーズ・ラベル編集UI + チュートリアル修正
...
- **修正済み未確認**: フェーズ編集 — Sidebar.tsxのLocalizedString変換修正、管理画面の境界判定修正（削除・JA優先表示）
- **実装済み未確認**: チュートリアル：既存プランありコンテンツでも進行可能に
