...
## 現在の状態 (次セッションはここから読む)
### ✅ 2026-08-01: OGPカードの技術的不具合を全て根治・本番push済(`4eaafb99`)
2026-07-31の3大バッチ(タグAND検索/繁体字/OGPカード作り込み)後、実機確認で発覚した問題を全て解消:
- WebP画像がsatoriで読み飛ばされる不具合(`9c1ee98b`、アバター148件/物件写真197枚バックフィル済み)
- YouTube動画サムネ404(`abde9fd5`→`4930dd7d`で解決順も修正、保存済みogImageUrlが古いmaxresdefaultのままだった既存データ問題)
- **satoriが`inset:0`省略記法を描画できず背景ぼかし画像・暗幕が一切表示されていなかった根本原因を特定・修正**(`71d3f619`、ローカルでsatori直呼び出しして切り分け。ツアー招待カードの暗幕も同一原因で修正)
- 背景ぼかしが強すぎた調整(`4eaafb99`、blur32px→10px)
- スマホのマイページ一覧見出しの縦潰れ(`f81a7049`)
**運用上の重要な学び**: OGPカードには3段のキャッシュがある(①Cloudflare edge ②Firestore `og_image_meta`のパラメータキャッシュ ③**Storage `og-images/*.png`に永続保存された生成済み画像=パラメータが同じ限りコード修正が反映されない**)。③のクリアには`scripts/purge-og-image-cache.ts --apply`が必須(Cloudflareパージだけでは効かない)。カード生成コードを直すたびに実行すること。
build+vitest OK、既知の無関係failure=EphemeralAddPanel 7件のみ。

### 🔴 次セッション最優先: OGPカード、ユーザー作成モックアップ3案の実装
技術的不具合が直ったところで、ユーザーが3パターンのデザイン案を自作(写真コラージュ+金枠+縦書き/横書き発光テキスト)。「完璧に再現してほしい」との依頼、次回着手。詳細・構図メモ・技術検証済み事項(縦書き回転テキスト+発光効果は動作確認済み)=`docs/.private/2026-08-01-ogp-card-design-mockups.md`。**画像3枚は再送付が必要**(この会話にのみ添付・未保存)。
