...
## 現在の状態 (次セッションはここから読む)
### 🔴 次セッション最優先: モバイル軽減表 競合アイコン右はみ出しバグ = 修正実装済・実機確認待ち(push保留中)
①レイアウト刷新・②競合(CDかぶり)表示は実機確認済み。**残る1件のバグは今回原因特定→修正完了、実機での最終確認だけ残っている**(詳細・解決済みの派生バグ4件は[TODO_COMPLETED.md](./TODO_COMPLETED.md)の2026-08-14エントリ参照)。

**🐛 スクロール連動(連動)モードで競合中のアイコンだけ画面右にはみ出て表示される** — 1回目の調査(`4fcaaa67`)はユーザー実機で否定済み、2回目の合成再現(Playwright+WebKit)でも右はみ出しは再現できず「実機固有かも」と保留していたが、**ユーザーが実機スクリーンショットを送付**(0:04行の一番右、黄色リング付きアイコンの切れ端が画面端で物理的に切れている)し、位置計算とも一致して原因特定に成功。
**根本原因**: `.mobile-effect-bar-icon`(競合エフェクト棒アイコン)は本来「スクロールしていない静止時はopacity:0で完全に隠す」設計だが、競合中に付与する`.animate-conflict-pulse`(黄色パルス点滅)がCSSアニメーションとして`opacity`/`transform`を丸ごと上書きしてしまい、**競合中のアイコンだけ静止時も透明度0にならず、変身前の待機位置(画面右端そば)にうっすら常時居座っていた**。通常アイコンにはこのアニメが付かないため症状が出ない。
**修正**(`src/components/MobileEffectBarLayer.tsx` + `src/index.css`): 競合中のみアイコンを`.mobile-effect-bar-icon-slot`ラッパーで包み、脈動アニメはラッパー側、進み具合連動のopacity/scaleは中のアイコン側に分離。2つの要素のopacity/scaleが掛け算で合成されるため、静止時は従来通り完全に不可視に戻る(パルス効果自体は表示中そのまま維持)。Playwrightで検証: 修正前は静止時に競合アイコンのopacityが0にならなかった箇所を確認済み、修正後は1周期(1秒)通してopacity=0を維持することを確認。tsc/build/関連vitest(既存の無関係な2件の失敗のみ、修正前から存在=無関係と確認済み)は通過。
**次セッションの最初にやること**: ユーザーに実機で①競合が右にはみ出さなくなったこと②パルス点滅自体は今まで通り見えることを確認してもらう → push前ゲート(build+全テスト)→push→デプロイ→本番実機で最終確認。
