...
- **✅ 2026-06-18 表の情報列固定(PC横スクロール時) main マージ&本番デプロイ済 (cc4674c)**: 横スクロールで左の情報列を sticky 不透明ペイン(`.timeline-info-pane` z=60・`TimelineInfoColumns`)へ分離、スキル列だけスクロール(Excel枠固定)。軽減バー(z≤30)非はみ出し。`TimelineRow`=スキルセル+折りたたみ追従スペーサー。**自由スクロール追加**(末尾メンバーを固定列の真横まで・絶対スペーサーで sheetWidth 不変)。実装=subagent-driven(7タスク)+レビュー。**実機QAで派生回帰5件発見→全修正**: ①ヘッダー/バーの侵食(不透明z10カバー)②軽減アイコンが pane 下に隠れ(memberLayout.left offsetParent累積補正・Task2由来)③行hover消失(.timeline-row-hover 相互同期)④sheetWidth膨張でメモ/カーソルズレ予防(絶対スペーサー)⑤他者カーソルが pane 裏(z 40→70)。**徹底監査(15挙動 main比較+敵対検証・workflow)=BROKEN1(=⑤修正済)/OK14**。データ層無改変=消失リスク増なし(data/collab テスト66緑)。**🔴 残=ユーザー本番実機**: 配置/hover/攻撃名クリック/最右端スクロール、**共同編集2タブで情報列上の他者カーソル可視**。非回帰の見え方メモ: 競合矢印が列のペイン潜りで隠れる/極小xRatio旧メモ pane裏(既存データ起因)→気になれば対応。
...
- **🆕 ヘッダーのジョブアイコンをドラッグで並び替え (2026-06-18 ユーザー質問・難易度=中)**: 8人パーティの列順を表ヘッダーのドラッグで自由入替。表示は `sortedPartyMembers` 順が単一ソースなので並びは概ね自動追従するが、**カスタム順を新規に保存+collab同期する必要があり=データ層に手が入る**(既存の並び替えプリセット light_party/role別 との関係整理も)。ドラッグ自体は既存の軽減ドラッグ流用可。**着手時 brainstorming**。
