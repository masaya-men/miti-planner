...
## 現在の状態 (次セッションはここから読む)
### 🔴 次セッション最優先: スマホ版ツアー招待のログイン導線UX改善(2026-08-12・要相談・未着手)
PC版はツアー画面の中央パネル下部に「ツアーへの招待にはログインが必要です」カード(`TourInvitePanel`/`HousingLoginPrompt context="tour"`)が常時表示されるが、スマホ版は`.housing-tour-invite {display:none}`(実機FB#7当時のCSS)で丸ごと非表示にしており、招待の入口は下部バーの人+アイコン(`UserPlus`)のみで説明が一切ない(ユーザー実機指摘で発覚)。ユーザー提案(未確定・要相談から再開): カードを複製せず文言調整で対応する方向。①未ログイン時はアイコン付近に「ツアーへの招待は右下のアイコンからログインを」等の一言テキストを常時表示 ②ログイン済み・未発行時、PC版`TourInvitePanel`にはあるがスマホには無い「ツアー名編集欄」を新設するか、それとも文言(ラベル/ヒント)だけで済ませるかが未確定(ユーザーへ確認中に区切り)。次セッション: まず②の実装スコープをユーザーと確定してから着手する。
