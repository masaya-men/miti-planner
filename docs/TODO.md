...
## 現在の状態 (次セッションはここから読む)
### 🆕 ツアー計画画面(蛇行グリッド)設計書+実装計画 完成・実装待ち(2026-08-11)
探す/お気に入りページの右トレイ(幅300px)だけでは50〜100件規模のツアーは並べ替えづらい問題と、「ツアー」タブの文言(「ツアー中」固定で不自然)・未開始時にトレイ内容と連動せずほぼ空表示になる問題をまとめてbrainstormingで解決。設計書=`docs/superpowers/specs/2026-08-11-housing-tour-tray-page-design.md`・実装計画=`docs/superpowers/plans/2026-08-11-housing-tour-tray-page.md`(共にコミット済み)。要点: タブ文言「ツアー」に変更/「ツアー」タブは未開始でもトレイに1件以上あれば計画画面(PC=左に選択中の家の詳細パネル+右に蛇行グリッドで全件表示・スマホ=縦一覧のみ)を表示/トレイ行カードから住所表示を削除しタイトルを大きく(サイドバートレイにも波及)/スマホの並べ替えシートは撤去し「ツアー」タブへ統合。実装計画は9タスク(i18n/共有ロジック抽出リファクタ/住所削除/蛇行位置計算のTDD/新規2コンポーネント/TourNavPage統合/スマホ導線整理/最終差分レビュー)構成、設計書の全決定事項との対応確認済み。**次: `subagent-driven-development`スキルで9タスクを実装 → 実機確認 → まとめてpush**。
