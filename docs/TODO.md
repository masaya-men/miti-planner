...
1. 🔴🔴 **ハウジング大規模耐性ハードニング + 住所非公開機能** (超大物・設計完成・承認済)
   ...
   - **✅ P0(緊急耐性)=本番稼働+ダッシュボード作業完了**(9タスク・最終opus Ready:Yes・build/test緑3275・複合index Ready・curl検証全PASS[レート制限/IP偽装不可])。**+登録UXフォローアップ本番済**(通報トースト文言/チケット残数を実描画RegisterPageに配線[孤児HousingQuotaIndicator修正]/管理者=登録無制限「管理者」表示)。**✅CFメディアキャッシュ(cf-cache HIT実測)+✅GCP読み取り急増アラート**。**残(非ブロッカー)**: 登録画面「管理者」+通報トーストの実機目視(任意)。**🔵P1送り**: tweet-video 25MB上限のRange回避/popular top-200窓の新着抑制。 **🟡 P1=実装完了・未デプロイ**(ブランチ`feat/housing-p1-windows`・未マージ): 版番号カウンタ+公開キャッシュ窓口(api/housing/public 4action・許可リスト射影)+client3経路(gallery/housinger/peers)切替+rules `allow list`をオーナー本人限定+housing_meta。build/test緑3285・射影/挙動保存を司令塔+opus最終レビューで確認済(admin hide/dismiss-one/SNS cron の版bump漏れ1件を検出→修正済)。**次=コードpush→窓口200確認→[G2]ユーザー実機(ops§5チェックリスト:共有リンク/閲覧/自分の登録編集削除)→rulesデプロイ→CF作業**。ops+チェックリスト=`.private/2026-07-14-p1-dashboard-ops.md`(HTML60秒CFルールもP0残タスクとして折込済)。その後=P1-M。
