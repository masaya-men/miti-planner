...
### 次にやること（最優先）
- **フェーズ/ラベル開始時間編集機能** → 実装完了（2026-04-13）、デプロイ待ち
- デプロイ確認: 管理画面フェーズ名編集UI（一覧形式）、FRUテンプレート修正→プラン再作成
- デプロイ確認: 開始時間編集機能（フェーズ/ラベルの開始時間変更、空白期間、エフェクト方向）
- **サイドバー大幅改修**（brainstormingから開始）
  - 課題: コンテンツ選択が面倒、選択肢多すぎ
  - 方向性: カテゴリ（零式/絶/その他）→ フラット表示
...
### フェーズ/ラベルのendTime必須化リファクタ
- **課題**: endTimeがオプショナルで「次の開始時間」を暗黙的に使う設計。effectiveEndTimeの計算が5箇所以上に散らばり、フォールバックが微妙に異なる
- **方向性**: endTimeを常に明示的に持たせ、コードを単純化
  - 空白期間が自然に表現できる（endTime < 次のstartTime）
  - 既存マイグレーションコード（phaseMigration.ts/labelMigration.ts）を拡張して対応
- ※ 要brainstorming。Firestore/localStorage既存データへの影響あり
