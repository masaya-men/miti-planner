...
### 最近の完了（2026-04-13）
- サイドバー刷新（4タブ構造、右クリックメニュー、Shift+複数選択、ホバーボタン）
- サイレント圧縮（7日未使用で自動gzip、クリックで即復元）
- 過去零式の自動アーカイブ（確認→通知のみに変更）
- UI表記「プラン」→「軽減表」統一（4言語）
- ゲストヒント強化（データ消失リスク明記）
- セキュリティ修正（console.logからplanId/fileName除去）
- SVGパスエラー修正（LoPoButton）
- Firestore削除同期エラー修正（ローカルプランの無限リトライ）
- endTime必須化 + プレビュー120fps化
- モンク「金剛周天」追加
