...
- **✅ 2026-06-16 バグ修正2件 完了 (branch `feat/mobile-bottom-nav-redesign`・未push・実機Playwright検証済)**: ①**リキャスト行クロック表示**(下記バグ欄 ✅ 参照・Timeline.tsx)。②**スマホ通知モーダルがメニューシート裏に隠れて既読不可**=`SystemNotificationModal` backdrop の z を `z-[100]`→`z-[9999]` に(シート z-301/ボトムナビ z-400 より上へ)。PC は常設サイドバーから開くので可視だったがスマホはメニューのボトムシート内の通知ベルから開くため裏に隠れていた。Playwright(モバイルvp+実Firestore通知)で「既読にする」ボタンが押せる=前面表示を確認。関連テスト33緑/build EXIT 0。**未push=前回の docs/PITRスクリプトと一緒にまとめて push 待ち(ユーザーGO)**。
...
- **✅ スマホ共有タブ B 完了 (2026-06-16・同ブランチ・本番反映完了)**: 設計書=[specs/2026-06-16-mobile-share-tab-design.md](./superpowers/specs/2026-06-16-mobile-share-tab-design.md)。共有タブ=現在プラン1つの共有(複数まとめて共有はメニュー側のまま)・PC共有判定をミラー。`useShareFlow` フックに共有状態機械を抽出(PC ShareButtons から逐語移植・1ソース化)/ ShareButtons 薄リファクタ(PC挙動不変)/ `OwnerCollabPanel` に `hideCursor`(スマホでカーソル共有UI非表示)/ `MobileShareController` がナビから flow 起動 / 純粋ロジック+テスト7件。build EXIT 0・vitest 既知5+新規7緑・yjs遅延チャンク維持・スマホ実機でコピー共有起動OK。**collab は 2026-06-14 一般公開済み**(旧メモリの「held」は stale)。**次=A+B まとめて push/デプロイ(ユーザーGO待ち)**=`git push origin HEAD:main`(Vercel自動デプロイ)→ ③本番確認 + スマホ実機総点検。
- **✅ 2026-06-16 追加修正(同ブランチ・本番反映完了・seed済)**:
  ...
  - **✅ B「共同編集2択が出ない」調査=バグなしで決着 (2026-06-16)**: 原因=前回はローカル(dev)テストで `/api` 認証が動かず**ログイン自体が不可**だったため(=未ログインはコピー直行が設計通り [shareView.ts:11](../src/lib/collab/shareView.ts#L11))。判定/配線/モーダル全経路を精査し、ログイン済み経路にコード不具合なし(ログイン+OFF→`choice`2択・テスト済)。PCも同 `useShareFlow` で同挙動=スマホ固有の不具合ではない。
...
- **✅ チャージ技のリキャスト表示が出ない (星天交差/ディヴァインベニゾン) 根治 (2026-06-16・branch feat/mobile-bottom-nav-redesign・未push)**: systematic-debugging で実機再現+instrument。真因=**recastRow 論理/データ/コンポーネントは全て正常**(Firestore実データ recast=30 確認・ユニット/コンポーネントテスト緑)。バグは Timeline 統合層=`RecastRow.update()` がスクロール時とスクロール effect マウント時しか呼ばれず、配置 effect の deps([Timeline.tsx](../src/components/Timeline.tsx) syncRecastRow 効果)に `timelineMitigations` が無かった→新規配置アイコンはデフォルト `--cd-display:none` のまま、ユーザーがスクロールするまで出ない。チャージ系ターゲットシールドは「今見ている位置」に置いて確認する=スクロールしないため最も顕在化(レイドバフは置いた後に次機構へスクロール→偶然 update が走り表面化せず)。**修正=`syncRecastRow` を useCallback 抽出し `timelineMitigations` 変化時にも即再同期**。Playwright実機で「配置直後スクロール無し」→修正前 disp=none/修正後 disp=flex,num表示を確認。recastRow.test.ts にチャージ技ケース追加(26緑)/build EXIT 0。全スキルで配置直後にクロックが出る正しい挙動に(通常スキル非破壊)。
