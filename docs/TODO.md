...
**🔴🆕最優先: 中央地図の完成度を世界一へ(ブレスト10改善→spec→Phase1 plan完成・実装これから・2026-07-04)**。ブランチ`feat/housing-tour-nav-m1`(未push・spec/plan commit済)。spec=`docs/superpowers/specs/2026-07-04-housing-tour-map-worldclass-design.md`、Phase1 plan=`docs/superpowers/plans/2026-07-04-housing-tour-map-phase1-nav-drawing.md`。**次=Phase1(改善1・2・3・8)をsubagent-driven実装(純関数=haiku/UI=sonnet/CSS=実画面ゲート)→実機→Phase2-5**。10改善:①出発を道に乗せる②到着を家の手前で止める③波紋撤去④地図番号撤去⑤パン&ズーム(戻す/次でﾃﾞﾌｫﾙﾄ/ｽﾏﾎ必須)⑥LIVE/凡例撤去⑦経路にﾌｨｯﾄしﾊﾟﾈﾙ充填(ｴｰﾃﾗｲﾄと家は見切れ厳禁)⑧枠線色を世界観へ(輪郭残す)⑨ﾚｲｱｳﾄ再編(左=ｼｮｰｹｰｽ/中=地図/右=進行専念)⑩進行ﾓﾃﾞﾙ拡張(移動節目DCﾄﾗﾍﾞﾙ/ﾜｰﾙﾄﾞ/ﾃﾚﾎﾟ+見学ﾎﾞﾀﾝ分離+見学ﾀｲﾏｰ)。408770d1(箱ﾊｲﾗｲﾄ実機OK・座標根治の残り=Goblet回転区画/ｱﾊﾟｰﾄ棟1-2は実機未確認)の上に積む・生きたカードもPhase5でやる。
