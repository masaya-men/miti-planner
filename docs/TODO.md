...
- **✅ スプシ取込バグ修正一式=本番デプロイ済(2026-06-23)**: ①Bug①=collab no-op根治(`commitImportedPlan.ts`・先にdisconnect+防御フラグ解除・main `994b9111`・ユーザー検証OK) ②Bug②=末尾フェーズ黙殺防止(未追加draftがある間は作成不可+警告・`importBlockReason`) ③作成不可理由をボタン直上に明示(未追加=黄/パーティ未割当=赤・main `ca98bd32`)。根本原因2つはsystematic-debugging+多エージェント検証で特定(反証=非collabのフェーズ境界/描画clipは犯人でない)。詳細=`docs/.private/2026-06-23-spreadsheet-import-issues.md`、spec/plan=`docs/superpowers/*2026-06-23-sheet-import*`。
- **🔵 次=「取込フロー v2」brainstorming(設計から)**: ①5/5満杯時に選択削除しながら取込(破壊操作=要確認UX・i18n `limit_*`雛形あり) ②選んだ表の時刻以降から途中取込(時刻オフセット/フェーズ連結/パーティ統合=本格機能・FFLogs置換/追記の前例) ③取込押下時にコンテンツ選択を前段(`currentContentId`流用の誤紐付け解消)。3つは"取込体験"として地続き→まとめて設計。
