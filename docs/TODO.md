...
### 🟡 ツアー追加フィードバックアニメーション(2026-08-10・設計書+実装計画完成・未着手)
探すページのカード/詳細ページで「ツアーに追加」を押しても成功/失敗の手がかりが無い問題への対応。成功=チェックマーク描画+「追加済み」トグル状態(もう一度押すと外れる)、失敗(別リージョン)=ボタン単体シェイク+ボタン直上の吹き出し(画面下中央の汎用トーストは置き換え)。ブレインストーミングで方向性確定済み・設計書=`docs/superpowers/specs/2026-08-10-housing-tour-add-feedback-design.md`・実装計画(7タスク・TDD手順込み)=`docs/superpowers/plans/2026-08-10-housing-tour-add-feedback.md`。次セッションはこの計画から実行開始。
