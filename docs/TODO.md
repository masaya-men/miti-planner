...
- **📌 次セッション最優先 = 同期安定化（業界水準で根治）→ 共同編集の土台**: 「別端末で消失/削除→復活」の真因＝マージが「未同期」と「他端末削除」を区別できない（再現テスト済 `src/lib/__tests__/planSyncMerge.repro.test.ts`、it.fails で固定）。**小手先でなく業界水準で**: ①削除をソフトデリート(墓標)化 ②Firestore オフラインキューを活かす(unload の read-before-write 廃止) ③読みを onSnapshot 化（→共同編集土台に収束）。設計・実装順・原則は **[docs/.private/2026-06-03-realtime-collab-and-sync-notes.md](./.private/2026-06-03-realtime-collab-and-sync-notes.md)** に集約。sync core=全データ影響、各段 test-first＋全テスト/build緑＋オフピーク段階投入。共同編集(presence/カーソル)・同一秒8件レイアウトも同ファイルに記録。
