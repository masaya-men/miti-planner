...
describe('HousingDetailContent', () => {
  ...
  it('postUrl があると「元の投稿を見る」リンクが postUrl を指す', () => {
    const withPostUrl = { ...listing, postUrl: 'https://twitter.com/someone/status/123' };
    const { container } = renderContent({ listing: withPostUrl });
    const link = container.querySelector('.housing-detail-source-link');
  ...
  it('postUrl が無いと「元の投稿を見る」リンクは出ない', () => {
    const { container } = renderContent();
    expect(container.querySelector('.housing-detail-source-link')).not.toBeInTheDocument();
  });
...
describe('HousingDetailContent: unlisted は住所を出さない (住所漏洩防止)', () => {
  ...
  it('unlisted かつ postUrl があっても「元の投稿を見る」リンクは出ない (住所の間接漏洩防止)', () => {
    const unlistedWithPostUrl = {
      ...listing,
      visibility: 'unlisted',
      postUrl: 'https://twitter.com/someone/status/123',
    };
    const { container } = renderContent({ listing: unlistedWithPostUrl });
    expect(container.querySelector('.housing-detail-source-link')).not.toBeInTheDocument();
  });
});
