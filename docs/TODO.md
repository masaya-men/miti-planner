...
## 次の作業順 (2026-06-30 更新)
1. **【最優先・原因特定済→方針転換】スプシ取込スマホ**: 原因確定=有名スプシは**TRUE/FALSEマトリクス**で、ジョブ/スキルは**アイコン画像+隠し行**。スマホ貼付(text/plain)はチェック値もアイコンも消え**構造的に不可**(実機TSVで実証)。**解決策=「スプシURL直読み」新機能**: 公開シートの `export?format=csv&gid=` をブラウザ直fetch(CORS OK・サーバー/APIキー/Vercel関数 不要)→CSV→TSV→**既存 `parseMitigationSheet` がそのまま成立**(実証: 軽減列118/データ83行)。PC/スマホ両対応で堅牢。次=**brainstorming**(URL→id+gid抽出・tab選択UX・非公開検出・実ブラウザCORS確認)。[[project_spreadsheet_mobile_grid]]。⚠`/clip`診断ページと臨時収集サーバーは撤去予定。
2. **🆕イベント追加の時刻を MM:SS でも入力可に**(2026-06-30 ユーザー要望。現状は秒数のみ→毎回手で秒換算が必要。EventForm/時刻入力。着手時 brainstorming-lite)。
...
- **🆕 2026-06-30 本番デプロイ済(スマホ スプシ取込・但し未完)**: ①スマホでも `<textarea>` 貼付方式で取込可に(iOS Safari は `readText()` が NotAllowedError でブロック=真因。textarea+onChange なら取れる)。`useIsMobile` フック追加。「近日公開」蓋を外した。②スマホは編集グリッド非表示で、grid 形式は各メンバー列に枠(MT/ST/H1/H2/D1-D4)を割り当てる縦リストを追加+小画面でフッターの注意書きが切れる問題をスクロール本体移設で解消(全て isMobile 分岐内・PC不変)。③LP 03 説明を英語化。SDD+opus最終レビュー通過・全てPC無変更を厳守。spec/plan=`2026-06-30-mobile-spreadsheet-import-paste-design.md`(+追補)/`2026-06-30-mobile-grid-party-and-scroll.md`。**🔴未解決**: ユーザーの実シート(grid形式)は実機で **軽減もパーティも取り込めない**。原因未特定=**スマホでコピーした実 TSV が必要**(PCコピーと中身が違う仮説。リンク=PC視点で不可)。次セッション最優先(作業順#1)。診断ページ `/clip`(=ClipboardInspectorPage・本番在)は原因特定まで残置→解決後にルート撤去。**注意**: 新規 Vercel Node 関数は **Hobby 12個上限**で追加不可(diag-clip 追加は13個目でデプロイError→revert済。本番は終始無事)。
