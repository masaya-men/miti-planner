...
- **🟡 スプシ軽減表取り込み: 実装+忠実性根治済・本番から一旦取り下げ→ユーザーのローカル確認待ち(2026-06-21)** feat ブランチ `feat/spreadsheet-import` に温存(**本番未投入**)。一度 main へ merge+デプロイしたが、ユーザーがローカル確認前だったため `git revert` で本番から外した(コードは履歴+ブランチに保持)。徹底チェック(実データ全5タブを `buildPlanFromSheets` に通し全TRUEを1:1突合+多エージェント敵対監査+Claude実機Playwright=技521/軽減218/枠8/skip1 一致・junk無し)で rising-edge化/メタ行除外/フェーズ単位構築/全滅技enrage化/チップ実配置数/版違い回帰固定 を根治済(詳細→`.superpowers/sdd/progress.md`)。**次=ユーザーがローカル(dev `npm run dev`→5173)で実データ確認→OKなら再 merge+デプロイ**。残=5/5破壊チューザー(既存5枠時の置換/削除UI)は後追い(現状は上限時 安全停止)。
