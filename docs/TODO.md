...
- **ブランチ**: main 直接
- **最新本番デプロイ**: セッション 16 (commit `b983c78`、 列幅 126/53 + 互い違いバグ修正 + 左飛び部分修正)
- **注意**: ENFORCE_APP_CHECK=true、 Vercel 関数 9/12、 月 100 ビルド制限
- **軽減アプリ: 完成・公開済み** (2026-04-13 完成ツイート済み)

---

## 次セッション最優先 (順序付き、 詳細は docs/.private/ 参照)

ユーザー方針: 「時間をかけて理解、 既存機能を守る」 → brainstorming で 1 件ずつ詳細詰めてから着手。

1. **【未解決 / 本番再発】 左から飛んでくるバグ** — `layoutReady` (1 フレーム visibility:hidden) では足りない。 本番では layout 確定までもっと長い。 ユーザー方針「ゆったり出現で OK」 確定。 修正案 A/B/C は docs/.private/ 参照
2. **【新計画】 表エリア全幅化 / フォーカスモード時に右端まで埋まる** — 「人によって環境が違うから大規模工事した (sizing 思想 v2)」 → 本来の目的に戻る。 列幅拡張で 0.5px ズレ治療と統合
3. **【新機能】 リキャスト専用行** — フェーズ│ラベル│時間 の下にリキャスト表示行 (折り畳み可、 デフォルト表示)、 ゲーム内本格表示、 パフォーマンス影響なし (過去議論済)
4. **【復活方針】 効果中スキル最上行残し** — 一度諦めたが復活、 上記 3 と相互影響

詳細・未解明点・brainstorming 議題: [docs/.private/2026-05-12-table-area-improvements.md](./.private/2026-05-12-table-area-improvements.md)

---

## 相談したい (次セッションで着手検討)

- **リキャスト本格表示**: 上記「2-4」 と統合、 専用行アプローチで確定
