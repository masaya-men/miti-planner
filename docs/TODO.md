...
0-1. ✅ **実機FB9件のうち②③⑥⑦⑧⑨を修正・本番反映済み・ユーザー実機OK(2026-07-20)**。②は初回pushでボタン文言指定(「＋ツアー」)を見落とし訂正(修正はコミット済・**push可否は本人確認待ち**・教訓=memory `feedback_deploy`)。**残る1件**=⑤YouTube概要欄住所自動入力は要ユーザー判断(公式API導入が必要)。詳細=TODO_COMPLETED。
