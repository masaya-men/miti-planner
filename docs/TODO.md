...
- **今回の作業**: ラベル列（旧ギミック区間列）の追加
- **完了した内容**:
  - ラベル列追加（フェーズ列右、PC: 50px、合計幅170px維持）
  - フェーズ列60px縮小、時刻列60px縮小
  - ラベルオーバーレイ（フェーズ同様の仕組み、フェーズ境界で区切る）
  - ラベル追加/編集/削除モーダル（LabelModal: 単一言語入力、contentLanguage依存）
  - ラベルヘッダードロップダウン（HeaderGimmickDropdown: ジャンプ機能）
  - mechanicGroup型をstring→LocalizedString化（後方互換: Firestore旧データ自動正規化）
  - モバイル: フェーズなし時にラベル列をフェーズ位置に表示（ヘッダーLb、セル24px）
  - 管理画面: ラベルen翻訳編集UI（TemplateEditor + useTemplateEditor）
  - ストアにsetLabelFromTime/updateLabelSection/removeLabelSection追加
