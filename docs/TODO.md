...
- **ブランチ**: main 直接
- **最新本番デプロイ**: セッション 16 末 (左飛びバグ useLayoutEffect 治療含む全 4 件完了済)
...
1. **【新計画】 表エリア全幅化 / フォーカスモード時に右端まで埋まる** — 「人によって環境が違うから大規模工事した (sizing 思想 v2)」 → 本来の目的に戻る。 列幅拡張で 0.5px ズレ治療と統合
2. **【新機能】 リキャスト専用行** — フェーズ│ラベル│時間 の下にリキャスト表示行 (折り畳み可、 デフォルト表示)、 ゲーム内本格表示、 パフォーマンス影響なし (過去議論済)
3. **【復活方針】 効果中スキル最上行残し** — 一度諦めたが復活、 上記 2 と相互影響
