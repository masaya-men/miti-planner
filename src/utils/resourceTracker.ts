...
function getAetherflowGainTimes(pattern: 1 | 2, upToTime: number): number[] {
    const times: number[] = [];
    if (pattern === 1) {
...
export function getAetherflowStacks(
    ...
    const gainTimes = getAetherflowGainTimes(pattern, time);
...
/**
 * Summon Seraph availability based on Aetherflow pattern
 */
export function canUseSummonSeraph(time: number, pattern: 1 | 2): boolean {
    const minTime = pattern === 1 ? 31 : 44;
    return time >= minTime;
}
