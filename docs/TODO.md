...
- **🛠 タンクスイッチ(挑発スキル) 実装完了・未デプロイ (2026-06-15・branch `feat/tank-switch-provoke`)**: 挑発を置くと同一フェーズ内・以降の攻撃の on対象 MT⇄ST を反転(derived/非破壊)。中核=純粋関数 `getEffectiveTarget`(TDD 10緑)+全計算/表示サイト適用(Timeline/CheatSheet/PC行/スマホ行)+recast30s通常スキル(`isTankSwap`)+アイコン切替アニメ+autoPlan除外。build緑。設計=specs/2026-06-15-tank-switch-provoke-design.md / 計画=plans/2026-06-15-tank-switch-provoke.md。**残=① main マージ→deploy ② デプロイ後に seed-icons + seed-skills-stats(ADDITIVE・dataVersion++で全ユーザー反映) ③ 本番実機検証**。⚠順序厳守=コード deploy→seed(seed が先だと未対応コードで0値スキルが見える)。
- **✅ 支援ページ刷新 本番デプロイ済 (2026-06-15・main `e07fcf2` push 済→Vercel自動デプロイ)**: 支援ティアを実在トレーニング器具に刷新(9段・税込実勢価格)+カードUIリッチ化。**残=本番実機確認(/support の表示・カードUI)**。
- **🔍 残・要本番サニティ (デプロイ済・ユーザー実機確認待ち / 2026-06-15)**: タイムライン種別クリックループ(物理→魔法→ユニーク3循環)+ デバフ軽減不可属性(`ignoresDebuffMitigation`)+ 種別赤箱表示 + スキルデータ棚卸し/seed安全化。本番で①種別循環②赤箱③デバフ無効を確認。古い形式が見えたら PWA SW キャッシュ→ハードリロード([[feedback_pwa_cache_after_deploy]])。任意=赤の濃さ実機微調整。詳細=[TODO_COMPLETED.md](./TODO_COMPLETED.md) (2026-06-15)。
- **✅ デバフ軽減不可ワンタッチ設定 本番デプロイ済 (2026-06-15・main `c41371c` push 済→Vercel自動デプロイ)**: ①タイムライン種別アイコンPC右クリックで`ignoresDebuffMitigation`即トグル(左クリック循環と同一`updateEvent`経路=collab/Undo/再計算/赤枠自動)②2行ツールチップ(左右クリック操作+現在ON/OFF)+`DamageTypeIcon`に`withTooltip`追加でPC二重ツールチップ統一③管理画面テンプレ編集に「デバフ軽減不可」チェック列(`updateCell`にcase追加=switch default無視の罠回避・`getSaveData`で公式テンプレ保存)。i18n4言語。TDD(DamageTypeIcon/PcTypeToggle/useTemplateEditor)。build EXIT=0/テスト緑(既知5失敗のみ)。設計=specs/2026-06-15-debuff-immune-quick-toggle-design.md。**残=本番デプロイ後の実機確認(右クリックトグル/管理画面チェック→保存)。**
- **✅ 共同編集 一般公開 完了 (2026-06-14・`feat/collab-public-release`・main push 済・Worker 5a6762aa)**: admin gate撤去で全ユーザー開放(編集=ログイン必須/閲覧=誰でも)。失効バグ根治(サーバ強制)・データ安全構造確認済(plans本体は不変)。**残=公開後サニティ(本番でログイン→共有→2ブラウザ→失効)。** 詳細=[TODO_COMPLETED.md](./TODO_COMPLETED.md) + memory `project_realtime_collab_status`。
