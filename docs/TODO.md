...
### 🔴 次セッション最優先 (2026-07-17 更新)
00. 🧹 **グローバル CLAUDE.md (~/.claude/CLAUDE.md) の剪定**(ユーザー依頼・プロジェクト外メタタスク): 公式方針「肥大化はルール無視の原因」に基づき適切な長さへ。**必要ルールは絶対に削らない=原則「削除」でなく「別ファイル/スキルへ移動+ポインタ化」**。手順: ①全セクションを「消すとミスが起きるか」で仕分け ②移動先を設計(例: 画面環境/サイズ設計→design-philosophy-sizing.md 系へ集約) ③before/after をユーザーに提示→承認後に確定。プロジェクト側 CLAUDE.md を含めるかは冒頭で要確認。
0. 🏠 **ハウジング公開前 残タスク**(網羅=`docs/.private/2026-07-15-housing-release-remaining-tasks.md`):
   - **✅スマホ対応=実機FB5弾まで反映し本番デプロイ(2026-07-17)**: 基盤(ナビ/FAB/シート/2列/全画面/ツアー横持ち→縦OK化)+FB第2〜5弾(トップ/フィルター▼ヘッダー/シート不透明+つまみ閉じ/行き方地図下部/終了/住所2行/次へ消失根治=sticky hover/トレイバー/カードをナビ下まで/スクロールバー右端/お気に入り2行化+文字ボタン)。#A削除即反映/#Bお気に入り件数重複も修正。PC無変更。**本番確認=全部OK(2026-07-17)**。台帳=.superpowers/sdd/progress.md。✅①#B残ストリップ件数(`d6261a9d`) ②LP導線(`0aec3971`) ③中韓用語CSV(`docs/.private/2026-07-17-housing-terms-ja-en-ko-zh.csv`+デスクトップ)=2026-07-17完了→TODO_COMPLETED。**🔴次**: ④お気に入りPC⇔スマホ非同期=根因確定([useHousingFavoritesStore.ts:26] zustand persist→localStorageのみ・サーバー同期なし)→同期設計はbrainstormingから。**FB第6〜8弾=全部実装・push済(2026-07-17・実機確認待ち)**: 同期✅/ツアー順+ピン✅/Xシェア(アイコン/#LoPo/本文なし)✅/OGPページ風カード(WebP根治・`?v=N`新クエリで確認)✅/左パネル画像固定=useTweetVideoFrames根治✅/跨ぎ通知=ボタン撤去・非ブロッキング化✅/スマホ地図整理✅。詳細=TODO_COMPLETED。**残TODO**: ①OGPカードのデザイン作り込み(ユーザー要望2026-07-17: 「LoPoのハウジングからの共有」と一目で分かるブランド感へ品質最大化・後日ちゃんと設計)。**ツアー招待URLのOGPも同様に見た目こだわり対象**(2026-07-17追記・①とセットで設計) ②アバターWebP勢はイニシャル表示(PNG変換は将来)。
