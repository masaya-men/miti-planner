...
1. **🔴 ハウジング次の着手順(2026-07-24決定・27日③完了)**: ①コスト面=✅完了 ②マイページ作成=✅完了(2026-07-24) ③タグAND検索=✅実装完了(2026-07-27、詳細は下記「現在の状態」)。**次: ④繁体字対応(LoPo全体) → ⑤⑥⑦は「その他細かいブラッシュアップ」**。④要点: DC分離は不要見込みだが翻訳量が桁違い(2,700キー規模+コード45ファイル規模)。詳細=`docs/.private/2026-07-24-tag-and-search-and-traditional-chinese-scoping.md`。⑤⑥⑦詳細=`docs/.private/2026-07-23-housing-task-inventory.md`。
2. **軽減編集タイムラプスのSNS投稿**(大物・要brainstorming)

## 現在の状態 (次セッションはここから読む)
### 2026-07-27: ④繁体字対応 brainstorming完了・設計書作成済み(次=writing-plansで実装計画へ)
台湾ユーザーは中国本土サーバーではなく、2025年12月開始の独立した繁体字版サーバー(UserJoy運営、台湾/香港/マカオ/星/馬来西亚)でプレイしていると判明(ユーザー指摘で発覆・前回メモの「DC分離不要」は誤りだった)。このためハウジングは「翻訳」だけでなく2026-07-18のKR/CN対応と同型の「新リージョン統合」が必須と判断。設計書=`docs/superpowers/specs/2026-07-27-housing-taiwan-region-and-traditional-chinese-design.md`。決定事項: 言語コード=`zh-Hant`(既存の"zh"衝突判定11箇所超はこの機会に正しく修正)/ リージョンコード=`TW`/ 翻訳方式=一般UIは簡体字ベース変換+ゲーム用語は公式繁体字ジョブガイド等で個別調査+ハウジング固有名詞はWiki→機械翻訳フォールバック/ 全体は5フェーズ(①台湾リージョン統合→②軽減表UI文言→③ハウジングUI文言→④管理画面のゲームデータ翻訳対応→⑤ゲームデータ翻訳流し込み)。実装計画も作成済み=`docs/superpowers/plans/2026-07-27-housing-taiwan-region-support.md`(10タスク、subagent-driven-developmentで実装予定)。**次回セッション最初にやること: この実装計画をレビューしてOKなら実装フェーズへ(サブエージェント方式 or インライン実行をユーザーに確認)**。詳細=`docs/.private/2026-07-24-tag-and-search-and-traditional-chinese-scoping.md`の「2026-07-27続き」節。
