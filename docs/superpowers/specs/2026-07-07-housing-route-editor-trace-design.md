...
> **確定設計は別書に移動**: `2026-07-07-housing-route-road-follow-design.md`。以下の A/B 段階案は実データ検証で否定された(カーブ突っ切り 34 件中 27 件が別 edge またぎ = 「同一 edge 内だけ」では 79% 取りこぼす)ため、別 edge をまたいで道グラフを辿る方式に確定。以下は経緯として残す。
