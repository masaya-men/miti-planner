...
## 現在の状態 (次セッションはここから読む)
### 🆕 ツアー計画画面(蛇行グリッド)設計書完成・レビュー待ち(2026-08-11)
探す/お気に入りページの右トレイ(幅300px)だけでは50〜100件規模のツアーは並べ替えづらい問題と、「ツアー」タブの文言(「ツアー中」固定で不自然)・未開始時にトレイ内容と連動せずほぼ空表示になる問題をまとめてbrainstormingで解決。設計書=`docs/superpowers/specs/2026-08-11-housing-tour-tray-page-design.md`(コミット済み)。要点: タブ文言「ツアー」に変更/「ツアー」タブは未開始でもトレイに1件以上あれば計画画面(PC=左に選択中の家の詳細パネル+右に蛇行グリッドで全件表示・スマホ=縦一覧のみ)を表示/トレイ行カードから住所表示を削除しタイトルを大きく(サイドバートレイにも波及)/スマホの並べ替えシートは撤去し「ツアー」タブへ統合。**次: ユーザーがspec内容を確認 → OKなら`writing-plans`スキルで実装計画作成**。
