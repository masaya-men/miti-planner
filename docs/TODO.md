...
- **最新セッション（2026-04-29・離脱ダイアログ廃止）**: ユーザー報告「タブを閉じる時の警告が鬱陶しい」を改善。`Layout.tsx` の onBeforeUnload で `e.preventDefault()` / `e.returnValue=''` を削除し、保存処理（saveSilently + syncToCloud）はそのまま継続。`visibilitychange (hidden)` がタブを閉じる前に必ず先行発火するため実用上ほぼ確実に同期は走る。sendBeacon 等の「完璧な同期」は Firestore SDK と相性悪く独自API追加が必要なため不採用。build+test 119 PASS、デプロイ完了。
