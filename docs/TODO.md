...
- **最新セッション（2026-04-28 終盤・続き）**: 1 段目の SW 修正でロゴ削除側は治ったが、ロゴ追加側が「ずっと生成中」のまま再現。Vercel ログから PUT /api/share は 200 成功、しかし続く GET /og/{ロゴ有り版hash}.png のリクエストがブラウザから一切発行されていないことを確認。原因は `<img>` 要素の src だけ書き換わるとブラウザの古いロード状態が残り新規 fetch がトリガーされないケース。**ShareModal の `<img>` に `key={ogImageUrl}` を追加して URL 変更時に強制再マウント**する 2 段目の修正を投入。build+test 244 PASS。
