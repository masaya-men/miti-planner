...
- **スマホUX改善**: 実装完了、本番動作確認待ち

### 次セッションでやること
1. **本番動作確認**（スマホUX改善の全機能、同期トースト、SyncButtonテキスト）
2. **スマホでイベント追加**の設計（brainstorming → writing-plans）
3. FAB言語切替を展開式セレクターに改善
