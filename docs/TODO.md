...
- **✅ 2026-06-20 共同編集中の進捗同期(Plan2) 本番マージ&デプロイ済 (main `17cee31` + worker `lopo-collab` 再デプロイ)**: 進捗の打点を memos と同じ Yjs コレクション同期レーン(`progressPoints`)に載せ、collab中は**全員ぶんを匿名 union**で同期・永続化(Firestore はネスト `data.progress.*`)。スカラー(cleared/活動日数/時間)は planMeta(LWW)。打点に固定 id 付与で同時記録の事故防止。設計=匿名 union(A案)・「誰の進捗を正に」論点を消した。設計/計画=specs・plans/2026-06-19-collab-progress-sync。
  - **多エージェント敵対監査が Critical 2件検出→根治**: ①setMeta が進捗 meta キー未マッピングで `schAetherflowPatterns`(表の学者自動配置)を破損 ②旧形式(id 欠落)進捗点が collab seed で消滅。両方 TDD 修正+再レビュー済。**進捗操作が表データを参照ごと一切変えないことを回帰テストで永久ロック**(`toBe`)。データ安全=各タスクレビュー+opus最終+敵対監査3観点+修正再レビュー+非collab保存経路の自己検証(Layout.tsx:307 collab中Firestore抑制)。
  - **🔴 残=本番2タブ実機(ユーザー・デプロイ完了後)**: ①tabB で道クリック→tabA に点が増える ②tabA で全消去→tabB でも消える&**表(軽減配置)が無傷** ③退室→再入室で点が残る(Firestore往復)。**両タブ最新版にリロードしてから**(memory `reference_collab_two_client_version_skew`)。
  - **🟢 defer Minor(データ消失経路なし)**: insertProgressPointAt の collab 非委譲=削除Undoが collab で一過性(設計上 collab 対象外) / collab中の空note削除が非対称(飾り) / UndoLastPointButton の null 防御 / 旧 SDD繰越(pending Undo クリア等)。**スマホ記録対応は別タスク**(今回スコープ外)。
