...
- **🟢 2026-06-18 表の情報列固定(PC横スクロール時) 実装完了・コードレビュー済・branch `feat/timeline-frozen-info-columns`(未push=正常)。残=ユーザー実機 視覚QAのみ**: 横スクロールで左の情報列(フェーズ/ラベル/時間/敵攻撃/元/軽減後ダメージ)を固定、スキル列だけスクロール(Excel枠固定相当)。常時固定/トグル無/モバイル対象外。設計=specs/・計画=plans/2026-06-18-timeline-frozen-info-columns(7タスク・subagent-driven)。**全タスク完了(コミット済・build EXIT0/component 156緑)**: 情報列を sticky 情報ペイン(`.timeline-info-pane` z=60 不透明・`TimelineInfoColumns`)へ分離、軽減バー(z≤30)はペイン下を通り情報列上にはみ出さない設計。`TimelineRow`=スキルセル+折りたたみ追従スペーサー。境界影+トークン化済。**Task4-6レビュー+最終whole-branchレビュー(opus)=Ready to merge**(Important1件=色トークン化対応済、派生バグ=dark セレクタ`.dark`→`.theme-dark`修正済)。台帳=`.git/sdd/progress.md`。**🔴 残=ユーザー実機 視覚QA(§7・特に#3 軽減バー非はみ出し / ライト・ダーク / フェーズ折りたたみ追従 / 境界影 / DPR2.58 開発者画面)→ OK後 push→PR/merge→デプロイ**。Minor持越(最終R defer合意)="Task N"コメント / timelineFrozenInfo.test.tsx→.ts。
