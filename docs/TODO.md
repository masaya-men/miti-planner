...
0. **🔴 次セッション最優先**: 軽減表ブラッシュアップ残り3件(0-2は本番反映済み・下記参照)。
...
## 現在の状態 (次セッションはここから読む)
### ✅ 2026-08-26 軽減表バリア計算バグ2件、本番反映済み・実機確認待ち
展開戦術のコピー計算でランパート等の自己バフが本人以外にも乗るバグ、および最大HP%固定タイプのバリア6種(ディヴァインヴェール/ブラックナイト/アルケインクレスト/シェイクオフ/テンペラグラッサ/インプロビゼーション)に回復効果アップ・確定クリティカルが誤って乗るバグを修正・push済み(`isPotencyBasedShield`をscholarShieldRules.tsへ切り出し、全28バリア技を回帰テストで網羅)。フェイイルミネーション・EventForm.tsxの手動チェックリストは調査の上で見送り(詳細はコミットメッセージ参照)。**masayaさんの実機確認待ち**。次は軽減表ブラッシュアップ残り3件(0-1/0-3/0-4、上記参照)。
