...
- **ブランチ**: main (push 済・本番反映済)。 2026-06-02 本セッション: **PiP タイムライン作成(Document PiP 版)を実装→本番投入済**。EventForm 抽出/手動ストップウォッチ/逆算入力固定/メニューのホバー展開/メモ機能 en・zh・ko 翻訳漏れ修正/**捕捉時刻の整数秒丸め(行が出ない根因)**/コンテンツ名+プラン名ヘッダ、まで完了し本番で書き込み動作確認済み。
- **📌 次セッション最優先 = 動画埋め込み式モーダルの実装** (Document PiP 版を置換): **計画を subagent-driven-development で実行**。設計=specs `2026-06-02-video-recorder-modal-design.md` / 計画=plans `2026-06-02-video-recorder-modal.md` (全7タスク・実コード入り)。YouTube を youtube-nocookie で埋込(URL貼付のみ・検索なし)、時刻=動画位置−戦闘開始、native コントロール、CSP に `script-src www.youtube.com` 追加。右UIは作成済み EventForm 流用、PipRecorder は削除して VideoRecorderModal へ統合。実機 E2E(動画再生・CSP)が肝。
