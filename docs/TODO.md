...
- **🐛 スプシ取込バグ(要調査・別セッション・優先度高)**: 既存表を開いた状態で**1回目取込が壊れる**(後半の軽減が欠ける/フェーズ時間ずれ)→**同じものを2回目入れると直る**。最終フェーズの後に空フェーズを足さないと取り込めない問題も。仮説=作業ストアの状態持ち越し([[reference_commitnewplan_loadsnapshot_contract]])＋最終フェーズend未確定で後半が落ちる。詳細・改善案(「ここまでで取込」導線)=`docs/.private/2026-06-23-spreadsheet-import-issues.md`。systematic-debugging で実機再現から。
