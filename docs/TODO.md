...
**🟡 次セッション最優先: 「連動アニメの初動だけかくつく」大幅改善・ユーザー実機確認待ち**。詳細・調査経緯は`docs/.private/2026-08-14-mobile-scroll-stutter-investigation.md`。**根本原因は2つ特定済み**: ①`.timeline-scroll-container`に登録された5つの`scroll`関連リスナー(RecastRowのCD表示更新/フェーズ見出し/エフェクト棒/ヘッダー同期/スムーズホイール)の累積コスト(5つ全部を同時に軽量化して初めて重いフレームがほぼ消えることを実測で確認、1つずつでは効果ゼロ)。②`.mobile-row-dim-text`(行の文字濃淡)が`[data-mobile-scrolling="1"]`属性セレクタでON/OFF切替されており、表全体を横断する再評価が発生(ユーザー実機報告「常時表示の方が軽い」から発覚)。**②を本採用(常時濃い状態に固定・属性切替廃止)したところ実測で重いフレームが6件→2件まで激減**、これを受けて一時撤去していた「連動」モードをFABピッカーへ復活済み・本番反映済み。①(5リスナー統合)はまだ未着手だが、②だけでもかなり改善したため、まずは**次セッション最初にユーザー実機で連動モードを確認**し、体感で十分ならそこで一区切り、まだ気になるなら①に着手。
