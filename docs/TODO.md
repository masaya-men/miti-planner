...
- **🆕 機能ブラッシュアップ案9件**(詳細=docs/.private/2026-06-15-feature-ideas-batch.md): ①同時刻3+イベント ②スマホ/タブレット最適化(残=ボトムナビ/FAB) ③軽減競合逆方向警告(✅実装済) ④MAXHP-10%でダメージ黄 ⑤Logsインポート上書き/追記(✅本番済2026-06-20) ⑥有名スプシ取込(✅実装完了2026-06-21=上記🟣・要実機確認) ⑦敵攻撃 or(2択) ⑧管理画面 攻撃ID保持で任意言語翻訳(GUID保持済=ほぼ実装済・仕上げのみ) ⑨メモに動画URL→iframe。⑥は実装完了(要実機確認)。取り込み導線チューザー統合は将来。**🆕列グリッド取込=実機ブラッシュアップ中(`feat/spreadsheet-grid-import`@`02fea99a`・未merge)。大モーダルを複数フェーズ対応ウィザードに統合済(コンテンツ選択→次へ→大グリッドにCtrl+V→「フェーズ名(任意)+このフェーズを追加」でタブ毎蓄積→メンバーMT/ST/H1/H2/D1-4順+枠選択→読めない軽減一覧→作成。実証エンジン[buildPlanFromSheets複数entries統合/applyTemplateTargets/commitImportedPlan]全再利用・「有名/famous」表記全消去[grep0])。spec=`docs/superpowers/specs/2026-06-24-spreadsheet-grid-import-design.md`§9.5/9.6に実機FB反映。残=①本人実機で新フロー確認 ②S2「攻撃の対象を行ごと編集可」要否判断(テンプレ§③自動引継で足りるか) →OKでmain merge+push。次セッションはここを集中ブラッシュアップ。**
