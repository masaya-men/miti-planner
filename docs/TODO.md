...
0. **🔴 次セッション最優先(2026-08-24 ユーザー指定)**: 軽減表周りのブラッシュアップ。**詳細未確定・セッション冒頭でユーザーに内容を確認すること**。ハウジングの「新着ハウジング自動ツイート下書き通知」(バックログ参照)と、下記1〜3のタイムラプスSNS投稿/スプシモード/Wiki型タイムラインは、いずれもこのブラッシュアップより後回み(さらに先送り確定)。
