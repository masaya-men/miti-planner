...
- **ブランチ**: main 直接
- **最新本番デプロイ**: セッション 22 末 (バグ 5 件解消 + admin 「コンテンツ管理」 積年バグ一掃 + 同期ボタン UI インジケータ化)
...
- **UI 改善 (検討中)**: SVG アイコンアニメ、 紹介 PV 動画 (CapCut/DaVinci)、 サイドメニュー軽減表名の全文表示 (折返し対応、 切れ ⇄ 折返しのトグル等含めて要相談)、 スマホ軽減追加シートをジョブごとの単純並びに (現状の複雑な並び順を撤廃、 「暗黒騎士: 軽減 1234... / 白魔導士: 軽減 123...」 形式、 2026-05-16 要相談)
