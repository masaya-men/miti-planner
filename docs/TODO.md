...
③**Allmarksまとめてインポート**: Allmarksのタグ絞り込み共有リンクを一時ツアーへ一括インポート(Allmarks側は無改修)。`EPHEMERAL_POOL_LIMIT`も50→100。**実機で1件も取り込めない不具合を発見・修正・本番デプロイ済み・ユーザー確認OK**(`GET /api/share/<id>`のレスポンス形不一致、詳細=`.private`)。**リージョン混在対応(コミット2575e7df・未push)**: 空の一時ツアーからの一括インポートは取り込み中ブロックせず全部追加→完了時に複数リージョン混在なら選択肢を出し選ばなかった方を取り消す。**インジケーター演出を作り直し(未コミット)**: ユーザーFBで「家が降ってくる→道でつながる→光の粒が歩く→消える」ループに変更(旧・青い輪っか+弾む家アイコンは撤去)。tsc/build/全テスト(519ファイル/4147件)グリーン。詳細=`docs/.private/2026-08-19-allmarks-import-handoff.md`の「追記3・4」。
**次回**: 演出作り直し分をコミット→ユーザーがローカルdevで見た目確認 → OKならリージョン混在対応と合わせてpush→デプロイ → 短縮URL「見つかる」経路・Allmarks実リンクでのリージョン混在ケースの実機確認 → 問題なければDiscordアップデート告知。
