...
- **✅ 2026-06-29 本番デプロイ済**: ①**共同編集の重さA根治**(カーソル描画を CursorOverlay に隔離し Timeline をカーソルパケットで再描画させない・実機OK) ②**メモURLリンク化#1**(通常表示のみhttp(s)→新タブ・SDD+opus最終Ready=Yes)。**🆕PS5リモプ貼り付けアシスト(/stgy)=✅本番デプロイ済(2026-06-29・PR#2マージ)。スマホ専用1画面・コード分割コピー・100dvhシェル(iOSスクロール堅牢化)・入口=軽減⋯メニュー+LP Tools 03。実機OK。ゲーム内プレビューは符号非公開で不可(確認済)。次=スプシ取込スマホ本格対応(作業順#3)**。
