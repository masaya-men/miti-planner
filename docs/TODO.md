...
- **🔴次=段取り③(Firestore保存)をbrainstormingから設計**(2026-06-04 順序変更: ⑤より先に③)。理由=ゴール「**リンクを後から開いてもオーナー不在でも中身が読めて編集できる**」には③(seed+書き戻し)が必須。⑤を先にすると「オーナーが居る間だけ繋がる同期セッション」止まりで未完成な手触り→③→⑤の順がムダがない(ユーザー承認済)。**③のscope**=DOの`onLoad`(Firestore plan→Y.Doc)+`onSave`(Y.Doc→オーナーのplans/{id}書き戻し)。当面**room鍵=plan IDのまま**(②-a同様)、トークン分離は⑤。書き戻しは既存保存ロジック(version楽観ロック・墓標・マージ [src/lib/mergePlans.ts])を再利用したい。前提=ログインユーザーのplanはFirestore自動同期済([Layout.tsx:211](./../src/components/Layout.tsx#L211))・共同編集中はFirestore保存抑制済([Layout.tsx:228](./../src/components/Layout.tsx#L228))。
