...
### 次にやること
- **みんなの軽減表ボトムシート — 追加ブラッシュアップ**
  - ✅ 基本実装・スマホ対応・プレビュー刷新（スキルアイコン+エフェクト棒）済み
  - ✅ ボタン名「野良の主流が見つかるかもしれないボタン」+ Tooltip
  - ✅ コピーオーバーレイ（擬似プログレス crawl→surge→done）
  - ✅ カプセル通知の残留ラグ修正・×ツールチップ除去・王冠削除
  - ✅ プレビュー攻撃名の言語修正（contentLanguage使用）
  - **残り: ユーザーが追加で修正したい点あり（次セッションで対応）**
  - 残り: shared_plansテストデータをFirebase Consoleで削除 → 正式な1件を共有
  - 残り: ライトテーマ確認、実データでのOGP・コピー動作確認
