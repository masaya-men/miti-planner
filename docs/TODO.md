...
発端: モバイル版でCDかぶり競合が画面に一切表示されない不具合。**ユーザー判断で着手順を「①レイアウト刷新 → ②競合表示」に確定**。**進め方**: `npm run dev -- --host`でLAN公開し、ユーザーが実機iPhoneで見ながら1件ずつ指示→実装→確認のループ(feedback_one_fix_one_verify)。**次セッション開始時: 開発サーバーが動いていなければ再起動しURL共有**。

**①-a 2行構成レイアウト化 = 実装済・コミット済(未push)**: `MobileTimelineRow.tsx`で行を「時間+攻撃名+ダメージ」+「軽減アイコン専用行」に分割。詳細はgit log参照。**未決**: 軽減アイコンサイズ18px⇄22px未確定。

**①-b エフェクト棒 = 実機調整で多数修正・ローカルコミット済(未push・いつでも戻せる状態で保持)**: 2026-08-13に右詰めクラスタ内の並び順修正(mt h1 d1 d3 st h2 d2 d4)/アイコンが1つ前の行にはみ出す不具合修正(bar.top基準+静止時アイコン行と同じ高さに統一)/タップ・長押しだけでもエフェクト棒に誤発動する不具合修正(実スクロール時のみ発火)/クロスフェードをduration・easing統一+`mix-blend-mode:plus-lighter`で濁り解消/静止時アイコン行を右寄せ化(ユーザー判断: 位置を厳密に合わせる必要はなく単純右寄せで十分)。**次(ユーザー合意済・未着手)**: スクロール速度に連動して不透明度が連続的に変わるようにする。方式調査済み——CSSのscroll-driven animationsは位置連動でこの用途に不適、JSでスクロール速度を計算しコンテナのCSS変数1箇所を更新→各行はCSS計算式で参照、が軽量。**注意**: 指を置いたまま止まってもガタつかない既存仕組み(タッチ中は消さない)との両立調整が必要、実機微調整前提。
同セッションでボトムシート系も修正(メニュー/軽減追加/イベント編集): ナビとの隙間を実測値ベースの共通定数に統一/3シート共通で下スワイプ閉じ対応(`useSwipeToDismiss`フック新設)/ドラッグハンドル当たり判定を見た目そのまま44pxまで拡張/軽減追加シートを独自実装からMobileBottomSheet共通部品に置き換え。

**残作業**: ①完了後に②競合表示(`conflictingIds`はPC同様リング/バッジ想定・検出ロジックは既存流用可)。
