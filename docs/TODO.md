...
- **🚧 2026-06-18 攻略の進捗トラッキングHUD Plan1 実装+実機イテレーション済 (branch `feat/progress-tracking-hud`・未push/未merge/未deploy=エンドユーザー不可視)**: 設計書=[specs/...hud-design.md](./superpowers/specs/2026-06-18-progress-tracking-hud-design.md) / 計画=[plans/...part1.md](./superpowers/plans/2026-06-18-progress-tracking-hud-part1.md)。Plan1全タスク完了(TDD+タスクレビュー+opus全ブランチレビュー)後、ユーザー実機検証で3点反映(最新 commit b3d94d4・詳細は `.git/sdd/progress.md`): ①**承認済みヘッダー整理**(proto fb5edee)が Plan1 に未含有→移植: ⋯メニュー(HeaderToolsMenu)集約[自動組立/MYジョブハイライト/進捗バー表示]・import=Download・MYジョブハイライトCSS駆動最適化(grayscale廃止→opacity0.2) ②**記録UX根治**: 行onClickCaptureで全列横取り(モーダル開かせない)/外側クリックで閉じない/1点で閉じる/記録モード視覚化(行ハイライト=select-mode overlayと同一の青2px枠+bg10%・crosshair) ③**データモデル**: dailyBest(同日max)→**points(毎クリック1点蓄積)**・normalizeProgressで旧形式救済・軌跡min/max自動スケールで高低差強調。軌跡canvas/お祝いは試作`4c0b94b`を1:1移植([[feedback_keep_liked_prototype_visuals]])。build EXIT=0/vitest既知housing5件のみ赤。**残=①記録時トースト(別スタイル・次回ユーザーと) ②高低差さらに欲しければ帯ヘッダーはみ出し背高化 ③normalizeProgress単体テスト ④Plan2(collab同期) ⑤最終レビュー→main merge=本番デプロイ(ヘッダー直撃・実機総点検後)**。次フェーズ=スケジュール調整(別spec)。
