...
**✅ 暫定対応2件・本番反映済み**(2026-08-14、根本修正[5リスナー統合]が済むまでのユーザー判断): ①エフェクト棒の「伸びる/縮む」演出を廃止・opacityクロスフェードのみに変更。②その上でユーザー判断により**「連動」モードをFABピッカーの選択肢から一時除外**(`icon`/`bar`の2択に変更、`useThemeStore`に`migrate`追加で既存`scroll`選択ユーザーは`icon`へ自動移行)、根本修正が完了するまで連動モード自体を選べなくした。**次セッション最初にユーザー実機確認(ピッカーが2択になっているか)を聞くこと**、その後は根本修正(5リスナー統合)に着手。
