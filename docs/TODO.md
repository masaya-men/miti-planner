...
Vercelから「Fluid Active CPU(月4時間の無料枠)75%消費」のメール→ダッシュボード実測(Observability→Functions)で`/api/collab`(共同編集の読込/保存)が全体の75%以上を占めると判明。ユーザーが8人固定で1-2週間使用しただけでこの消費量=このペースだと無料枠だけでは月4時間を超える計算。原因はバグではなく、共同編集の保存が「DOストレージ(Cloudflare側・無料・こまめな保護用)」と「Firestore反映(Vercel経由・有料枠消費)」の2層を**同じ15秒debounceで一緒に動かしていた**ため(ライブ同期自体は別経路のWebSocket/DOなので、Firestore反映はあくまで保険用途)。**対応**: `workers/collab/src/server.ts`でDOストレージ保存(`saveBinaryOnly`)とFirestore反映(`syncFirestore`)を分離、後者だけソロモードの定期バックアップと同じ5分間隔に間引く判定を`saveThrottle.ts`(純粋関数)へ抽出。全員退室時(`onClose`)は間引かず強制反映(タブを閉じたら必ず保存、の体験は不変)。ライブ同期(他の参加者に見える速さ)には一切影響なし。対象vitest66件(新規6件)・tsc確認済み。`wrangler deploy`で`workers/collab`本番反映済み(Version ID `2c162875`)。**次セッション: 数日後にVercelダッシュボードで消費量の減り方を確認**。
### ✅ 2026-08-12 スマホ背景動画が読み込まれない(リロードで直る)不具合=自己修復ロジックで対応・実装完了(未push)
ユーザー実機で「スマホを開いた直後、背景動画が映らず一色の背景(黒〜青っぽい)のまま」と報告。原因は`.housing-scenery`の下地背景色(ダークテーマは`#050814`〜`#0a1230`〜`#0d1140`の紺〜紫グラデ)が動画未描画のまま透けて見えていたと判明(症状の色と完全一致)。production実機でCloudflareのRangeリクエスト挙動をcurlで直接検証し、CDNキャッシュ側の破損は否定(Rangeごとに正しいbyteを返し、同一Rangeは正しくHIT)。`SceneryVideo.tsx`に読み込み失敗/無応答時の自動リトライが一切無かった点が根本的なギャップ(ブラウザは動画の読み込み失敗を自分から再試行しない)。**対応**: `useVideoLoadRecovery`フックを追加し、アクティブな動画が6秒以内に描画開始(`loadeddata`)しなければ`load()`し直す/`error`イベントでも即リトライ、上限1回でCDN帯域浪費を防止。追加コストは実質ゼロ(今まで手動リロードで発生していた再取得を自動化するだけ+CDNキャッシュ済みなので原サーバー負荷なし)。対象vitest7件(新規3件含む)・tsc確認済み。**次セッション: push前ゲート→pushのみ残(スマホ版ツアー招待UXの件と合わせてpush予定)**。
### ✅ 2026-08-12 スマホ版ツアー招待のログイン導線UX改善=実装完了(未push)
PC版はツアー画面中央パネル下部に招待カードが常時表示されるが、スマホ版は`.housing-tour-invite{display:none}`で丸ごと非表示にしており招待の入口(下部バーの人+アイコン)に説明が一切ない問題(実機指摘)。「PC版パネルをそのまま出せば?」も検討したが、地図右下フロートは位置的に下部固定バーと重なる(過去に一度その形で実装→実機FBで統合済みという経緯がコード上に残っていた)ため不採用。最終案: ①アイコンをログイン状態だけで決める(未ログイン=人+マーク`UserPlus`/ログイン済み=最初からリンクマーク`Link2`、発行有無では変えない) ②ログイン済み・未発行時のタップは「発行→自動コピー」まで1タップに統合(文言と挙動を一致させるため`doCreate`に`alsoCopy`引数追加・`onMobileInviteTap`新設) ③地図下部の行き方帯(`.housing-tour-map-directions`)に間借りする形で短い案内文を常時表示(未ログイン=「ログインするとツアーに招待できます」/ログイン済み=「右下のボタンから招待リンクをコピーできます」、行き方データが無い場合でも案内文だけは出るよう表示条件を`footerDirections || inviteHint`に変更)。5言語i18n追加(`invite.mobile_hint_login`/`invite.mobile_hint_copy`)。対象tsc/vitest(TourNavPage+TourNavMap, 71件)・`npm run build`確認済み。**次セッション: push前ゲート(全体像の再確認)→pushのみ残**。
### ✅ 2026-08-12 大型push完了(80コミット・`e27b4429..084a885f`)= 本番push・Vercelデプロイ完了
ツアー計画画面(蛇行グリッド)/ツアー追加フィードバック演出/ハウジングツアー不具合修正一式/小粒ブラッシュアップ4件(スムーズスクロール・ツアートレイ画像リンク・登録チケット説明・マイページログインボタン統一)/セラフィズム自動変化+展開戦術追従修正/タイムライン25分延長、まとめて本番push・デプロイ完了。詳細は全て[TODO_COMPLETED.md](./TODO_COMPLETED.md)。push前ゲート: `npm run build`成功、対象変更は都度tsc/vitestで確認済み(vitestフルスイートは既知のハングのため今回もスキップ・[[reference_vitest_vmthreads_hang]])。
### 📤 Discordアップデート告知 = 統合版下書き完成・投稿待ち(ユーザー側アクション)
統合版下書き=`docs/.private/2026-08-12-discord-full-update-draft.md`(旧2下書きを統合、ハウジング13項目+共同編集3件+軽減/学者3件)。アイコンアップロード不具合の再発有無は、実際に不具合報告をしたユーザー自身が次にアップロードを試した際の反応を待つ方針(こちらから確認を仕掛けない)。修正自体(Storageルール)は完了済み。
