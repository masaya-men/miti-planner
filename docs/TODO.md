...
いずれもi18n 5言語対応・token経由。ユーザーのローカル実機確認待ち。
### ✅ ツアー計画画面(蛇行グリッド)実装+ブラッシュアップ完了(2026-08-11・未push)
探す/お気に入りの右トレイだけでは50〜100件規模のツアーは並べ替えづらい問題を解決する計画画面(PC=左に詳細パネル+右に蛇行グリッド/スマホ=縦一覧)を実装後、ユーザーと何度も実機往復しながら細部を仕上げた。設計書=`docs/superpowers/specs/2026-08-11-housing-tour-tray-page-design.md`。主な変更: ①接続線は「角丸ポリライン」方式に収束(直線+曲がる箇所だけ丸くカーブ)/②線の上を流れる光の玉アニメーション追加(SMILのbeginElement強制開始で初回スタック不具合を解消)/③マウスホイール横スクロール対応/④カードのホバー浮き上がり/⑤カードの番号バッジを撤去しタイトル表示幅を拡大(共有部品のためサイドバー/スマホ一覧にも波及)/⑥**ピン留めの設計刷新**: 「ドラッグ=自動ピン留め」「効率順に並び替え=全ピン解除してリセット」の2ルールに単純化し、旧来の「ドラッグ1回で全ピンが無反応になる」バグを解消/⑦招待前ログインボタンの文字が左寄りになる不具合・初回マウント時に線や玉が誤った位置から動いて見える不具合を修正。tsc・関連テストとも全て緑。**残: ピンボタンの視覚的な説明(hoverツールチップ)を追加するか未決定**。まとまったのでセラフィズム分・ツアー追加フィードバック分と合わせてpushタイミングを相談。
...
- その他コミット済み3件: エンピレアム21号地ナビ経路override修正([[project_housing_tour_route_fix_policy]]方式)/ハウジンガー「まとめてツアー」の既存トレイ上書きバグ修正(全5開始経路確認済み)/OGPカードSQUARE ENIX著作権表記追加(テスト2件)。
...
下書き=`docs/.private/2026-08-04-discord-housing-update-draft.md`(複数投稿URL登録/編集ページ画像管理/表示順ランダム化+初心者タグ/タグAND検索/繁体字対応/OGPカード新デザイン/タグ検索UX向上/アイコンアップロード不具合修正、計8件。マイページ作成は別便で告知済みのため除外)。ユーザー確認・投稿待ちのみ(Claudeからは直接投稿しない)。
