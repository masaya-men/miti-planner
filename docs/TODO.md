...
**✅中央地図 Phase2 + Phase3(レイアウト再編)+ツアーUX調整①〜⑨ すべて本番反映済(main `95ce47e4`・2026-07-08 実機OK→ff-merge+push+自動デプロイ・ブランチ削除済)**。内容=左=ショーケース/右=進行状況の左右入替+ステップ一覧右移設(NextDestination→ShowcasePanel改名)/マップ既定ズーム上限scale2(寄りすぎ抑制)/シロガネ黒境界道再着色/推定時間・ETA撤去/いいねハート押下FB(pop+粒)/トレイ件数のみ/お気に入り横スクロール/地図perf(重いdrop-shadow撤去)。plan=`docs/superpowers/plans/2026-07-08-housing-tour-map-phase3-layout-reorg.md`。詳細=memory [[project_housing_phase_status]]。DEV変更後はハードリロード([[reference_dev_editor_hmr_hardreload]])。
- **✅🆕生きたカード全面配線(段階2) 本番反映済(main `2c0b95a0`・2026-07-08 実機OK→ff-merge+自動デプロイ)**: `HousingShell` に `HousingPlaybackProvider` mount + 探す/お気に入りカード(`ListingCard`)・お気に入りプレビュー strip・ツアー hero を画像クロスフェード+動画spotlight(cap1・15s順送り)へ配線(旧workspace機構の再利用)。3並列監査→subagent-driven実装→opus最終レビュー(Ready to merge Yes)。spec/plan=`docs/superpowers/{specs,plans}/2026-07-08-housing-living-card-rollout*`。**残(follow-up)**=詳細peers配線(任意・シェル外) / representativeImage 3重複撤去 / `useIsScrolling` が body overflow:hidden で発火しない件(必要なら container対応)。
  - **🟢2026-07-09 セッション(同ブランチ `feat/housing-tour-panel-restructure`・未マージ・全コミット済/全緑)**: アパートのツアー地図で最寄りエーテライト名が出ない不具合修正 / ①ツアー完了オーバーレイ化(下3パネル inert+白フロスト+「素敵な時間でしたね！」) / ⑧登録URL欄 autoComplete=off(**本番ログインで要確認**) / ⑤テーマ切替の日の出(下→上)/日の入り(上→下)縦リビール(View Transitions・⑤ブラッシュアップは後日)。**⑦ハウジングスナップ住所自動入力=既に動作(og:descriptionにあり・既存OGP経路が抽出済。実データ15件検証)→新規実装不要でクローズ**。**🆕②詳細ページ「大パネル1枚」化+旧UI一掃+編集一本化に着手→別ブランチ `feat/housing-detail-panel-cleanup`(現ブランチから分岐)。spec/plan=`docs/superpowers/{specs,plans}/2026-07-09-housing-detail-panel-and-legacy-cleanup*`。P1(掃除=死にコード+旧ワークスペース経路一式+旧作成フォーム・約5950行削除)完了・全緑・未push。🔜次=P2(詳細=シェル大パネル+地図流用)→P3(編集一本化)。台帳=`.superpowers/sdd/progress.md`**。詳細=`docs/.private/2026-07-08-housing-release-feature-braindump.md`。
