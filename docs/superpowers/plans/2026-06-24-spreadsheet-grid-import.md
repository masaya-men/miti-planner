...
- 新規 `src/components/SpreadsheetGridImportModal.tsx` — 画面いっぱいモーダル UI。
...
## Task 11: モーダル UI(列ごと貼り付け→作成の最小フロー)
...
export const SpreadsheetGridImportModal: React.FC<Props> = ({ isOpen, onClose, onImport }) => {
  ...
  const handleConfirm = useCallback(async () => {
    if (!preview) return;
    const ok = await onImport(preview, { contentId: null });
    if (ok) onClose();
...
## Task 12: まるごと貼り付け + 列割当 + 枠割当(UI 統合)
...
**Interfaces:**
- Consumes: `parseGridPaste`/`isFamousSheetFormat`(Task 7), `detectField`(Task 6), `partyAssignment`(`SLOTS_BY_ROLE`/`SLOT_ROLE`/`autoFillSingles` 等)。
- Produces: モーダル内の貼り付け・列割当・枠割当ハンドラ(外部シグネチャ不変)。
...
## Task 13: Timeline / ImportMenu 配線 + 実機 + マージ
...
**Interfaces:**
- Consumes: `SpreadsheetGridImportModal`(Task 11/12), 既存 `handleSheetImport`(`onImport`・`commitImportedPlan` 経由)。
...
`npm run dev` で:
1. 自作風スプシ(member列形式)を**まるごと貼り付け** → 列が自動検出・枠割当・対象/種別が入る・未解決はskip → 作成 → タイムラインに反映。
2. **列ごと貼り付け**でも同様に作れる。
3. ダメージ列を右端に置いても検出される。計算列(最大HP 等)は無視される。
4. 4人ぶんのメンバー列でも作成できる。
5. collab-ON の表を開いた状態から取り込んでも「前の表」を引きずらない(Bug #1 回帰)。
6. 既存の有名スプシ取込([SpreadsheetImportModal])が従来通り動く。
7. 英語/中国語/韓国語モードで文言が崩れない。
...
- **2経路(まるごと/列ごと)**: Task 7(parse)+ Task 11/12(UI) ✓
- **正典列(phase任意/label/time/action/damage/target/damageType/member)**: Task 8(build)+ Task 11(UI) ✓
- **メンバー列=ジョブ名+枠割当**: Task 4(job解決)+ Task 8(party)+ Task 12(枠UI) ✓
- **列チップ青/黄/灰(緑不使用)**: Task 9 + Task 11 ✓
- **見出し自動検出(位置非依存)**: Task 6 + Task 7 + Task 12 ✓
- **4言語スキル/ジョブ解決**: Task 3 + Task 4 ✓
- **攻撃の対象列**: Task 5 + Task 8 ✓
- **4人対応**: Task 8(party は検出駆動・枠固定数なし)✓
- **有名スプシ経路温存**: 既存ファイル不変・`isFamousSheetFormat` で別経路へ(Task 7/12)・Task 13 回帰確認 ✓
- **commitImportedPlan 経由(collab安全)**: Task 13 で既存 `handleSheetImport` 再利用 ✓
- **§③テンプレ対象引き継ぎ**: 既存 `handleSheetImport` 内 `applyTemplateTargetsToResult` を通る(取込側 target 明示があれば優先=実装時 Task 13 で確認)✓
- **i18n 4言語 + rights_notice + ユーモア**: Task 10 + Task 11/12 ✓
- **マーキー(§5)**: スコープ外(別タスク)— 本計画に含めない(意図的)✓

placeholder/型整合: ステップ内コードは実型(`GridTable`/`SheetImportResult`/`PartySlot`)で一貫。`mmssToSec`/`resolveSheetSkill`/`normalizeTarget`/`buildPlanFromGrid` の名称は全タスクで一致。
