...
## 現在の状態 (次セッションはここから読む)
### 🔴 次セッション最優先: モバイル軽減表レイアウト刷新(継続中・ローカルdevで実機確認しながら進行) → その後に競合(CDかぶり)表示
発端: モバイル版でCDかぶり競合が画面に一切表示されない不具合。**ユーザー判断で着手順を「①レイアウト刷新 → ②競合表示」に確定**(理由=競合表示は過去に何度も着手→未完了/記録ミスで失敗した経緯があり慎重に進めたいため。正体=過去「③軽減競合逆方向警告=✅済」の記録がPC版のみ指していたのにスマホも完了扱いだった記録ミス)。**今回の教訓**: 手元確認だけで完了と言わず、スマホ実機でユーザーが確認するまで完了記録しない。

**①の進め方**: brainstorming用visual companionツールは実機とズレて機能せず中断・撤去済み(`.superpowers/brainstorm/`)。以降は`npm run dev -- --host`でLAN公開し、ユーザーが実機iPhoneで直接見ながら1件ずつ指示→実装→確認のループで進行中(feedback_one_fix_one_verifyそのまま)。**次セッション開始時: 開発サーバーが動いていなければ`npm run dev -- --host`を再起動し、URLをユーザーに共有してから再開**。

**①でここまで実装済(全てローカルdev確認済・未コミット・未push)**:
- `MobileTimelineRow.tsx`: 行を「時間+種別+攻撃名+対象+ダメージ(1行)」+「軽減アイコン専用行」の2行構成に分割。左24px列(旧フェーズラベル用ガター)を撤去し全幅使用。ダメージ数字のbaseline揃え(`items-baseline`+`AnimatedDamage`への`!h-[13px]`高さ上書き)。行の縦位置を`justify-start`固定(アイコン有無で時間位置が上下するバグを修正)。軽減アイコンは使用者ごとにグルーピング(`PARTY_MEMBER_IDS`固定順=PCの並び替え設定と独立)、同一人物内はgap-px(ほぼ密着)・人が変わる境目だけgap-1.5(6px)。同時刻2件目の時間表示を「〃」から実時刻に変更(i18n観点=〃は日本語専用記号で他言語ロケールに残っていたため)+間の罫線除去。
- `Timeline.tsx`: ヘッダー直下にフェーズ名を固定表示する新バー追加(`Phase1　Kefka`形式・ヘッダーと同じガラス背景で継ぎ目なし)。スクロール位置(scrollTopとtimeToYMapの差分=ピクセル空間)に連動して次フェーズが右から/現フェーズが左へスライドする「スクラブ」方式(時刻ベースだとhideEmptyRows時に飛び飛びで一足飛びに切り替わるバグがあり根治)。行ごとの旧・縦回転フェーズラベルはモバイルのみ非表示化(PC不変)。モバイル軽減追加ボトムシート(`mobileMitiFlow`)を50vh→80vhに拡大+配置済みアイコンを専用行に分離+「配置済み」判定を活性時間ベースに修正(本体行の`mitigationsByTime`と不一致だった真因を特定・修正)。
- `MobileBottomNav.tsx`: 下端に数px分のガラス背景延長(安全域の誤差でタイムラインが透けて見える不具合の保険)。

**残作業/未決事項**: 軽減アイコンサイズは18px→22pxに一旦差し戻し中(比較検討目的、確定値ではない)。グループ間gapも22px前提での再調整が必要な可能性あり。次セッションは実機で見ながらこの続きから。①完了後に②競合表示(`conflictingIds`はPC同様リング/バッジ等で個別アイコンに乗せる想定・検出ロジック自体は既存流用可)。
